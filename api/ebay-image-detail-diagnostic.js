...
// Read-only APG eBay image identity diagnostic v1.5.
...
// hides review rows, so v1.5 includes a narrowly bounded allowlist of current review/recovery item IDs;
// arbitrary item IDs cannot be supplied by callers. The Echo diagnostic now targets the current white
// 3rd-gen listing after the charcoal listing resolved to an external seller image host.
...
async function handler(req,res){
  ...
  try{
    ...
    const detail=await ebay.getItem(clean(state.item_id),{referenceId:`apg:${slug}:image-diagnostic-v15`,timeoutMs:10000});
