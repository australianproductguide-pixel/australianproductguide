...
async function readJsonBody(req,limit=20000){return new Promise((resolve,reject)=>{let raw='';req.on('data',c=>{raw+=c;if(raw.length>limit){reject(new Error('too_large'));req.destroy();}});req.on('end',()=>{try{resolve(raw?JSON.parse(raw):{});}catch{reject(new Error('invalid_json'));}});req.on('error',reject);});}
...
function signInPage(id,message=''){
  return `<span class="k">APG private operator</span><h1>Authorise ChatGPT</h1><p class="muted">Sign in with the operator identity approved for APG Growth Intelligence.</p><div class="note"><strong>This is not a My APG consumer profile.</strong><br>This sign-in is used only to approve the private ChatGPT connector and does not sign you into saved products, account sync or marketing preferences.</div><form class="login" method="post" action="/oauth/consent"><input type="hidden" name="authorization_id" value="${esc(id)}"><input type="hidden" name="action" value="operator_login"><label>Operator email<input name="email" type="email" autocomplete="username" required></label><label>Operator password<input name="password" type="password" autocomplete="current-password" required></label><div class="actions" style="margin-top:18px"><button class="primary" type="submit">Continue to authorisation</button></div><p class="error">${esc(message)}</p></form>`;
}
...
module.exports=async function handler(req,res){
  try{
    ...
    if(!authorizationId)return html(res,400,'<span class="k">APG private operator</span><h1>Invalid authorisation request</h1><p class="muted">The OAuth authorisation identifier is missing. Start the connection again from ChatGPT.</p>');
    ...
    if(req.method==='POST'&&action==='operator_login'){
      ...
      if(!email||!password)return html(res,401,signInPage(authorizationId,'Enter the approved operator email and password.'));
      const login=await signInOperator(email,password);
      if(!login.ok||!login.data?.access_token)return html(res,401,signInPage(authorizationId,'Operator email or password is incorrect.'));
      const operator=await auth.validateUserOperator(login.data.access_token);
      if(!operator.ok)return html(res,403,'<span class="k">APG private operator</span><h1>Access not authorised</h1><p class="muted">This identity is not approved to operate the private APG Growth Intelligence connector.</p>');
    ...
    if(!operator.ok){
      ...
      return html(res,403,'<span class="k">APG private operator</span><h1>Access not authorised</h1><p class="muted">This identity is not approved to operate the private APG Growth Intelligence connector.</p>');
    }
    if(req.method==='POST'){
      ...
      clearOperatorCookie(res);
      if(!result.ok||!result.data?.redirect_url)return html(res,400,`<span class="k">APG private operator</span><h1>Authorisation could not be completed</h1><p class="muted">${esc(result.data?.msg||result.data?.message||result.data?.error_description||'Please restart the connection from ChatGPT.')}</p>`);
    ...
    if(!details.ok)return html(res,400,`<span class="k">APG private operator</span><h1>Authorisation request unavailable</h1><p class="muted">${esc(details.data?.msg||details.data?.message||details.data?.error_description||'Please restart the connection from ChatGPT.')}</p>`);
