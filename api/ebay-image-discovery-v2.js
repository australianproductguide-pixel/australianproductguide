...
// APG eBay image discovery worker v2.18
// Broader exact-product recall for the 482-product image-completion programme.
// v2.18 uses catalogue-enrichment v1.11's governed media selector so an unsupported eBay catalogue
// image can no longer mask a valid HTTPS i.ebayimg.com listing/summary image from the same exact item.
// Product identity, category, condition, voltage, variant, AUD-price, active-listing and item-URL gates
// remain unchanged. The failed Crucial X9 Pro direct retrieval has also been removed to avoid spending
// a Browse call on an item eBay's Browse detail endpoint does not expose. Public browsing makes no calls.
...
async function insertDiscoveredState(workerToken,product,candidate){
  return supabase.rpc('apg_insert_ebay_image_state',{p_proof:workerToken,p_payload:discoveryPayload(product,candidate)},{timeoutMs:6000});
}
...
async function verifyImageCandidate(product,candidate){
  let detail;
  try{detail=await ebay.getItem(candidate.itemId,{referenceId:`apg:${product.slug}:image-discovery-v218`,timeoutMs:10000});}
  ...
  const verified={
    ...
    imageSource:imageUrl===catalogImage?'ebay-product-catalog':'ebay-listing',
  ...
  if(!familyChecked||familyChecked.status!=='accept'||!familyChecked.accepted)return {ok:false,reason:familyChecked&&familyChecked.familyGuard&&familyChecked.familyGuard.reason||'family-variant-guard'};
...
function searchRequest(plan){return {q:plan.q,gtin:plan.gtin,epid:plan.epid,categoryIds:plan.categoryIds,limit:plan.limit,filter:plan.filter,aspectFilter:plan.aspectFilter,sort:plan.sort};}
function publicPlan(plan){return {kind:plan.kind,q:plan.q||null,gtin:plan.gtin||null,epid:plan.epid||null,categoryIds:plan.categoryIds||null,limit:plan.limit,filter:plan.filter||null};}
async function discoverProduct(product,budget){
  ...
  const seen=new Map(),searchErrors=[],rejects=[];let calls=0;
  ...
  if(directItemId&&budget.remaining>0){
    ...
    if(verified.ok)return {ok:true,candidate:verified.candidate,plans:plans.map(publicPlan),calls,candidateCount:1,searchErrors,directItemId};
  ...
  for(let index=0;index<plans.length;index+=1){
    ...
    try{result=await ebay.searchItems(searchRequest(plan),{referenceId:`apg:${product.slug}:image-search-v218:${index+1}`,timeoutMs:10000});}
  ...
  const candidates=[...seen.values()].sort((a,b)=>(b.score-a.score)||(b.modelCoverage-a.modelCoverage)||(b.nameCoverage-a.nameCoverage)||(a.queryIndex-b.queryIndex)).slice(0,MAX_DETAIL_CHECKS_PER_PRODUCT);
  for(const candidate of candidates){
    if(budget.remaining<1)break;budget.remaining-=1;calls+=1;
    const verified=await verifyImageCandidate(product,candidate);
    if(verified.ok)return {ok:true,candidate:verified.candidate,plans:plans.map(publicPlan),calls,candidateCount:candidates.length+(directItemId?1:0),searchErrors,directItemId:directItemId||null};
...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    if(available<MAX_CALLS_PER_PRODUCT)return json(res,200,{ok:true,status:'quota-reserved',version:VERSION,searchPlanVersion:searchPlan.VERSION,quota:quotaPublic(summary),processed:0});
    ...
    if(!rows.length)return json(res,200,{ok:true,status:'nothing-due',version:VERSION,searchPlanVersion:searchPlan.VERSION,quota:quotaPublic(summary),processed:0});
    ...
    for(const row of rows){
      ...
      let found;try{found=await discoverProduct(product,budget);}catch(error){found={ok:false,error:clean(error&&error.code)||'DISCOVERY_V218_ERROR',calls:0,plans:[]};}
      if(found.ok){
        ...
        const status=found.candidateCount?'review':'no-match';await recordDiscoveryResult(workerToken,slug,status,found.error||'DISCOVERY_V218_NOT_ACCEPTED');
  ...
  }catch(error){return json(res,500,{ok:false,status:'worker-error',version:VERSION,searchPlanVersion:searchPlan.VERSION,code:clean(error&&error.code)||'EBAY_DISCOVERY_V218_ERROR'});}
