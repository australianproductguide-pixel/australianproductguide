...
assert.equal(growth.providerAudience(),'//iam.googleapis.com/projects/4015856724/locations/global/workloadIdentityPools/apg-vercel-oidc/providers/apg-vercel');
...
assert(clientSource.includes("import('@vercel/oidc')"),'must use the supported Vercel OIDC runtime helper');
assert(clientSource.includes('getVercelOidcToken'),'must retrieve OIDC from Vercel function request context');
assert(!clientSource.includes('process.env.VERCEL_OIDC_TOKEN'),'must not assume the OIDC token is exposed as a plain runtime environment variable');
...
assert(apiSource.includes("process.env.VERCEL_ENV!=='production'"),'preview-only detailed diagnostic allowance must remain explicit');
assert(apiSource.includes('APG_GROWTH_API_TOKEN'),'Production detailed diagnostics must require a separate private token');
