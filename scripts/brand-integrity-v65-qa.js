...
assert(entry.includes('const finalHandler=brandLogoStability.wrap(certifiedHandler)'),'public entrypoint must keep governed brand identity outermost after certification');
...
assert(stability.includes("pathname.match(/^\\/assets\\/brand-marks"),'v125 must intercept governed brand-mark asset requests');
