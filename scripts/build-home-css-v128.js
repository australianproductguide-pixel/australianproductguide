...
// Brotli/gzip sidecars. It also atomically writes the small bundle manifest consumed by the next
// serverless process, eliminating public-request CSS file I/O while retaining exact signature
// validation. A failed build leaves the source-controlled fallback manifest intact.
...
const baselineManifest=require('../data/home-css-v128-manifest');
...
const MANIFEST_OUTPUT=path.join(ROOT,'data','home-css-v128-manifest.js');
...
function removeOutputs(){
  for(const filename of [OUTPUT,BROTLI_OUTPUT,GZIP_OUTPUT,OUTPUT+'.tmp',BROTLI_OUTPUT+'.tmp',GZIP_OUTPUT+'.tmp',MANIFEST_OUTPUT+'.tmp']){
...
function sourceSha(){
  for(const candidate of [process.env.VERCEL_GIT_COMMIT_SHA,process.env.GITHUB_HEAD_SHA,process.env.GITHUB_SHA,baselineManifest.sourceSha]){
    const value=String(candidate||'').trim().toLowerCase();
    if(/^[a-f0-9]{40}$/.test(value))return value;
  }
  throw new Error('No valid 40-character source SHA is available for the Home bundle manifest');
}
function manifestModule(manifest){
  return `'use strict';\n\n// Generated by scripts/build-home-css-v128.js. Public runtime consumes this small manifest only;\n// the browser fetches the generated CSS through Vercel's filesystem route.\nmodule.exports=Object.freeze(${JSON.stringify(manifest,null,2)});\n`;
}
async function build(){
  ...
  const manifest=Object.freeze({
    version:'128.2',
    sourceSha:sourceSha(),
    sourceStylesheets:links.length,
    expandedBytes,
    bytes:buffer.length,
    brotliBytes:compressed.br.length,
    gzipBytes:compressed.gzip.length,
    linkSignature:signature,
    sha256:hash
  });

  // Write all derived assets before replacing the small manifest. If any earlier write fails, the
  // checked-in manifest remains available and runtime substitution fails closed rather than
  // pointing at incomplete output.
  atomicWrite(OUTPUT,buffer);
  atomicWrite(BROTLI_OUTPUT,compressed.br);
  atomicWrite(GZIP_OUTPUT,compressed.gzip);
  atomicWrite(MANIFEST_OUTPUT,Buffer.from(manifestModule(manifest),'utf8'));

  const compactRatio=(buffer.length/expandedBytes).toFixed(4);
  console.log(`APG_HOME_CSS_BUNDLE_V128=PASS styles=${links.length} expandedBytes=${expandedBytes} bytes=${buffer.length} compactRatio=${compactRatio} br=${compressed.br.length} gzip=${compressed.gzip.length} linkSignature=${signature} sha256=${hash} manifestSource=${manifest.sourceSha}`);
}

build().catch(error=>{
  // Generated CSS/sidecars are removed on failure. The source-controlled manifest is not deleted;
  // a stale signature therefore restores the established cascade instead of referencing a partial
  // asset. The temporary manifest is removed by removeOutputs().
  removeOutputs();
