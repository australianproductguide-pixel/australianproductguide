...
// reads static CSS from source and generated CSS through APG's own handler, preserves cascade
// order and media conditions, rewrites relative asset URLs and writes one deterministic bundle.
...
const PRESERVED_PATHS=new Set([
  '/assets/desktop-home-header-v126.css',
  '/assets/desktop-about-trust-contrast-v127.css',
...
function stylesheetLinks(html){
  ...
  for(const match of head.matchAll(/<link\b[^>]*>/gi)){
    ...
    if(url.pathname==='/assets/apg-home-v128.css'||PRESERVED_PATHS.has(url.pathname))continue;
...
async function main(){
  ...
  if(links.length<MIN_STYLES)throw new Error(`Expected at least ${MIN_STYLES} bundleable Home stylesheets, found ${links.length}`);
  ...
  fs.writeFileSync(MANIFEST_PATH,JSON.stringify({version:VERSION,generatedAt:new Date().toISOString(),source:'maintained SSR Home and same-origin APG CSS',internetRequests:0,stylesheetCount:manifest.length,preservedViewportStyles:[...PRESERVED_PATHS],output:{path:path.relative(ROOT,OUTPUT_PATH),bytes:Buffer.byteLength(output),sha256:hash(output)},stylesheets:manifest},null,2)+'\n');
  console.log(`APG_HOME_CSS_BUNDLE_V128=PASS version=${VERSION} styles=${manifest.length} bytes=${Buffer.byteLength(output)} sha256=${hash(output).slice(0,16)} externalNetwork=0`);
}
