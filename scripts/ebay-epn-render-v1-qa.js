...
(async()=>{
  ...
  const v112=fs.readFileSync(path.join(__dirname,'..','lib','premium-mobile-decision-commerce-v112-runtime.js'),'utf8');assert.match(v112,/@media\s*\([^)]*max-width/i,'v112 must retain an explicit mobile responsive contract');assert.match(v112,/apg112-retailer-row/,'Responsive v112 retailer-row styling must remain present');const source=fs.readFileSync(path.join(__dirname,'..','lib','ebay-epn-surface-v1-runtime.js'),'utf8');assert(!/user-agent|mobile\s*===|desktop\s*===/i.test(source),'eBay retailer truth must not fork by device/user agent');
