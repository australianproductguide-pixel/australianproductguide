...
assert(entry.includes("module.exports=require('../lib/brand-mark-complete-v67')"),'public entrypoint must use current complete brand identity v67');
assert(brandMarkComplete.includes("require('./brand-mark-device-parity-v66')"),'v67 must preserve v66.2 parity/integrity immediately underneath');
assert(brandMarkComplete.includes("BRAND_MARK_COMPLETE_VERSION='67.0'"),'v67 must expose the current complete-brand generation');
assert(brandMarkComplete.includes("BRAND_MARK_ASSET_VERSION='67.0'"),'v67 must use one cache-busted identity generation across desktop/mobile');
assert(brandMarkComplete.includes('official-domain-declared-identity'),'v67 must identify official-domain declared marks separately');
assert(brandMarkComplete.includes('official-domain-favicon-cache'),'v67 must expose governed official-domain favicon fallback provenance');
assert(brandMarkComplete.includes('drop_404_icon=true'),'v67 must refuse a generic default favicon');
assert(brandMarkComplete.includes("String(image.assetKind||'').toLowerCase()!=='brand_img'"),'v67 must preserve rejection of generic product/lifestyle candidates');
...
console.log('APG Search Brand Identity v59 source QA passed beneath v60, v61, v62, CSP-safe v63, product brand placeholder v64, brand mark quality v65, curated brand mark v66, parity/integrity v66.2 and complete official brand identity resolver v67');