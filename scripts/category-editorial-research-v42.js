...
function normalise(r,q){
  ...
  return {id,title:String(r.title||keys.slice(0,3).join(' ')||q),creator:String(r.user_name||r.author_name||r.author||r.username||''),landingUrl:id?`https://stocksnap.io/photo/${prefix}-${id}`:'',imageUrl:id?`https://cdn.stocksnap.io/img-thumbs/960w/${prefix}_${id}.jpg`:'',width:Number(r.width||0),height:Number(r.height||0),keywords:keys.slice(0,20),views:Number(r.page_views||r.views||0),downloads:Number(r.downloads||0),favourites:Number(r.favorites||r.favourites||0),source:'stocksnap',licence:'cc0',query:q};
}
function score(x,q){
