...
assert.equal(brand.FINAL_CSS_VERSION,'46.3');
...
for(const approved of ['#FFD65B','#F4BB45','#F2B348']){
  assert(!finalCss.includes(approved),`yellow must not leak into standard UI cleanup: ${approved}`);
  assert(!v47Source.includes(approved),`yellow must not leak into Consumer Intelligence v47: ${approved}`);
}
...
assert(once.includes('/assets/brand-system-v46-final.css?v=46.3'),'v46 final cleanup stylesheet link missing');
...
assert(once.indexOf('/assets/brand-system-v46-final.css?v=46.3')>once.indexOf('/assets/brand-system-v46-imagery.css?v=46'),'final standard-UI cleanup must load after general imagery styling');
assert(once.indexOf('/assets/brand-system-v46-research-proof.css?v=46.2')>once.indexOf('/assets/brand-system-v46-final.css?v=46.3'),'research-proof exception must remain the final v46 stylesheet');
...
assert(v47Source.includes('@media(max-width:700px)'),'Consumer Intelligence v47 must retain mobile layout coverage');

console.log('APG Brand System v46/v47 source QA passed: palette=PASS typography=PASS account=PASS search=PASS scout=PASS research=PASS shopping-v39=PASS shopping-v41=PASS imagery=PASS v47=PASS legacy-colour-cleanup=PASS maintained-research-yellow-exception=PASS contrast=PASS responsive=PASS');
