...
for(const contract of [
 "window.__APG_SEARCH_RESILIENCE_V52__",'new AbortController()','DEADLINE_MS=10000',"Accept:'application/json'","'X-APG-Search-JSON':'1'",'response.json()',"payload.version!=='search-ranking-v4'",'payload.commercialRecommendationWeight!==0','main.replaceChildren(root)','history.pushState',"form.matches('form[data-search-shell]')",'event.preventDefault()','event.stopImmediatePropagation()','migrateRecentSearches()','moveVisibleSuggestion(event,input,1)','moveVisibleSuggestion(event,input,-1)','[data-apg-history-show]','search_deadline_exceeded'
...
const transformed=api.ensureSearchController(runtime.inject(html)),v52=transformed.indexOf('/assets/search-resilience-v52.js?v=52.0'),app=transformed.indexOf('/assets/app.js');
assert(v52>=0&&app>=0&&v52<app,'composed P0 runtime must register Search v52 before legacy app.js');
assert(transformed.includes('/assets/search-resilience-v52.js.css?v=52.0'),'Search v52 CSS asset must be present');
assert.equal((transformed.match(/search-resilience-v52\.js\?v=52\.0/g)||[]).length,1,'Search v52 controller must be injected exactly once');
