...
const PATCH='decision-lab-p0-2026-08-20-soft-nav-r2';
...
async function submitSoft(page,input,trigger=null){
 ...
 assert(state.marker===PATCH,'v50.1 controller marker disappeared after soft render');
...
async function journey(browser,name,viewport,fn){
 ...
 try{await fn(page);report.journeys.push({name,ok:true,durationMs:Date.now()-started})}
   ...
   const state=await readOutcomeState(page).catch(()=>null);
