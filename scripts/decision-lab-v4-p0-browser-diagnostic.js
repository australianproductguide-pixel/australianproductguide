...
async function go(page,url='/decision-lab/'){
 ...
 await page.waitForFunction(p=>window.__APG_DECISION_LAB_RESILIENCE_V50__===p,{timeout:10000},PATCH);
...
async function waitForOutcome(page,input,timeout=15000){
 ...
 await page.waitForFunction(exp=>{
   ...
   const paramsOk=Object.entries(exp).every(([k,v])=>p.get(k)===String(v));
   ...
   return location.pathname==='/decision-lab/'&&paramsOk&&!!outcome&&!!form&&!form.hasAttribute('aria-busy')&&!button?.disabled&&!button?.hasAttribute('aria-busy');
 },{timeout},expect);
}
async function submitSoft(page,input,trigger=null){
 ...
 await waitForOutcome(page,input);
 await dismissConsent(page);
 ...
 const state=await page.evaluate(()=>({
   results:document.querySelectorAll('.decision-result').length,
   zero:!!document.querySelector('.zero-state'),
   recovery:!!document.querySelector('.decision-server-recovery'),
   busy:document.querySelector('form.decision-form')?.getAttribute('aria-busy'),
   disabled:document.querySelector('form.decision-form button[type="submit"]')?.disabled,
   text:document.querySelector('main#main')?.innerText||'',
   marker:window.__APG_DECISION_LAB_RESILIENCE_V50__
 }));
 assert(state.marker===PATCH,'v50 controller marker disappeared after soft render');
 assert(state.results>0||state.zero||state.recovery,'no controlled Decision Lab outcome');
 assert(!state.busy&&!state.disabled,'Decision Lab remained busy after outcome');
 return state;
}
async function journey(browser,name,viewport,fn){
 ...
 catch(e){report.journeys.push({name,ok:false,durationMs:Date.now()-started,error:e.message});try{await page.screenshot({path:path.join(OUT,`${name}.png`),fullPage:false})}catch{}}
...
(async()=>{
 ...
 const matrix=[
  ...
  ['desktop-vague',{q:'Something useful for a tiny apartment but not expensive.',budget:'100'}],
 ...
 await journey(browser,'desktop-duplicate-submit-single-fetch',desktop,async page=>{const input={q:'quiet headphones under $500 for commuting',budget:'500'};await go(page);await setForm(page,input);const before=report.network.length;await page.evaluate(()=>{const f=document.querySelector('form.decision-form');f.requestSubmit();f.requestSubmit()});await waitForOutcome(page,input);const slice=report.network.slice(before);const fetches=slice.filter(x=>x.type==='fetch'&&new URL(x.url).pathname==='/decision-lab/');assert(fetches.length===1,`duplicate submit emitted ${fetches.length} Decision Lab fetches`)});
