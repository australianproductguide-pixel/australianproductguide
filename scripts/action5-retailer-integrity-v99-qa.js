...
assert.equal(x.amazon.exact,20,'Verified exact mappings must remain evidence-backed, including Philips NA353/10');
assert.equal(x.amazon.variant,13,'Verified variant mappings must remain evidence-backed');
assert.equal(x.amazon.fallback,448,'Only unresolved products may remain on active retailer-search fallbacks');
...
assert.equal(x.p1.resolved.find(r=>r.productId==='philips-3000-series-dual-basket-na35310')?.amazonState,'EXACT_VERIFIED','Philips NA353/10 P1 resolution must reconcile to its newly verified direct state');
assert.equal(x.priority.counts.P1,0);
assert.equal(x.priority.counts.P2,60,'P2 must preserve the current governed demand queue after the P1 Philips direct promotion');
...
require('./ebay-epn-interim-v1-qa');
