...
assert.equal(x.amazon.exact,19,'Verified exact mappings must remain evidence-backed');
assert.equal(x.amazon.variant,13,'Verified variant mappings must remain evidence-backed');
assert.equal(x.amazon.fallback,449,'Only unresolved products may remain on active retailer-search fallbacks');
...
assert.equal(x.priority.counts.P2,60,'P2 must shrink as verified direct mappings are promoted from the governed candidate queue');
