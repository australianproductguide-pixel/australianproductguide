...
test('A7-C04',()=>{const rule=criterion('wireless-headphones','travel');assert(rule.interpretationRule);const t={criteria:[{kind:'decision',requested:'highest',evidenceStatus:'VERIFIED',evidenceConfidence:'medium',explanationEligible:true,productValue:'strong',note:'Rule-derived from maintained exact-model ANC, battery and portability evidence.'}],conflicts:[]};assert.strictEqual(action7.classifyTrace(t),'INFERRED');});
