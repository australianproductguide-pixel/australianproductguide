...
for(const row of data.categories){
  for(const c of row.candidates){
    const text=`${c.title||''} ${c.description||''} ${c.categories||''}`;
    let adjustment=0;
...
data.policy.selection='Premium-first relevance scoring + APG second-pass curation that demotes ambiguous, dated, technical and non-consumer hero imagery.';
data.summary.withCandidates=data.categories.filter(r=>r.candidates.length).length;
data.summary.withPremiumCandidates=data.categories.filter(r=>r.candidates.some(c=>Number(c.score)>=90)).length;
fs.writeFileSync(candidatesPath,JSON.stringify(data,null,2)+'\n');

const rows=data.categories.map(r=>({slug:r.slug,label:r.label,query:r.query,error:r.error,selected:r.candidates[0]||null,candidateCount:r.candidates.length}));
