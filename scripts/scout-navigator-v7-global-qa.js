...
(async()=>{
  ...
  assert.equal(navigator.HOME_RUNTIME_STATE,'RESTORED_HEADER_MARKETPLACE_SMART_PLACEMENT_BYPASS');
  ...
  for(const route of routes){
    ...
    if(isHome)assert.equal(response.headers['x-apg-scout-navigator-home-runtime'],navigator.HOME_RUNTIME_STATE,'Home must disclose restored marketplace header with Smart Placement isolated');
  ...
  assert(runtimeSource.includes('return headerDownstream(req,res);'),'Home must keep marketplace header while bypassing only Smart Placement');
  assert(runtimeSource.includes('return smartDownstream(req,res);'),'non-Home routes must retain Smart Placement composition');

  console.log(JSON.stringify({version:navigator.VERSION,status:'PASS',routesChecked:routes.length,checks:{marketplaceHeaderAllRoutes:true,homeSmartPlacementIsolated:true,navigatorFinalCascade:true,runtimeRecursiveCaptureDisabled:true,decisionLogicUntouched:true,commercialScoringUntouched:true}},null,2));
