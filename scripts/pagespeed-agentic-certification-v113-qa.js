...
const assets=new Map([
  ...
  ['/assets/feature.css?v=1',['text/css; charset=utf-8','.feature{display:grid}']],
...
const home='<!doctype html><html><head><title>Home</title><link rel="stylesheet" href="/assets/core.css?scope=core&v=test"><link rel="stylesheet" href="/assets/feature.css?v=1"><link rel="stylesheet" href="/assets/final.css?v=2"></head><body><main><h1>APG</h1></main><aside id="apgAssistantPanel" hidden aria-hidden="true"><button>Close</button></aside></body></html>';
const about='<!doctype html><html><head><title>About</title><link rel="stylesheet" href="/assets/core.css?scope=core&v=test"><link rel="stylesheet" href="/assets/feature.css?v=1"></head><body><main><h1>About</h1></main></body></html>';
...
assert.equal(layer.VERSION,'113.0');
...
assert.equal(homepage.headers.get('x-apg-pagespeed-agentic-certification'),'v113.0');
assert(homepage.body.includes('name="apg-pagespeed-agentic-certification" content="v113.0"'));
assert(homepage.body.includes('/assets/pagespeed-home-v113.css?v='));
assert.equal((homepage.body.match(/rel="stylesheet"/g)||[]).length,1,'homepage must expose one render-blocking stylesheet request');
...
assert(css.body.includes('.feature{display:grid}'));
...
assert.equal((aboutPage.body.match(/rel="stylesheet"/g)||[]).length,2,'non-home routes must keep their established stylesheet delivery');
...
assert.equal(request(installedHandler,'/').headers.get('x-apg-pagespeed-agentic-certification'),'v113.0');

console.log(JSON.stringify({
  ok:true,
  version:layer.VERSION,
  homepageStylesheetRequests:1,
