...
assert.equal(runtime.FOOTER_NAVIGATION_VERSION,'83.2','footer runtime version');
assert.equal(api.FOOTER_NAVIGATION_VERSION,'83.2','api outer runtime must expose footer v83.2');
...
assert.equal(api.TRUST_CENTRE_VERSION,'82.1','Trust Centre v82.1 must remain underneath footer v83.2');
...
assert.match(injected,/\/assets\/footer-navigation-v83\.css\?v=83\.2/,'footer CSS injection');
assert.match(injected,/\/assets\/footer-navigation-v83\.js\?v=83\.2/,'footer JS injection');
...
assert.match(css,/grid-template-columns:minmax\(0,1fr\) minmax\(0,1fr\)!important/,'standard mobile footer must use two columns');
assert.match(css,/padding:10px 0!important/,'mobile link text must align exactly with group heading');
assert.match(css,/\.apg-cookie-footer-button\{/,'cookie footer utility treatment required');
assert.match(css,/font-size:12\.5px!important/,'cookie footer utility text must remain visually subordinate');
assert.match(css,/min-height:40px!important/,'cookie footer utility must remain comfortably tappable');
assert.match(css,/border-radius:999px!important/,'cookie footer utility uses compact pill treatment');
assert.match(css,/@media\(max-width:340px\)/,'exceptionally narrow mobile fallback required');
assert.match(css,/grid-template-columns:1fr!important/,'narrow mobile fallback must use one column');
...
assert.match(js,/__APG_FOOTER_NAVIGATION_V83__='83\.2'/,'browser guard must expose v83.2');
...
console.log('FOOTER_NAVIGATION_V83_SOURCE=PASS version=83.2 mobile=2x2 aligned cookie=compact search=52.0 decision=50.6 trust=82.1');
