...
const registry=require('../data/category-editorial-images-v44');
const finalReview=require('../data/category-editorial-final-review-v44.json');
...
if(!pages.includes("categoryEditorialImages=require('../data/category-editorial-images-v44')"))fail('Category image registry not wired into pages');
if(!pages.includes('function categoryHeroMedia(c)')||!pages.includes('${categoryHeroMedia(c)}'))fail('Category hero renderer not wired');
if(!pages.includes('Editorial category scene — not a reviewed product.'))fail('Consumer-facing non-product disclaimer missing');
const css=fs.readFileSync(path.join(root,'lib','premium-css.js'),'utf8');
for(const token of ['.category-hero-media{','.category-hero-media-overlay{','.category-hero-media figcaption{'])if(!css.includes(token))fail(`Missing editorial hero CSS ${token}`);
console.log(JSON.stringify({version:'category-editorial-v44-qa',categories:90,uniqueSources:sources.size,selfHostedAssets:srcs.size,totalBytes:bytes,manualOverrides:finalReview.summary.manualOverrides,manualApproved:finalReview.summary.manualApproved,autoReviewed:finalReview.summary.autoReviewed,failures:0},null,2));
