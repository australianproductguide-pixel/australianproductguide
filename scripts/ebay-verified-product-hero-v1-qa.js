...
for(const slug of slugs){
  const row=registry.forSlug(slug);
  assert(hero.validateDetail(slug,row,detailFor(slug),{now:Date.parse('2026-08-31T00:00:00Z')}),`${slug} valid exact detail should pass`);
...
// Rate-limit helpers remain covered even though public product pages no longer make Browse calls.
...
(async()=>{
  ...
  const result=await hero.inject(sample,`/products/${sampleSlug}/`,{
    now:()=>freshNow,
    getItem:async()=>{networkCalls+=1;throw new Error('public pilot hero must never call eBay');}
  });
  assert.strictEqual(result.usedEbayImage,true,'fresh verified pilot registry row should produce hero image');
  ...
  assert(result.html.includes('verified within freshness window'),'public copy must disclose freshness model');
  assert(!result.html.includes('verified at render time'),'public copy must not claim request-time eBay verification');
  assert(result.html.includes(canonical),'canonical must remain unchanged after registry injection');
  assert(result.html.includes(jsonLd),'Product JSON-LD must remain unchanged after registry injection');

  const fresh=await hero.currentDetail(sampleSlug,sampleRow,{now:()=>freshNow,getItem:async()=>{networkCalls+=1;}});
  ...
  assert.strictEqual(networkCalls,0,'currentDetail must ignore any injected network callback');

  hero.cache.clear();
  let expiredRejected=false;
  try{
    await hero.currentDetail(sampleSlug,sampleRow,{now:()=>observed+hero.REGISTRY_FALLBACK_MAX_AGE_MS+1,getItem:async()=>{networkCalls+=1;}});
  }catch(error){expiredRejected=error&&error.code==='EBAY_HERO_REGISTRY_STALE';}
  assert.strictEqual(expiredRejected,true,'pilot registry must fail closed after five hours');
  assert.strictEqual(networkCalls,0,'stale pilot must not attempt eBay network fallback');

  hero.cache.clear();
  const stale=await hero.inject(sample,`/products/${sampleSlug}/`,{now:()=>observed+hero.REGISTRY_FALLBACK_MAX_AGE_MS+1,getItem:async()=>{networkCalls+=1;}});
