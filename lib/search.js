...
function tokenScore(q,p){const tokens=words(q).filter(t=>t.length>1&&!['best','for','the','with','and','under','below','versus','vs'].includes(t));const hay=words([p.brand,p.name,p.summary,p.categoryLabel,(p.tags||[]).join(' ')].join(' '));let score=0;for(const t of tokens){if(hay.includes(t))score+=8;else if(hay.some(h=>h.startsWith(t)||t.startsWith(h)))score+=4;else if(t.length>=4&&hay.some(h=>Math.abs(h.length-t.length)<=1&&distance(t,h)<=1))score+=2;}const nn=norm(q),name=norm(p.name),brand=norm(p.brand);if(nn.includes(name))score+=40;if(nn.includes(brand))score+=14;return score;}
...
function searchSite(query){
  const q=String(query||'').trim(),n=norm(q),tags=extractTags(q),budget=extractBudget(q),catHit=categoryFromQuery(q),category=catHit?.score?categories[catHit.slug]:null;
  const productResults=products.map(p=>{let score=tokenScore(q,p);if(category&&p.category===category.slug)score+=18;for(const tag of tags)if(p.tags.includes(tag))score+=13;if(budget&&p.price){if(p.price<=budget)score+=18;else score-=30;}score+=(p.tags||[]).length*.2;return {p,score};}).filter(x=>x.score>0).sort((a,b)=>b.score-a.score).slice(0,12).map(x=>x.p);
  ...
  const interpretation=[];if(category)interpretation.push(category.label);if(tags.length)interpretation.push(...tags.map(x=>x.replace(/-/g,' ')));if(budget)interpretation.push(`under A$${budget.toLocaleString('en-AU')}`);
  return {q,products:productResults,categories:categoryResults.slice(0,4),comparisons:comparisonResults,guides:guideResults,directCompare,interpretation,budget,tags};
}
...
module.exports={searchSite,searchIndex,matchProduct,norm};
