...
// APG Action 3 Search Commerce v90.1.
// Confidence-gated retailer actions for Search plus cross-surface retailer-state
// reconciliation. Recommendation/ranking remains upstream and affiliate economics
...
function variantLinkCopy(anchor){
  return anchor
    .replace(/View on Amazon Australia/g,'View available variant on Amazon Australia')
    .replace(/View on Amazon AU/g,'View available variant on Amazon AU')
    .replace(/Check exact product on Amazon Australia/g,'View available variant on Amazon Australia')
    .replace(/Check exact product on Amazon AU/g,'View available variant on Amazon AU');
}
function fallbackLinkCopy(anchor){
  return anchor
    .replace(/View on Amazon Australia/g,'Search this model on Amazon Australia')
    .replace(/View on Amazon AU/g,'Search this model on Amazon AU');
}

function reconcileAmazonAffiliateMarkup(body){
  if(typeof body!=='string'||!body.includes('data-affiliate-retailer="Amazon Australia"'))return body;
  const pageSlug=(body.match(/<body\b[^>]*\bdata-product-slug="([^"]+)"/i)||[])[1]||'';
  const pageState=pageSlug?retailerState(productBySlug.get(pageSlug)):{status:'UNAVAILABLE'};
  let out=body.replace(/<a\b[^>]*\bdata-affiliate-retailer="Amazon Australia"[^>]*>[\s\S]*?<\/a>/gi,anchor=>{
    const openEnd=anchor.indexOf('>');
    if(openEnd<0)return anchor;
    let open=anchor.slice(0,openEnd+1),rest=anchor.slice(openEnd+1);
    const slug=attrValue(open,'data-product-slug')||pageSlug;
    if(!slug)return anchor;
    const state=retailerState(productBySlug.get(slug));
    if(state.status==='UNAVAILABLE'||!state.url)return anchor;
    open=setAttr(open,'href',state.url);
    open=setAttr(open,'data-product-slug',slug);
    open=setAttr(open,'data-affiliate-kind',state.kind);
    open=setAttr(open,'data-affiliate-destination',state.destination);
    let reconciled=open+rest;
    if(state.status==='VARIANT_VERIFIED')reconciled=variantLinkCopy(reconciled);
    else if(state.status==='SEARCH_FALLBACK')reconciled=fallbackLinkCopy(reconciled);
    return reconciled;
  });

  // Product-page prose must use the same canonical retailer state as the links.
  // These replacements are deliberately Amazon-specific and do not alter verified
  // exact-model claims for non-affiliate retailers such as JB Hi-Fi.
  if(pageState.status==='VARIANT_VERIFIED'){
    out=out
      .replace(/data-amazon-link-type="exact"/g,'data-amazon-link-type="variant"')
      .replace(/<strong>Paid Amazon Associate link\.<\/strong> Exact Amazon product destination verified\./g,'<strong>Paid Amazon Associate link.</strong> Verified Amazon variant destination. Check the selected variant before purchase.')
      .replace(/Exact Amazon Australia individual product listing verified\./g,'Verified Amazon Australia variant listing. Check the selected variant before purchase.')
      .replace(/Paid link · Amazon Associate · Exact individual product page verified/g,'Paid link · Amazon Associate · Verified variant listing')
      .replace(/Ready to check the exact product\?/g,'Ready to check the available variant?')
      .replace(/Verified destination · paid Amazon Associate link/g,'Verified variant · paid Amazon Associate link')
      .replace(/Move from research to the exact product/g,'Move from research to a verified retailer option');
  } else if(pageState.status==='SEARCH_FALLBACK'){
    out=out.replace(/data-amazon-link-type="exact"/g,'data-amazon-link-type="search"');
  }
  return out;
}
  ...
  res.end=(body,...args)=>{
    ...
    if(req.method!=='HEAD'&&typeof body==='string'){
      if(path==='/search/'&&type.startsWith('application/json')){
        ...
        body=reconcileAmazonAffiliateMarkup(injectCss(decorateHtmlBody(body,path,q)));
...
Object.assign(handler,downstream,{
  ACTION3_SEARCH_COMMERCE_VERSION:VERSION,
  ACTION3_SEARCH_COMMERCE_ASSET_PATH:ASSET_PATH,
  retailerState,exactIntent,enrichPayload,decorateSearchHtml,decorateHtmlBody,reconcileAmazonAffiliateMarkup
});
