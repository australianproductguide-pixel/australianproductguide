...
function decorateHtmlBody(body,path){
  if(path!=='/search/')return body;
  const qMatch=body.match(/<input[^>]+name="q"[^>]+value="([^"]*)"/i);
  if(!qMatch)return body;
  const q=qMatch[1].replace(/&quot;/g,'"').replace(/&amp;/g,'&');
  try{
...
function sendCss(req,res){res.statusCode=200;res.setHeader('Content-Type','text/css; charset=utf-8');res.setHeader('Cache-Control','public, max-age=3600');return res.end(req.method==='HEAD'?'':css);}
...
function handler(req,res){
  ...
  res.end=(body,...args)=>{
    ...
    if(req.method!=='HEAD'&&typeof body==='string'&&path==='/search/'){
      if(type.startsWith('application/json')){
        try{body=JSON.stringify(enrichPayload(JSON.parse(body)));res.removeHeader('Content-Length');}catch{}
      }else if(type.startsWith('text/html')){
        body=injectCss(decorateHtmlBody(body,path));try{res.removeHeader('Content-Length')}catch{}
      }
    }
...
Object.assign(handler,{VERSION,ASSET_PATH,retailerState,exactIntent,enrichPayload,decorateSearchHtml});
