...
// buildResponse classifies again inside the core module. Preserve the same methodology
// intent through that second pass by normalising only this proven natural-language form
// to an existing methodology phrase. This leaves every other Scout request untouched.
const baseBuildResponse=upstream.core&&upstream.core.buildResponse;
if(typeof baseBuildResponse==='function'&&!upstream.core.__action1MethodologyResponseHardened){
  upstream.core.buildResponse=(input={})=>{
    if(METHODOLOGY_WORK_RE.test(String(input&&input.text||'')))return baseBuildResponse({...input,text:'How do you decide what to recommend?'});
    return baseBuildResponse(input);
  };
  Object.defineProperty(upstream.core,'__action1MethodologyResponseHardened',{value:true,enumerable:false,configurable:false,writable:false});
}
