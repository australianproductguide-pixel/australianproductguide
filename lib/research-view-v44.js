...
const {searchSite,exactBrand}=require('./search');
...
const displayProductName=p=>{const brand=String(p?.brand||'').trim(),name=String(p?.name||'').trim();if(!brand)return name;if(!name)return brand;return name.toLowerCase().startsWith(brand.toLowerCase())?name:`${brand} ${name}`;};
...
function sourceView(r){
  ...
  return {slug:p.slug,product:displayProductName(p),apgUrl:`/products/${p.slug}/`,source:p.source||null,sourceLabel:p.source?`${p.brand} official product information`:'APG maintained product record',sourceType:p.sourceType||null,lastSourceVerification:p.lastSourceVerification||null,lastRetailerCheck:p.lastRetailerCheck||null,freshnessStatus:p.freshnessStatus||null,exactRetailerOffers:offers.length,offers:offers.slice(0,3).map(o=>({retailer:o.retailer,url:o.url,price:Number(o.price)>0?Number(o.price):null,availability:o.availability||null,checkedAt:o.checkedAt||null}))};
}
function normalisePayloadLabels(payload){
  if(!payload)return payload;
  return {...payload,sources:(payload.sources||[]).map(source=>{const p=bySlug.get(source.slug);return p?{...source,product:displayProductName(p)}:source;})};
}
...
function brandDiscoveryPayload(raw,payload,brand){
  const maintained=products.filter(p=>p.brand===brand).sort((a,b)=>(a.evidenceTier==='deep'?0:1)-(b.evidenceTier==='deep'?0:1)||a.name.localeCompare(b.name));
  const categorySlugs=unique(maintained.map(p=>p.category)).filter(slug=>categories[slug]);
  const results=maintained.slice(0,8).map(p=>fallbackResult(p,'Maintained brand product'));
  const followUps=categorySlugs.slice(0,4).map(slug=>({label:`Explore ${brand} ${categories[slug].label}`,query:`${brand} ${categories[slug].label}`}));
  const count=maintained.length,categoryCount=categorySlugs.length;
  return {
    ...payload,
    version:VERSION,
    query:String(raw||'').trim(),
    mode:'brand-discovery',
    commercialRecommendationWeight:0,
    signals:[`Brand: ${brand}`,`${count} maintained product${count===1?'':'s'}`,`${categoryCount} categor${categoryCount===1?'y':'ies'}`],
    headline:`Maintained ${brand} products`,
    answer:`APG currently maintains ${count} ${brand} product${count===1?'':'s'} across ${categoryCount} categor${categoryCount===1?'y':'ies'}. A brand-only search is discovery, not a recommendation. Choose a product category, use case or exact model to compare like-for-like options.`,
    confidenceLabel:'Brand discovery',
    comparison:null,
    results,
    compareSlugs:[],
    sources:results.slice(0,3).map(sourceView).filter(Boolean),
    whatCouldChange:['Adding a product category, budget or use case turns brand discovery into a more decision-specific shortlist.'],
    followUps,
    note:'Brand-only searches stay within the selected maintained brand and do not create cross-category recommendation alternatives.'
  };
}
...
function researchPayload(raw=''){
  const payload=normalisePayloadLabels(upstream.researchPayload(raw)),search=raw?searchSite(raw):null;
  const brandCompare=brandOnlyComparison(raw);
  if(brandCompare)return brandComparisonPayload(raw,payload,brandCompare);
  const brand=exactBrand(raw);
  if(brand)return brandDiscoveryPayload(raw,payload,brand);
  ...
  return {
    ...
    headline:`Matched product: ${displayProductName(anchor)}`,
    answer:`APG matched “${String(raw).trim()}” to ${displayProductName(anchor)}. The alternatives below are kept within ${String(label).toLowerCase()} so the comparison remains like-for-like. Check the maintained product evidence and current retailer details before purchase.`,
...
function renderResult(r,i,mode){
  const why=(r.reasons||[]).slice(0,2).map(x=>`<li>${esc(x)}</li>`).join('');
  const verify=(r.verificationNeeds||[])[0]||(r.gaps||[])[0]||'';
  const label=mode==='brand-discovery'?(i===0?'Maintained product':'Another maintained product'):(i===0?'Best current fit':i===1?'Strong alternative':'Another option');
  return `<article class="apg-rv-card-v43${i===0?' is-lead':''}"><div class="apg-rv-card-top-v43"><span>${label}</span><b>${esc(r.confidence?.label||r.match||'Maintained result')}</b></div><p class="apg-rv-brand-v43">${esc(r.brand)}</p><h3><a href="${esc(r.url)}">${esc(r.name)}</a></h3>${r.priceBasis?`<strong class="apg-rv-price-v43">${esc(money(r.priceBasis))} <small>APG price context</small></strong>`:'<span class="apg-rv-price-v43 is-check">Check current retailer price</span>'}${why?`<ul>${why}</ul>`:''}${verify?`<p class="apg-rv-verify-v43"><strong>Verify:</strong> ${esc(verify)}</p>`:''}<a class="apg-rv-open-v43" href="${esc(r.url)}">Inspect product evidence →</a></article>`;
}
...
function renderResearch(p){
  const resultCards=(p.results||[]).slice(0,3).map((r,i)=>renderResult(r,i,p.mode)).join('');
...
function searchTransform(html,pathOrUrl){
  ...
  if(!q)return out;
  const payload=researchPayload(q),mustReplace=payload?.modelScoped||['comparison-needs-category','no-match','brand-discovery'].includes(payload?.mode);
...
module.exports.displayProductName=displayProductName;
