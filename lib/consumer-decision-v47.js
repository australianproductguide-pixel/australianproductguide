...
const hardVerification=require('./hard-constraint-verification-v1');
...
function applySoftDecisionSignals(r,intent){
  ...
  if(target>0&&!intent.budgetHard&&price>0){
    ...
    if(ratio<=1){
      ...
      r.reasons.unshift(`Known maintained price basis is at or below your ${money(target)} target budget`);
    }else if(ratio<=1.1){
      r.score+=4;
      r.reasons.unshift(`Known maintained price basis is close to your ${money(target)} target budget`);
    }else if(ratio<=1.25){
  ...
  if(preferred&&String(r.p.brand||'').toLowerCase()===preferred.toLowerCase()){
    r.score+=6;
    r.reasons.unshift(`Matches your stated ${preferred} brand preference`);
  }
...
function rankDecision(q='',opts={}){
  ...
  const ranked=(out.ranked||[]).map(r=>{
    const copy={...r,reasons:[...(r.reasons||[])],gaps:[...(r.gaps||[])],conflicts:[...(r.conflicts||[])],hardFailures:[...(r.hardFailures||[])],verificationNeeds:[...(r.verificationNeeds||[])]};
    return hardVerification.applyConstraintEvidence(applySoftDecisionSignals(copy,intent),intent);
  });
  ...
  const hardRecognised=hardVerification.recognisedConstraints(intent).length;
  const hardConstraintFallback=!out.unsupportedCategory&&counts.eligible===0&&hardRecognised>0;
  return {...out,ranked,counts,hardConstraintFallback,hardConstraintVerificationVersion:hardVerification.VERSION};
}
...
function criterionTrace(r,intent){
  ...
  for(const item of hardVerification.constraintVerification(r,intent)){
    const status=item.state===hardVerification.STATES.VERIFIED?'met':item.state===hardVerification.STATES.FAILED?'conflict':'unverified';
    add('hard',item.key,item.label,item.requirement,item.observed,status,item.reason);
  }

  if(intent.budget&&!intent.budgetHard){
  ...
  for(const c of intent.numericConstraints||[]){
    if(c.hard)continue;
    const label=human(c.label||c.key);
    let status='unverified';
    if(includes(r.hardFailures,c.label||c.key))status='conflict';
    else if(includes(r.reasons,c.label||c.key))status='aligned';
    else if(includes(r.gaps,c.label||c.key))status='gap';
    add('soft',`numeric:${c.key}`,label,`${c.mode} ${c.value}${c.unit==='in'?' in':c.unit?` ${c.unit}`:''}`,null,status,null);
  }
...
function publicDecision(q='',opts={}){
  ...
  const recognised=hardVerification.recognisedConstraints(x.intent);
  const results=display.map(r=>{
    const criteria=criterionTrace(r,x.intent),coverage=criterionCoverage(criteria),constraintVerification=hardVerification.constraintVerification(r,x.intent);
    return {slug:r.p.slug,brand:r.p.brand,name:r.p.name,category:r.p.categoryLabel,evidenceTier:r.p.evidenceTier||'starter',freshnessStatus:r.p.freshnessStatus||null,match:r.matchLabel,confidence:r.confidence,hardConstraintStatus:r.eligibility,pareto:r.pareto,reasons:r.reasons.slice(0,5),gaps:r.gaps.slice(0,4),conflicts:r.conflicts,hardFailures:r.hardFailures,verificationNeeds:r.verificationNeeds,criteria,criterionCoverage:coverage,constraintVerification,tradeoff:r.p.watch||null,priceBasis:r.p.price||null,url:`/products/${r.p.slug}/`};
  });
  const topConstraintSummary=hardVerification.summary(results[0]?.constraintVerification||[]);
  const payload={
    version:engine.ENGINE_VERSION,
    consumerIntelligenceVersion:VERSION,
    hardConstraintVerificationVersion:hardVerification.VERSION,
    ...
    basis:'Maintained APG data, explicit hard constraints, evidence-verifiable eligibility, user-weighted preferences, soft target-budget context and category decision signals',
    commercialRecommendationWeight:0,
    decisionState:x.intent.decisionState,
    constraintVerification:{version:hardVerification.VERSION,recognised,states:Object.values(hardVerification.STATES),topResultSummary:topConstraintSummary,policy:'Recognition is not proof. Each candidate hard constraint is separately VERIFIED, UNVERIFIED or FAILED. Missing positive evidence never proves an excluded capability is absent.'},
    ...
    audit:{candidateCount:x.ranked.length,eligibleCount:x.counts.eligible,unverifiedCount:x.counts.unverified,ineligibleCount:x.counts.ineligible,hardConstraintFallback:x.hardConstraintFallback,topCriterionCoverage:results[0]?.criterionCoverage||null,hardConstraintsRecognised:recognised.length,topHardConstraintsVerified:topConstraintSummary.verified,topHardConstraintsUnverified:topConstraintSummary.unverified,topHardConstraintsFailed:topConstraintSummary.failed},
    results,
    note:'Fit is not a review score. Hard constraints are not silently traded away. Recognition and verification are separate. Missing proof is marked unverified rather than guessed, including absence claims. Retailer participation, affiliate status and imagery contribute zero suitability points.'
  };
...
function install(){
  ...
  engine.HARD_CONSTRAINT_VERIFICATION_VERSION=hardVerification.VERSION;
  installed=true;
  return engine;
}

module.exports={VERSION,LEARNING_SCHEMA_VERSION,rankDecision,publicDecision,criterionTrace,criterionCoverage,learningSignals,constraintVerification:hardVerification.constraintVerification,recognisedConstraints:hardVerification.recognisedConstraints,hardVerification,install,original};
