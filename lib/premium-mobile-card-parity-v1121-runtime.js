...
function upgradeProductArticles(html,path,u){
  ...
  return String(html||'').replace(/<article\b[^>]*>[\s\S]*?<\/article>/gi,article=>{
    ...
    return product?v112.productCardV2(product,{query}):article;
  });
}
...
module.exports={VERSION,HEADER,articleClasses,productSlug,shouldUpgradeArticle,upgradeProductArticles,preserveExactRetailerVisualContract,transform,wrap};
