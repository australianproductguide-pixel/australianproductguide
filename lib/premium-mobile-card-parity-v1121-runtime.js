...
function productCardV1121(product,{query=''}={}){
  ...
  const labelled=`<h3><a href="/products/${product.slug}/" aria-label="${attr(`Open product guide: ${product.brand} ${product.name}`)}">`;
...
module.exports={VERSION,HEADER,STRICT_NO_MATCH,articleClasses,productSlug,isMaintainedProductArticle,isStrictNoMatch,shouldUpgradeArticle,productCardV1121,removeConflictingProductArticles,upgradeProductArticles,preserveExactRetailerVisualContract,transform,wrap};
