...
// Google Analytics Enhanced Measurement can infer a search term directly from a q=
// query string when the Google tag first loads. APG deliberately does not transmit raw
// shopping queries, so remove Search query/hash state from the address bar immediately
// before the consented Google tag is loaded. The already-rendered Search result remains
// intact; only the sensitive URL state is discarded after the visitor opts into analytics.
const baseAnalyticsLoader=typeof window.apgLoadGoogleAnalytics==='function'?window.apgLoadGoogleAnalytics:null;
if(baseAnalyticsLoader){
  window.apgLoadGoogleAnalytics=function(){
    if(location.pathname==='/search/'&&(location.search||location.hash)){
      try{history.replaceState(history.state,'',location.pathname)}catch{}
    }
    return baseAnalyticsLoader.apply(this,arguments);
  };
}
