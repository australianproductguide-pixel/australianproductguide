...
async function imageStates(slugs,options={}){
  const values=[...new Set((Array.isArray(slugs)?slugs:[]).map(clean).filter(value=>/^[a-z0-9][a-z0-9-]{1,160}$/.test(value)))].slice(0,100);
  if(!values.length)return [];
  // One RLS-protected read is materially more reliable on category/search/compare surfaces than
  // launching N parallel network reads from a short-lived serverless request.
  const inList='('+values.map(value=>`"${value.replace(/"/g,'')}"`).join(',')+')';
  const rows=await request(`/rest/v1/apg_ebay_image_state?slug=in.${encodeURIComponent(inList)}&select=${encodeURIComponent(IMAGE_STATE_SELECT)}`,{timeoutMs:1800,...options});
  return Array.isArray(rows)?rows:[];
}

module.exports={VERSION,DEFAULT_URL,DEFAULT_PUBLISHABLE_KEY,DEFAULT_TIMEOUT_MS,STATE_FUNCTION,IMAGE_STATE_SELECT,configuration,request,rpc,imageState,imageStates};
