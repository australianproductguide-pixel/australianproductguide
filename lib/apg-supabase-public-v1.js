...
// APG public, read-mostly Supabase client for server-rendered operational state.
// The publishable key is deliberately non-secret and is already used by APG browser surfaces.
// All material writes remain protected by RLS plus one-time background-worker capabilities.
...
async function request(path,{method='GET',body=null,timeoutMs=DEFAULT_TIMEOUT_MS,fetchImpl=global.fetch,headers={}}={}){
  ...
  try{
    ...
    if(!response.ok){
      ...
      error.code='APG_SUPABASE_HTTP_ERROR';error.status=response.status;throw error;
    }
    return payload;
  }catch(error){
...
function rpc(name,args,options={}){
  ...
  return request(`/rest/v1/rpc/${safe}`,{...options,method:'POST',body:args||{}});
}
async function imageState(slug,options={}){
  const value=clean(slug);
  if(!/^[a-z0-9][a-z0-9-]{1,160}$/.test(value))return null;
  const select='slug,product_name,status,detail_verified,exact_model,verification_level,verification_evidence,item_id,legacy_item_id,title,condition,price_value,price_currency,image_url,image_source,item_web_url,item_affiliate_web_url,match_score,match_reasons,match_flags,recommendation_weight,last_verified_at,next_refresh_at,consecutive_failures,recovery_required,last_error_code';
  const rows=await request(`/rest/v1/apg_ebay_image_state?slug=eq.${encodeURIComponent(value)}&select=${encodeURIComponent(select)}&limit=1`,options);
  return Array.isArray(rows)&&rows.length?rows[0]:null;
}

module.exports={VERSION,DEFAULT_URL,DEFAULT_PUBLISHABLE_KEY,DEFAULT_TIMEOUT_MS,configuration,request,rpc,imageState};
