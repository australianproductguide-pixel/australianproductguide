...
// APG category featured product imagery v1.1
...
// to that category. Existing editorial/category imagery remains visible until a governed image
// has actually loaded; a blocked, slow or failed image therefore cannot create a blank card.
// Department cards resolve their listed categories in DOM order so the representative product
// is deterministic rather than whichever async request happens to finish first.
...
;(()=>{
'use strict';if(window.__APG_CATEGORY_FEATURED_IMAGERY_V1__)return;window.__APG_CATEGORY_FEATURED_IMAGERY_V1__='1.1';
const path=location.pathname;if(path==='/'||/^\/products\//.test(path))return;
const cache=new Map(),pending=new Map(),busy=new WeakSet();
...
function departmentSlot(scope){
  ...
  let slot=title.querySelector(':scope > .apg-category-product-slot-v11');if(slot)return slot;
  const original=title.querySelector(':scope > img');if(!original)return null;
  slot=document.createElement('span');slot.className='apg-category-product-slot-v11';original.insertAdjacentElement('beforebegin',slot);slot.appendChild(original);return slot;
}
...
function apply(scope,row){
  return new Promise(done=>{
    if(!scope||!row||!row.imageUrl||scope.dataset.apgCategoryFeaturedImage==='v1.1')return done(false);
    const visual=visualFor(scope);if(!visual)return done(false);
    const original=originalImage(visual),stage=document.createElement('span');stage.className='apg-category-product-stage-v1';stage.dataset.apgCategoryProduct=row.productSlug||'';stage.dataset.apgImageReady='false';
    const img=document.createElement('img');img.alt='';img.loading='lazy';img.decoding='async';img.width=420;img.height=260;
    let settled=false;
    const finish=(ok)=>{if(settled)return;settled=true;if(ok){if(original){original.style.opacity='0';original.style.pointerEvents='none'}stage.dataset.apgImageReady='true';visual.classList.remove('apg-category-product-visual-pending-v11');visual.classList.add('apg-category-product-visual-v1');scope.dataset.apgCategoryFeaturedImage='v1.1';scope.removeAttribute('data-apg-category-featured-image-fallback');done(true);return}stage.remove();visual.classList.remove('apg-category-product-visual-pending-v11');done(false)};
    img.addEventListener('load',()=>finish(true),{once:true});img.addEventListener('error',()=>finish(false),{once:true});stage.appendChild(img);
    visual.classList.add('apg-category-product-visual-pending-v11');if(original)original.insertAdjacentElement('afterend',stage);else visual.insertBefore(stage,visual.firstChild);img.src=row.imageUrl;
  });
}
async function enhanceScope(scope,links){
  if(!scope||busy.has(scope)||scope.dataset.apgCategoryFeaturedImage==='v1.1')return;busy.add(scope);const seen=new Set();
  try{for(const link of links){const slug=categorySlug(link);if(!slug||seen.has(slug))continue;seen.add(slug);const row=await resolve(slug);if(!row)continue;if(await apply(scope,row))return}scope.dataset.apgCategoryFeaturedImageFallback='editorial'}finally{busy.delete(scope)}
}
function boot(){
  const links=[...document.querySelectorAll('main a[href^="/categories/"]')],groups=new Map();
  for(const link of links){const scope=scopeFor(link);if(!scope||!hasVisual(scope))continue;if(!groups.has(scope))groups.set(scope,[]);groups.get(scope).push(link)}
...
/* APG category featured product imagery v1.1 */
.apg-category-product-visual-pending-v11,.apg-category-product-visual-v1{position:relative!important;overflow:hidden!important;isolation:isolate!important}
.apg-category-product-visual-v1{background:radial-gradient(circle at 64% 42%,#fff 0 30%,#f5f8fc 60%,#eef3f8 100%)!important}
.apg-category-product-stage-v1{position:absolute!important;inset:3% 4% 7%!important;z-index:1!important;display:flex!important;align-items:center!important;justify-content:center!important;pointer-events:none!important;opacity:0!important;filter:drop-shadow(0 10px 13px rgba(15,23,42,.10))!important}
...
.apg-category-product-slot-v11{position:relative!important;display:block!important;width:44px!important;height:44px!important;min-width:44px!important;min-height:44px!important;overflow:hidden!important;border-radius:8px!important}
.apg-category-product-slot-v11>.apg-category-product-stage-v1{inset:2px!important;border-radius:7px!important}
.apg-category-product-slot-v11>.apg-category-product-stage-v1 img{max-width:100%!important;max-height:100%!important;transform:scale(1.05)!important}
@media(max-width:720px){.apg-category-product-stage-v1{inset:2% 3% 8%!important}.apg-category-product-stage-v1 img{max-width:96%!important;max-height:96%!important;transform:scale(1.18)!important}.apg-category-product-slot-v11>.apg-category-product-stage-v1{inset:2px!important}.apg-category-product-slot-v11>.apg-category-product-stage-v1 img{transform:scale(1.05)!important}}
`;
...
function wrap(downstream){
  ...
  function handler(req,res){
    ...
    if(eligibleCspPath(path)&&res&&typeof res.setHeader==='function'){
      const originalSetHeader=res.setHeader.bind(res);res.setHeader=function(name,value){if(String(name||'').toLowerCase()==='content-security-policy'&&!Array.isArray(value))value=withEbayImageCsp(value);return originalSetHeader(name,value)};originalSetHeader('X-APG-Category-Featured-Imagery','v'+VERSION);
    }
    if(path===APP_PATH)return augmentAsset(downstream,req,res,'js');if(path===CSS_PATH)return augmentAsset(downstream,req,res,'css');return downstream(req,res);
  }
...
module.exports={VERSION,APP_PATH,CSS_PATH,LOOKUP_PATH,EBAY_IMAGE_ORIGIN,CATEGORY_MAP,safeCategory,eligibleCspPath,withEbayImageCsp,featuredPayload,CLIENT_JS,CLIENT_CSS,wrap};
