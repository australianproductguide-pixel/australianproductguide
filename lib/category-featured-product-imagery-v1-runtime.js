...
// APG category featured product imagery v1.4
...
//  - each department also uses one existing <img> only, selecting the strongest governed product
//    candidate across its linked categories rather than stacking an additional product layer.
...
;(()=>{
'use strict';
if(window.__APG_CATEGORY_FEATURED_IMAGERY_V1__==='1.4')return;
window.__APG_CATEGORY_FEATURED_IMAGERY_V1__='1.4';
...
function recoverLegacy(scope){
  ...
  if(scope.dataset.apgCategoryFeaturedImage&&scope.dataset.apgCategoryFeaturedImage!=='v1.4')scope.removeAttribute('data-apg-category-featured-image');
}
...
async function swapSingle(scope,row){
  ...
  return new Promise(done=>{let settled=false,timer=0;const finish=ok=>{if(settled)return;settled=true;clearTimeout(timer);img.removeEventListener('load',onload);img.removeEventListener('error',onerror);if(!ok){restoreEditorial(scope,img);done(false);return}img.dataset.apgFeaturedProduct=row.productSlug||'';img.dataset.apgImageSource='product';scope.dataset.apgCategoryFeaturedImage='v1.4';scope.dataset.apgCategoryImageMode='single-product';scope.removeAttribute('data-apg-category-featured-image-fallback');done(true)};const onload=()=>finish(img.naturalWidth>=80&&img.naturalHeight>=80),onerror=()=>finish(false);img.addEventListener('load',onload);img.addEventListener('error',onerror);img.removeAttribute('srcset');img.removeAttribute('sizes');img.setAttribute('src',row.imageUrl);timer=setTimeout(()=>finish(false),6000);if(img.complete)setTimeout(onload,0)})
}
async function choose(scope,links){const slugs=[...new Set((links||[]).map(categorySlug).filter(Boolean))];if(!slugs.length)return null;const rows=(await Promise.all(slugs.map(resolve))).filter(Boolean);if(scope.matches('.apg-category-department-card'))return strongest(rows);return strongest(rows)}
async function enhanceScope(scope,links){if(!scope||busy.has(scope)||scope.dataset.apgCategoryFeaturedImage==='v1.4')return;busy.add(scope);try{const row=await choose(scope,links);if(!row){const img=imageFor(scope);if(img){captureEditorial(img);restoreEditorial(scope,img)}return}await swapSingle(scope,row)}finally{busy.delete(scope)}}
function boot(){
...
// v1.4 deliberately adds no imagery CSS. The existing SSR image element owns layout and dimensions.
