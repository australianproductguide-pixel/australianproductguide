...
// APG category featured product imagery v1.2
// Progressive, non-blocking collection imagery for the maintained category catalogue.
// A category prefers the strongest currently governed exact-model product image belonging to
// that category. Existing editorial/category imagery remains visible until the replacement has
// actually loaded, so a missing, blocked, slow or failed product image cannot create a blank tile.
// The 90-category directory resolves its complete image map through one cacheable APG endpoint;
// the server batches registry reads in groups of at most 100 products instead of issuing one
// browser/API request per category. Affiliate relationships and retailer economics never enter
// image selection. No category/product HTML response waits on Supabase or eBay.
...
;(()=>{
'use strict';if(window.__APG_CATEGORY_FEATURED_IMAGERY_V1__)return;window.__APG_CATEGORY_FEATURED_IMAGERY_V1__='1.2';
...
function apply(scope,row){
  return new Promise(done=>{
    if(!scope||!row||!row.imageUrl||scope.dataset.apgCategoryFeaturedImage==='v1.2')return done(false);
    ...
    let settled=false;
    const finish=(ok)=>{if(settled)return;settled=true;if(ok){if(original){original.style.opacity='0';original.style.pointerEvents='none'}stage.dataset.apgImageReady='true';visual.classList.remove('apg-category-product-visual-pending-v12');visual.classList.add('apg-category-product-visual-v1');scope.dataset.apgCategoryFeaturedImage='v1.2';scope.removeAttribute('data-apg-category-featured-image-fallback');done(true);return}stage.remove();visual.classList.remove('apg-category-product-visual-pending-v12');done(false)};
    img.addEventListener('load',()=>finish(true),{once:true});img.addEventListener('error',()=>finish(false),{once:true});stage.appendChild(img);
    visual.classList.add('apg-category-product-visual-pending-v12');if(original)original.insertAdjacentElement('afterend',stage);else visual.insertBefore(stage,visual.firstChild);img.src=row.imageUrl;
  });
}
async function enhanceScope(scope,links){
  if(!scope||busy.has(scope)||scope.dataset.apgCategoryFeaturedImage==='v1.2')return;busy.add(scope);const seen=new Set();
