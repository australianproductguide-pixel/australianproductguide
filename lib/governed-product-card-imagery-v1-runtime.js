...
// APG governed product-card imagery v1.3.
...
function productVisualBounds(html,anchorIndex){
  ...
  if(!last)return null;let depth=1,cursor=last.end;const token=/<div\b[^>]*>|<\/div>/gi;token.lastIndex=cursor;let part;
...
async function governedMappings(slugs,options={}){
  ...
  try{
    ...
    const states=await Promise.all(slugs.map(async slug=>{try{return [slug,await continuity.currentMapping(slug,options)];}catch{return [slug,null];}}));return new Map(states);
  }
}
