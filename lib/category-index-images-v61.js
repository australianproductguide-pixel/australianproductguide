...
// v61.9 adds a separate APG brand-alignment presentation layer without changing
// the governed category imagery, catalogue taxonomy or directory behaviour.
...
const CATEGORY_BRAND_ALIGNMENT_VERSION='61.9';
const ORIGIN='https://australianproductguide.au';
const STYLESHEET='/assets/category-directory-v61.css?v=61.8';
const BRAND_ALIGNMENT_STYLESHEET='/assets/category-directory-brand-v619.css?v=61.9';
const FALLBACK_THUMBNAIL_PX=40;

const QUICK_BROWSE_SLUGS=[
...
function retailDirectoryTools(){
  const quick=QUICK_BROWSE_SLUGS.map(slug=>`<a href="/categories/${esc(slug)}/">${esc(categories[slug]?.label||slug)}</a>`).join('');
  return `<nav class="apg-category-retail-tools" aria-label="Category browsing shortcuts"><div class="apg-category-retail-tools-inner"><div class="apg-category-quick"><span>Quick browse</span>${quick}</div><div class="apg-category-task-links"><a href="/decision-lab/">Decision Lab</a><a href="/compare/">Compare products</a><a href="/brands/">Browse brands</a></div></div></nav>`;
}
...
function departmentDirectory(){
  const cards=CATEGORY_DEPARTMENTS.map(department=>{
    ...
    return `<article class="apg-category-department-card" data-apg-department="${esc(department.key)}"><div class="apg-category-department-title"><img src="${esc(image?.src||'')}" alt="" width="44" height="44" loading="lazy" decoding="async"><div><span>Department</span><h3>${esc(department.label)}</h3></div></div><div class="apg-category-department-links">${links}</div><a class="apg-category-department-all" href="#apgAllCategories">View all categories A–Z</a></article>`;
  }).join('');
...
function sortCategoryGridAlphabetically(html){
  ...
  return String(html||'').replace(/(<div class="category-grid premium-category-grid">)([\s\S]*?)(<\/div>)(?=\s*<\/section>)/i,(whole,open,inner,close)=>{
    const cards=inner.match(/<article\b[^>]*class="[^"]*\bcategory-card\b[^"]*"[^>]*>[\s\S]*?<\/article>/gi)||[];
    if(cards.length!==total)return whole;
    cards.sort((a,b)=>{
...
function polishDirectoryShell(html){
  ...
  out=out.replace('<p class="kicker">Current coverage</p><h2>90 populated comparison categories</h2>','<p class="kicker">Complete directory</p><h2 id="apgAllCategories">All categories A–Z</h2>');
...
function inject(html,path){
  ...
  out=out.replace('</head>',`<link rel="stylesheet" href="${STYLESHEET}"><link rel="stylesheet" href="${BRAND_ALIGNMENT_STYLESHEET}"><meta name="apg-category-index-images" content="v${CATEGORY_INDEX_IMAGES_VERSION}"><meta name="apg-category-brand-alignment" content="v${CATEGORY_BRAND_ALIGNMENT_VERSION}"></head>`);
...
function handler(req,res){
  ...
  res.setHeader('X-APG-Category-Brand-Alignment','v'+CATEGORY_BRAND_ALIGNMENT_VERSION);
...
Object.assign(handler,downstream,{
  ...
  CATEGORY_BRAND_ALIGNMENT_VERSION,
  CATEGORY_INDEX_STYLESHEET:STYLESHEET,
  CATEGORY_BRAND_ALIGNMENT_STYLESHEET:BRAND_ALIGNMENT_STYLESHEET,
