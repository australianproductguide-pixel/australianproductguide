...
const CATEGORY_INDEX_IMAGES_VERSION='61.0';
...
function enrichCategoryCard(card){
  ...
  // Base APG category cards begin with an icon-only visual wrapper. Replace only
  // that wrapper; the category name, description, decision signals and actions stay unchanged.
  const replaced=card.replace(/(<article\b[^>]*class="[^"]*\bcategory-card\b[^"]*"[^>]*>)\s*<div>\s*<span\b[^>]*class="[^"]*\bcategory-icon\b[^"]*"[\s\S]*?<\/span>\s*<\/div>/i,`$1${figure}`);
  // Defensive fallback for later card-template changes: prepend the image rather
  // than silently dropping it.
  if(replaced===card)return card.replace(/(<article\b[^>]*class="[^"]*\bcategory-card\b[^"]*"[^>]*>)/i,`$1${figure}`);
  return replaced;
}
