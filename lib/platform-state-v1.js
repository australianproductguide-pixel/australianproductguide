...
// census, category-completion controls and Action 5 retailer-certification objects that
// drive Production. It does not attempt to self-certify GitHub Actions or Vercel after
// deployment; those remain post-deploy controls and must be reconciled into the Operating
// Backend only after the exact Production SHA is verified.
const {products,categories}=require('../data');
const decisionState=require('./decision-state-v2');
const hardVerification=require('./hard-constraint-verification-v1');
const categoryCompletion=require('./category-completion-gate-v1');
...
function snapshot({downstream,retailerSnapshot,env=process.env}={}){
  ...
  const categoryMaturity=categoryCompletion.snapshot();
  ...
  return {
    ...
    decision:{
      canonicalStateSchemaVersion:decisionState.VERSION,
      hardConstraintVerificationVersion:hardVerification.VERSION,
      inputPolicy:'Structured state is authoritative when supplied; legacy query interpretation remains supported during migration.',
      retailerAndAffiliateRecommendationWeight:0
    },
    categoryMaturity:{
      gateVersion:categoryCompletion.VERSION,
      priorityCategoryCount:categoryMaturity.summary.priorityCategoryCount,
      decisionGradeCount:categoryMaturity.summary.decisionGradeCount,
      notDecisionGradeCount:categoryMaturity.summary.notDecisionGradeCount,
      priorityCategories:categoryMaturity.priorityCategories,
      publicDecisionGradeClaimAllowed:categoryMaturity.summary.decisionGradeCount===categoryMaturity.summary.priorityCategoryCount,
      policy:'Every required category gate must be PASS; PARTIAL never rolls up to DECISION_GRADE.'
    },
    retailer:retailer?{
    ...
    controls:{
      ...
      categoryCompletionGate:categoryCompletion.VERSION,
