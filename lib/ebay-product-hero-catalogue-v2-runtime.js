...
// APG catalogue-wide eBay product hero v2.0.
// Replaces only the top product-page brand placeholder, and only after a current eBay AU
// exact-product match survives enrichment, family-variant controls and the stricter hero guard.
// Anything uncertain fails closed to the existing APG logo placeholder. Retailer participation
// and affiliate status contribute zero recommendation points. eBay imagery is not written into
// canonical Product.image structured data or APG-owned image provenance.

const {products}=require('../data');
const {enrichProduct}=require('./ebay-catalogue-enrichment-v1');
...
function projectedDetail(product,row,current,resolvedAt){
...
async function resolveExactProduct(slug,{now=Date.now,enrich=enrichProduct,getItem=ebay.getItem}={}){
  ...
  try{
    const base=await enrich(product);
    const guarded=familyGuard.applyToEnrichment(product,base);
    const firstCheck=exactGuard.evaluate(product,guarded,products,{now:t});
    if(!firstCheck.eligible){cache.set(slug,{at:t,detail:null,reason:firstCheck.reason});return null;}

    const raw=await getItem(guarded.accepted.itemId,{timeoutMs:4500,referenceId:`apg:${slug}:hero-v2`});
    const current=ebay.safeItemProjection(raw);
    if(!exactIdentity(current,guarded.accepted))throw new Error('eBay hero item identity changed');
    const refreshed=refreshedRow(guarded,current);
    ...
    // eBay public display data must remain fresh. A previously verified success may bridge a
    // temporary API failure for less than five hours; after that the page fails closed to APG.
...
function reconcileImageCopy(html){
  return String(html||'')
    .replace(/Brand identity placeholder via APG governed brand resolver/g,'Current eBay Australia listing image · exact model verified at render time')
...
async function inject(html,pathname,options={}){
  ...
  if(!detail)return {html:original,usedEbayImage:false,slug,reason:'no-verified-exact-ebay-product'};
  ...
  return {html:ensureStyle(reconcileImageCopy(replaced)),usedEbayImage:true,slug,itemId:detail.legacyItemId,imageUrl:detail.imageUrl,resolvedAt:detail.resolvedAt};
}
...
module.exports={
  VERSION,STYLE_HREF,CACHE_TTL_MS,NEGATIVE_CACHE_TTL_MS,MAX_CACHE_AGE_MS,EBAY_IMAGE_ORIGIN,PRODUCT_MAP,PILOT_SLUGS,cache,
  displayName,canonicalPath,slugForPath,productForSlug,topHeroRange,topHeroHasPlaceholder,exactIdentity,refreshedRow,projectedDetail,
