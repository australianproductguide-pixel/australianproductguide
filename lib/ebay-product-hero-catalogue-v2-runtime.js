...
//
// Public product pages are deliberately registry-only. They must never perform Browse API
// search/detail calls. Exact eBay AU rows are generated by APG's controlled enrichment process,
// stored in the governed registry and independently re-checked here before display. Anything
// absent, stale, malformed, used, accessory-like or variant-ambiguous fails closed to the APG
// brand placeholder.
//
// Retailer participation and affiliate status contribute zero recommendation points. eBay
// imagery remains retailer-scoped and is never written into canonical Product.image structured
// data or APG-owned image provenance.

const {products}=require('../data');
const exactGuard=require('./ebay-product-hero-exact-guard-v2');
const pilotRegistry=require('../data/ebay-verified-offers-v1');
const catalogueRegistry=require('../data/ebay-product-hero-registry-v21');

const VERSION='2.1';
const STYLE_HREF='/assets/ebay-verified-product-hero-v1.css';
const CACHE_TTL_MS=45*60*1000;
const NEGATIVE_CACHE_TTL_MS=15*60*1000;
// eBay's API licence permits displayed item-listing information to lag the eBay Site by no
// more than six hours. APG retains the stricter pre-existing five-hour display window.
const MAX_CACHE_AGE_MS=5*60*60*1000;
const MAX_REGISTRY_AGE_MS=MAX_CACHE_AGE_MS;
...
// Retained as a pure helper for compatibility and QA. Public runtime does not use it to merge
// live API responses.
...
function registryObservedAt(row){
  const value=Date.parse(clean(row&&row.observedAt));
  return Number.isFinite(value)?value:null;
}
function registryAge(row,now=Date.now()){
  const observed=registryObservedAt(row);const t=Number(now);
  if(observed==null||!Number.isFinite(t)||t<observed)return Infinity;
  return t-observed;
}
function registryFresh(row,now=Date.now()){
  return registryAge(row,now)<=MAX_REGISTRY_AGE_MS;
}
function projectedDetail(product,row,resolvedAt){
  const current=row&&row.accepted||{};
  return Object.freeze({
    ...
    observedAt:clean(row&&row.observedAt)||null,
...
function registryRow(registry,slug){
  if(!registry)return null;
  if(typeof registry.forSlug==='function')return registry.forSlug(slug);
  const offers=registry.offers&&typeof registry.offers==='object'?registry.offers:registry;
  return offers&&offers[clean(slug)]||null;
}
async function resolveExactProduct(slug,{now=Date.now,registry=catalogueRegistry}={}){
  const product=productForSlug(slug);
  if(!product||PILOT_SLUGS.has(slug))return null;
  const t=Number(now());
  const row=registryRow(registry,slug);
  if(!row){cache.set(slug,{at:t,detail:null,reason:'registry-miss'});return null;}
  if(!registryFresh(row,t)){cache.set(slug,{at:t,detail:null,reason:'registry-stale'});return null;}

  const checked=exactGuard.evaluate(product,row,products,{now:t});
  if(!checked.eligible){cache.set(slug,{at:t,detail:null,reason:checked.reason});return null;}
  const observed=registryObservedAt(row);
  const detail=projectedDetail(product,row,observed);
  cache.set(slug,{at:t,detail,reason:'exact-fresh-ebay-au-registry'});
  return detail;
}
...
module.exports={
  VERSION,STYLE_HREF,CACHE_TTL_MS,NEGATIVE_CACHE_TTL_MS,MAX_CACHE_AGE_MS,MAX_REGISTRY_AGE_MS,EBAY_IMAGE_ORIGIN,PRODUCT_MAP,PILOT_SLUGS,cache,
  displayName,canonicalPath,slugForPath,productForSlug,topHeroRange,topHeroHasPlaceholder,exactIdentity,refreshedRow,
  registryObservedAt,registryAge,registryFresh,projectedDetail,registryRow,resolveExactProduct,ensureStyle,heroMarkup,replaceHeroPlaceholder,
