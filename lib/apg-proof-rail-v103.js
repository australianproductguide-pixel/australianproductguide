...
const VERSION='103.3';
const CSS='/assets/apg-proof-rail-v103.css?v=103.3';
const JS='/assets/apg-proof-rail-v103.js?v=103.3';
...
function autoplayToggle(){
  return `<button class="apg-proof-autoplay-toggle-v103" type="button" data-proof-autoplay-toggle aria-label="Pause automatic proof rotation" aria-pressed="false"><svg class="is-pause" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="M8 6v12M16 6v12"/></svg><svg class="is-play" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="m9 6 9 6-9 6Z"/></svg></button>`;
}

function ApgProofRail(stats=proofStats()){
  const cards=proofCards(stats);
  return `<section class="apg-proof-rail-v103" aria-label="Australian Product Guide proof rail" data-apg-proof-rail data-apg-proof-rail-version="${VERSION}" data-apg-proof-autoplay="5000" data-apg-proof-products="${esc(stats.products)}" data-apg-proof-categories="${esc(stats.categories)}" data-apg-proof-brands="${esc(stats.brands)}"><div class="wrap apg-proof-shell-v103"><div class="apg-proof-stage-v103">${arrow('prev')}<div class="apg-proof-track-v103" id="apg-proof-track-v103" data-proof-track tabindex="0" role="group" aria-label="Australian Product Guide proof points">${cards.map((card,index)=>proofCard(card,index,cards.length)).join('')}</div>${arrow('next')}<div class="apg-proof-progress-v103" data-proof-progress aria-live="off" aria-atomic="true"><span class="sr-only">Proof <strong data-proof-current>1</strong> of <span data-proof-total>${cards.length}</span></span>${proofDots(cards.length)}${autoplayToggle()}</div></div></div></section>`;
}
