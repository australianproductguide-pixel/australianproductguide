...
const VERSION='103.5';
const CSS='/assets/apg-proof-rail-v103.css?v=103.5';
const JS='/assets/apg-proof-rail-v103.js?v=103.5';
...
function proofCard(card,index,total){
  ...
  return `<article class="apg-proof-card-v103" data-proof-card aria-label="Proof ${number} of ${total}: ${esc(card.kicker)}"><span class="apg-proof-kicker-v103">${esc(card.kicker)}</span>${proofValue(card)}<h3>${esc(card.headline)}</h3><p>${esc(card.support)}</p></article>`;
}
