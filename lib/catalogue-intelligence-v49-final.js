...
const enrichmentPass2=pass2.apply({categoryMaps:[categories]});
const enrichmentPass3=pass3.apply({categoryMaps:[categories]});
const enrichmentPass4=pass4.apply({categoryMaps:[categories]});
const enrichmentPass5=pass5.apply({categoryMaps:[categories]});
const enrichmentPass6=pass6.apply({categoryMaps:[categories]});
...
function depthFor(input){
  ...
  return{
    ...
    evidenceDepthVersion:p.evidenceDepthVersion||null,evidenceDepthStatus:p.evidenceDepthStatus||null,independentlyResearchedInV49:/^new-primary-research-v49/.test(String(p.evidenceDepthStatus||'')),
...
function snapshot(){
  const depths=products.map(depthFor),strong=depths.filter(x=>x.meetsStrongDepthBenchmark).length,primary=depths.filter(x=>x.exactPrimaryEvidenceEstablished).length,researched=depths.filter(x=>x.independentlyResearchedInV49).length,structuredPass1=products.filter(p=>p.evidenceDepthStructuredAt===evidence.VERIFIED).length,v48=v48Catalogue.summary();
  const entityCorrections=[...enrichmentPass2.unresolvedEntityCorrections,...enrichmentPass3.unresolvedEntityCorrections,...enrichmentPass4.unresolvedEntityCorrections];
  return{
    ...
    evidenceDepth:{strongDepthVerified:strong,belowStrongDepth:products.length-strong,strongDepthCoveragePct:Math.round(strong/products.length*1000)/10,exactPrimaryEvidenceEstablished:primary,independentlyResearchedInV49:researched,primarySourceRecordsStructuredInPass1:structuredPass1,maintainedPrimaryRecordsStructuredInPass5:enrichmentPass5.structuredPrimaryRecords},
    enrichment:{
      pass1:{version:enrichment.version,verifiedAt:enrichment.verifiedAt,newPrimaryResearch:enrichment.newPrimaryResearch,existingPrimaryStructured:enrichment.existingPrimaryStructured},
      pass2:{version:enrichmentPass2.version,verifiedAt:enrichmentPass2.verifiedAt,newPrimaryResearch:enrichmentPass2.newPrimaryResearch,unresolvedEntityCorrections:enrichmentPass2.unresolvedEntityCorrections},
      pass3:{version:enrichmentPass3.version,verifiedAt:enrichmentPass3.verifiedAt,newPrimaryResearch:enrichmentPass3.newPrimaryResearch,unresolvedEntityCorrections:enrichmentPass3.unresolvedEntityCorrections},
      pass4:{version:enrichmentPass4.version,verifiedAt:enrichmentPass4.verifiedAt,newPrimaryResearch:enrichmentPass4.newPrimaryResearch,unresolvedEntityCorrections:enrichmentPass4.unresolvedEntityCorrections},
      pass5:{version:enrichmentPass5.version,structuredAt:enrichmentPass5.structuredAt,targetCount:enrichmentPass5.targetCount,structuredPrimaryRecords:enrichmentPass5.structuredPrimaryRecords,failures:enrichmentPass5.failures,newPrimaryResearch:0,provenancePolicy:enrichmentPass5.provenancePolicy},
      pass6:{version:enrichmentPass6.version,verifiedAt:enrichmentPass6.verifiedAt,newPrimaryResearch:enrichmentPass6.newPrimaryResearch,missing:enrichmentPass6.missing}
    },
    v48EvidenceCompatibility:v48.evidence,
    researchBacklog:{needsStrongDepthVerification:products.length-strong,needsEntityCorrection:entityCorrections.length,entityCorrections,policy:'This is the remaining source-by-source evidence programme. Products are not upgraded to strong-depth merely because they are maintained, commercially available or classification-relevant. Identity conflicts, superseded generations, archived products and regional-model mismatches are corrected rather than papered over.'},
...
Object.assign(handler,{VERSION,API_PATH,PRODUCT_API_PREFIX,BENCHMARK,enrichment,enrichmentPass2,enrichmentPass3,enrichmentPass4,enrichmentPass5,enrichmentPass6,depthFor,categoryDepth,snapshot,downstream});
