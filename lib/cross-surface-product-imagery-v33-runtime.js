...
function collectProductSlugs(html,limit=MAX_PAGE_SLUGS){
  ...
  const patterns=[
    /href=["']\/products\/([a-z0-9][a-z0-9-]{1,160})\/["'?#]/gi,
    /data-product-slug=["']([a-z0-9][a-z0-9-]{1,160})["']/gi,
    /data-compare-product=["']([a-z0-9][a-z0-9-]{1,160})["']/gi
  ];
  for(const re of patterns){
    let m;
    while((m=re.exec(source))){
      const slug=m[1];
      if(validSlug(slug)&&!seen.has(slug)){seen.add(slug);out.push(slug);if(out.length>=limit)return out;}
    }
  }
  return out;
}
...
async function currentMappings(slugs,{now=Date.now,fetchStates=supabase.imageStates}={}){
  const values=uniqueSlugs(slugs),result=new Map();if(!values.length)return result;
  const t=Number(now()),pending=[];
  ...
  // A transient state-service failure follows the same bounded continuity rule as product pages.
  for(const slug of pending){
    const cached=continuity.stateCache.get(slug);
...
function replaceBrandPlaceholders(html,mappings){
  ...
  while((m=re.exec(source))){
    ...
    out+=slug?photoMarkup(slug,mappings.get(slug)):m[0];
    cursor=re.lastIndex;
  }
...
function enrichProductArticles(html,mappings){
  ...
  return String(html||'').replace(/<article\b[^>]*>[\s\S]*?<\/article>/gi,article=>{
    ...
    return article.replace(/^(<article\b[^>]*>)/i,`$1${photoMarkup(slug,mappings.get(slug),{compact:true,linked:true})}`);
  });
}
...
function exactQuerySlug(query){
  ...
  try{
    const r=search.searchSite(q),u=r.queryUnderstanding||{},rows=Array.isArray(r.products)?r.products:[];
    return rows.length===1&&Number(u.modelMatchCount)===1&&!u.modelAmbiguous&&validSlug(rows[0].slug)?rows[0].slug:null;
  }catch{return null;}
}
...
function decorateSearchPayload(payload,mappings){
  ...
  if(payload.directCompare){
    for(const key of ['a','b']){const p=payload.directCompare[key];if(p&&mappings.has(p.slug))payload.directCompare[key]={...p,presentationImage:publicImage(p.slug,mappings.get(p.slug))};}
  }
...
function applySlug(slug,image,root=document){if(!slug||!image)return;const safe=CSS.escape(slug),selector='a[href="/products/'+safe+'/"],a[href^="/products/'+safe+'/?"],[data-product-slug="'+safe+'"],[data-compare-product="'+safe+'"]';root.querySelectorAll?.(selector).forEach(el=>{const a=el instanceof HTMLAnchorElement?el:el.querySelector?.('a[href^="/products/'+safe+'/"]');if(a)replaceNearAnchor(a,image)})}
function scan(root=document){
 const anchors=[];if(root instanceof Element&&root.matches('a[href]'))anchors.push(root);root.querySelectorAll?.('a[href]').forEach(a=>anchors.push(a));
 for(const a of anchors){const slug=slugFromAnchor(a);if(slug){if(cache.has(slug))replaceNearAnchor(a,cache.get(slug));else pending.add(slug);continue}if(suggestion(a)){const q=queryFromAnchor(a);if(q){const slugForQuery=queryCache.get(q);if(slugForQuery&&cache.has(slugForQuery))replaceNearAnchor(a,cache.get(slugForQuery));else pendingQueries.add(q)}}}
 schedule();
}
...
function sendAsset(req,res,path){
  const js=path===JS_PATH,body=js?JS:CSS;
...
async function sendApi(req,res,url){
  ...
  try{
    const payload=await apiPayload(url);res.statusCode=200;res.setHeader('Content-Type','application/json; charset=utf-8');res.setHeader('Cache-Control','private, no-store');res.setHeader('X-Content-Type-Options','nosniff');res.setHeader('X-APG-Cross-Surface-Product-Imagery','v'+VERSION);return res.end(req.method==='HEAD'?'':JSON.stringify(payload));
  }catch{
    res.statusCode=503;res.setHeader('Content-Type','application/json; charset=utf-8');res.setHeader('Cache-Control','no-store');return res.end(req.method==='HEAD'?'':JSON.stringify({version:VERSION,commercialRecommendationWeight:0,images:[],queryImages:{},status:'temporarily-unavailable'}));
  }
}
...
function wrap(downstream){
  ...
  function handler(req,res){
    ...
    res.end=function(chunk,encoding,cb){
      ...
      const slugs=collectProductSlugs(body);
      currentMappings(slugs).then(mappings=>{patchResponseCsp(res);finish(decorateHtml(body,mappings));}).catch(()=>{patchResponseCsp(res);finish(injectAssets(body));});return res;
    };
