...
const research=require('./research-view-v44');
const researchPolish=require('./research-view-v431');

const CSS='/assets/platform-cohesion-v26.css?v=26';
const JS='/assets/platform-cohesion-v26.js?v=26.5';
const PRIMARY_ORIGIN='https://australianproductguide.au';
const SCOUT_EARLY_MARKER='data-v26-scout-early="true"';
const SCOUT_EARLY_GUARD=String.raw`<script data-v26-scout-early="true">(()=>{if(window.__APG_SCOUT_EARLY_GUARD_V26__)return;window.__APG_SCOUT_EARLY_GUARD_V26__=true;const hit=e=>e&&e.target&&e.target.closest?e.target.closest('[data-v26-scout-open]'):null;const isolate=e=>{if(hit(e))e.stopImmediatePropagation()};['pointerdown','mousedown','touchstart','pointerup','touchend'].forEach(t=>window.addEventListener(t,isolate,true));window.addEventListener('keydown',e=>{if(!hit(e)||(e.key!=='Enter'&&e.key!==' '))return;e.stopImmediatePropagation()},true);window.addEventListener('click',e=>{const trigger=hit(e);if(!trigger)return;e.preventDefault();e.stopImmediatePropagation();const open=window.__apgOpenScoutV26;if(typeof open==='function'){try{open(trigger);return}catch{}}window.__apgScoutPendingV26=trigger},true)})();</script>`;
...
function addEarlyScoutGuard(out){
  if(out.includes(SCOUT_EARLY_MARKER))return out;
  return out.replace(/<head\b[^>]*>/i,m=>m+SCOUT_EARLY_GUARD);
}
function addScoutNavigation(out){
  // Desktop and mobile Scout entries are independent surfaces. A mobile trigger can
  // already exist before this transform runs, so a global data-v26-scout-open check
  // would incorrectly suppress the primary-nav Scout button (and vice versa).
  if(!out.includes('apg-v26-scout-nav')){
    out=out.replace(
      '<a class="apg-power-link" href="/decision-lab/" data-decision-nav>Decision Lab</a>',
      '<a class="apg-power-link" href="/decision-lab/" data-decision-nav>Decision Lab</a><button type="button" class="nav-trigger apg-v26-scout-nav" data-v26-scout-open aria-controls="apgAssistantPanel">Ask Scout</button>'
    );
  }
...
function cohesionTransform(html,pathOrUrl){
  ...
  // Register the Scout event boundary while the document head is still being parsed.
  // This deliberately runs before deferred/body-end scripts so a fast mobile tap after
  // Search cannot reach an older capture handler first and become q=[object Object].
  out=addEarlyScoutGuard(out);
...
function applyIntegratedTransforms(html,url){
  let out=String(html||'');
  if(!out.includes('apg-exact-offers-v42'))out=commerce.commerceTransform(out,url);
...
module.exports.addEarlyScoutGuard=addEarlyScoutGuard;
module.exports.addScoutNavigation=addScoutNavigation;
module.exports.SCOUT_EARLY_GUARD=SCOUT_EARLY_GUARD;
module.exports.SCOUT_EARLY_MARKER=SCOUT_EARLY_MARKER;
