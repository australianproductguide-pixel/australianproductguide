...
const JS='/assets/platform-cohesion-v26.js?v=26.1';
...
function applyIntegratedTransforms(html,url){
  let out=String(html||'');
  // Some current product renderers already include the v42 exact-retailer block.
  // Keep the reconciliation layer idempotent so the same verified offer cannot
  // appear twice when older and newer runtime chains overlap.
