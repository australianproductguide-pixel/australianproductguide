...
const myApgAccount=require('./my-apg-account-v123-runtime');
...
function wrap(downstream){
  ...
  // My APG v123 is route-scoped and sits inside the established header/Navigator boundary.
  // It consolidates only /my-apg/ presentation and auth-journey ownership; authentication,
  // shopper state, decisions, evidence and retailer weighting remain with existing runtimes.
  const accountDownstream=myApgAccount.wrap(downstream);
  // Header Marketplace v122.6 remains inside Navigator so APG's established certification
  // invariant is preserved: Navigator stays the final Scout visual-cascade wrapper on every
  // route. v122.6 retains the v122.5 priority-first supermenu and semantic menu -> brand ->
  // account order while pinning the visible APG lock-up immediately beside the hamburger.
  const headerDownstream=headerMarketplace.wrap(accountDownstream);
  ...
  Object.assign(handler,smartDownstream,{
    ...
    MY_APG_ACCOUNT_JOURNEY_VERSION:myApgAccount.VERSION
  });
  return handler;
}
