...
function wrap(downstream){
  ...
  // route. v122.7 adds desktop-only presentation for the v122.5 supermenu while preserving
  // the v122.6 mobile lock-up/search and the v122.5 priority-first mobile hierarchy unchanged.
  const headerDownstream=headerMarketplace.wrap(downstream);
  // Smart Placement remains immediately inside Navigator, but its route-safety adapter guarantees
  // that unrelated routes (especially Home) bypass its write/end buffering entirely.
