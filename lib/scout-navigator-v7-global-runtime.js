...
const headerMarketplace=require('./header-marketplace-v1225-runtime');
...
function wrap(downstream){
  ...
  // Header Marketplace v122.5 remains inside Navigator so APG's established certification
  // invariant is preserved: Navigator is still the final Scout visual-cascade wrapper on every
  // route. v122.5 keeps menu -> brand -> account semantic order, pulls the brand closer to the
  // hamburger and replaces the category-heavy mobile drawer with progressive disclosure.
  ...
  Object.assign(handler,smartDownstream,{
    ...
    HEADER_MARKETPLACE_MOBILE_SUPERMENU_VERSION:headerMarketplace.VERSION
  });
  return handler;
}
