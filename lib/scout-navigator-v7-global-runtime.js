...
const headerAriaNormalisation=require('./header-navigation-v118-aria-normalisation');
...
function wrap(downstream){
  ...
  // Header v118 remains inside Navigator so APG's established certification invariant is preserved:
  // Navigator is still the final visual-cascade wrapper on every route. The header layer itself is
  // presentation/navigation only and contributes no product, evidence or retailer scoring.
  const headerDownstream=headerNavigation.wrap(downstream);
  // v118.1 is a narrow post-transform accessibility normaliser. It removes inherited duplicate
  // aria-controls/aria-expanded attributes and leaves one canonical #apgAllDrawer relationship.
  const accessibleHeaderDownstream=headerAriaNormalisation.wrap(headerDownstream);
  ...
  const smartDownstream=ebaySmartPlacement.wrap(accessibleHeaderDownstream);
  ...
  Object.assign(handler,smartDownstream,{SCOUT_NAVIGATOR_PRESENTATION_VERSION:VERSION,SCOUT_NAVIGATOR_CSS_PATH:CSS_PATH,scoutNavigatorCss:css,injectScoutNavigatorPresentation:inject,EBAY_SMART_PLACEMENT_VERSION:ebaySmartPlacement.VERSION,HEADER_NAVIGATION_VERSION:headerNavigation.VERSION,HEADER_ARIA_NORMALISATION_VERSION:headerAriaNormalisation.VERSION});
  return handler;
}
