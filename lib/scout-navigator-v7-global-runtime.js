...
const headerMarketplace=require('./header-marketplace-v1224-runtime');
...
function wrap(downstream){
  ...
  // Header Marketplace v122.4 remains inside Navigator so APG's established certification
  // invariant is preserved: Navigator is still the final Scout visual-cascade wrapper on every
  // route. v122.4 only reorders the mobile masthead to menu -> brand -> account while inheriting
  // v122.3 search removal; it cannot score, rank or persist state.
  ...
  Object.assign(handler,smartDownstream,{
    ...
    HEADER_MARKETPLACE_MOBILE_OWNERSHIP_VERSION:headerMarketplace.HEADER_MARKETPLACE_MOBILE_OWNERSHIP_VERSION,
    HEADER_MARKETPLACE_MOBILE_CONDENSED_VERSION:headerMarketplace.HEADER_MARKETPLACE_MOBILE_CONDENSED_VERSION,
    HEADER_MARKETPLACE_MOBILE_ORDER_VERSION:headerMarketplace.VERSION
  });
  return handler;
}
