...
// APG eBay Smart Placement pilot v1.2
// v1.0 was hidden below 960px. v1.1 made the APG section visible, but a 29 Aug 2026
// desktop screenshot then confirmed a second, separate defect: the APG shell rendered while the
// actual eBay banner area remained blank. v1.2 addresses the client-render path rather than
// misclassifying the shell as success. It loads eBay Smart Tools only after the placement element
// exists, scopes the extra eBay CSP capabilities commonly required by dynamic placements to
// /deals/ only, records a non-identifying load state, and provides a transparent fallback when the
// third-party placement cannot paint. APG recommendation and retailer-ranking weight remain zero.
const VERSION='1.2';
...
const CSS=`
.apg-ebay-smart-placement{padding-top:10px}
...
.apg-ebay-smart-placement .apg-ebay-smart-placement__frame{display:block;max-width:100%;overflow-x:auto;overflow-y:hidden;overscroll-behavior-inline:contain;-webkit-overflow-scrolling:touch;padding:2px 0 8px;min-height:220px}
.apg-ebay-smart-placement .apg-ebay-smart-placement__unit{display:block;width:900px;min-width:900px;min-height:220px;max-width:none;margin:0 auto}
.apg-ebay-smart-placement .apg-ebay-smart-placement__note{margin-top:12px}
.apg-ebay-smart-placement .apg-ebay-smart-placement__fallback{margin:10px 0 0;padding:12px 14px;border:1px solid #cbd5e1;border-radius:10px;background:#f8fafc;color:#475569;font-size:.92rem;line-height:1.5}
.apg-ebay-smart-placement .apg-ebay-smart-placement__fallback[hidden]{display:none!important}
@media(max-width:959px){
  .apg-ebay-smart-placement{display:block;padding-top:6px}
  .apg-ebay-smart-placement .apg-ebay-smart-placement__head{display:block}
  .apg-ebay-smart-placement .apg-ebay-smart-placement__head .text-link{display:inline-block;margin-top:10px}
  .apg-ebay-smart-placement .apg-ebay-smart-placement__frame{margin-right:calc(var(--page-gutter,20px) * -1);padding-right:var(--page-gutter,20px)}
  .apg-ebay-smart-placement .apg-ebay-smart-placement__unit{margin-left:0}
}
`;
...
const LOADER=`'use strict';(function(){
  function boot(){
    ...
    var fallback=root.querySelector('[data-apg-ebay-smart-fallback]');
    if(!unit)return;
    ...
    s.onload=function(){
      root.setAttribute('data-apg-ebay-load-state','script-loaded');
      window.setTimeout(function(){
        var painted=unit.children.length>0||String(unit.innerHTML||'').trim().length>0;
        root.setAttribute('data-apg-ebay-load-state',painted?'paint-detected':'script-loaded-no-paint');
        if(fallback)fallback.hidden=painted;
      },3500);
    };
    s.onerror=function(){
      root.setAttribute('data-apg-ebay-load-state','script-error');
...
function section(){
  return `<section class="section apg-ebay-smart-placement" data-apg-ebay-smart-placement="v${VERSION}" data-apg-ebay-load-state="ssr-ready" aria-labelledby="apgEbaySmartPlacementTitle"><div class="wrap"><div class="apg-ebay-smart-placement__head"><div><p class="kicker">eBay Australia marketplace discovery</p><h2 id="apgEbaySmartPlacementTitle">More electronics options on eBay Australia</h2><p>This live marketplace panel is separate from APG recommendations. It can surface current eBay listings related to electronics, but APG does not treat the banner as evidence that a listing is the best choice, an exact product match, or the best available price.</p></div><a class="text-link" href="/retailers/">How APG handles retailers →</a></div><div class="apg-ebay-smart-placement__frame" aria-label="eBay Australia Smart Placement"><ins class="epn-placement apg-ebay-smart-placement__unit" data-config-id="${CONFIG_ID}"></ins></div><p class="apg-ebay-smart-placement__fallback" data-apg-ebay-smart-fallback hidden><strong>eBay marketplace content did not load in this browser.</strong> The placement can be blocked by third-party network, browser privacy or eBay-side delivery conditions. APG has not replaced it with fabricated listings.</p><div class="notice affiliate-disclosure-inline apg-ebay-smart-placement__note"><strong>Paid marketplace content.</strong> APG may earn a commission from qualifying eBay purchases. eBay participation and commission do not influence APG product suitability, ranking or recommendations.</div><p class="fine-inline">The embedded marketplace content is supplied by eBay and can change independently of APG. Confirm the exact model or variant, seller, condition, warranty, delivery, price and availability on eBay before purchase.</p></div></section>`;
}
...
function enhanceDeals(html,path){
  ...
  if(!out.includes('data-apg-ebay-smart-placement=')){
    const anchor='<section id="shopping-destinations"';
