...
// APG eBay Smart Placement pilot v1.1
// The initial v1.0 pilot was correctly present in SSR but its own <960px media query hid the
// entire section at tablet/mobile/print widths. The user-supplied 29 Aug PDF demonstrated that
// exact failure mode: the page jumped from APG governance into the footer with no Smart Placement
// section. v1.1 keeps the exact eBay configuration, loads eBay's supplied script directly in the
// document <head>, keeps the supplied 900x220 unit scroll-contained on narrower viewports, and
// moves the pilot adjacent to APG's existing eBay discovery content so it is easy to verify.
const VERSION='1.1';
...
const LOADER_PATH='/assets/ebay-smart-placement-v1.js'; // compatibility endpoint; no longer required by head markup.
...
// Kept only so any previously cached same-origin reference fails safe by loading eBay's supplied
// script. The current head markup uses SMART_TOOLS_SRC directly, exactly as eBay provides it.
...
function addHeadAssets(html){
  let out=String(html);
  if(!out.includes(`href="${CSS_PATH}"`))out=out.replace(/<\/head>/i,`<link rel="stylesheet" href="${CSS_PATH}"><script async src="${SMART_TOOLS_SRC}" data-apg-ebay-smart-tools="true"></script></head>`);
  return out;
}
