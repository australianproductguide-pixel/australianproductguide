...
const ACTION2_MEASUREMENT_VERSION='87.1';
...
function classifyAmazonLink(url,amazonMeta={}){
  ...
  const matchStatus=String(amazonMeta.matchStatus||'').toUpperCase();
  if(matchStatus==='VARIANT_VERIFIED')return {kind:'direct',destination:'verified_variant'};
  if(matchStatus==='EXACT_VERIFIED')return {kind:'direct',destination:'direct_asin'};
...
function handler(req,res){
  ...
  res.setHeader('X-APG-Measurement-Truth','v'+ACTION2_MEASUREMENT_VERSION);
  return downstream(req,res);
}

Object.assign(handler,downstream,{ACTION2_MEASUREMENT_VERSION,CLIENT_PATH,SCOUT_API_PATH,classifyAmazonLink,enrichScoutPayload,patchClient,escAttr});
