...
// APG Missing Brand Logo Completion v73.1
//
...
//   - eagerly hydrate only those directory images so print/PDF capture cannot omit them.
const downstream=require('./earbuds-category-image-v72');

const BRAND_MARK_MISSING_ONLY_VERSION='73.1';
const ASSET_VERSION='73.1';
...
function patchTargetedDirectoryHtml(html,path){
  ...
  out=out.replace(/<img\b[^>]*\bsrc=["']\/assets\/brand-marks\/[^"']+["'][^>]*>/gi,tag=>{
    ...
    next=next.replace(/\bloading=["']lazy["']/i,'loading="eager"');
    if(!/\bloading=["'][^"']+["']/i.test(next))next=next.replace(/^<img\b/i,'<img loading="eager"');
