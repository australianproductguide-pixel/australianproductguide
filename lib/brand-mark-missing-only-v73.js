...
const MISSING_ONLY_SLUGS=Object.freeze([
  ...
  'hisense','honor','hulkman','hyperice','hyperx','iniu','inkbird','insta360','instant-pot',
...
function serve(req,res,image){
  ...
  res.setHeader('X-APG-Brand-Mark-Missing-Only','v'+VERSION);
...
function patchTargetedDirectoryHtml(html,path){
  ...
  if(!out.includes('name="apg-brand-mark-missing-only"')){
    out=out.replace('</head>',`<meta name="apg-brand-mark-missing-only" content="v${VERSION}"></head>`);
  }
  return out;
}
...
Object.assign(handler,downstream,{VERSION,ASSET_VERSION,MISSING_ONLY_SLUGS,TARGETS,svgRenderable,graphical,resolveMissingOnly,patchTargetedDirectoryHtml});
