...
function decorateHtml(html,mappings){
  return base.injectAssets(base.enrichProductArticles(replaceBrandPlaceholders(html,mappings),mappings));
}
...
function wrap(downstream){
  ...
  function handler(req,res){
    ...
    res.end=function(chunk,encoding,cb){
      ...
      base.currentMappings(slugs).then(mappings=>{base.patchResponseCsp(res);finish(decorateHtml(body,mappings));}).catch(()=>{base.patchResponseCsp(res);finish(base.injectAssets(body));});
      return res;
    };
...
module.exports={...base,VERSION,CSS,JS,containingArticleBounds,choosePlaceholderSlug,replaceBrandPlaceholders,decorateHtml,publicImage,decorateSearchPayload,apiPayload,wrap,install};
