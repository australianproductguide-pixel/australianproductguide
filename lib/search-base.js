...
function searchSite(query){
  ...
  const candidateProducts=category?products.filter(p=>p.category===category.slug):products;
  let productResults=candidateProducts.map(p=>{let score=tokenScore(q,p);if(category&&p.category===category.slug)score+=24;for(const tag of tags)if((p.tags||[]).includes(tag))score+=13;for(const tag of intent.excludedTags)if((p.tags||[]).includes(tag))score-=35;if(budget&&p.price){if(p.price<=budget)score+=18;else score-=30;}if(useDecision){const d=decisionRank.get(p.slug);if(d)score+=Math.max(-20,34-d.i*3)+Math.max(-12,Math.min(16,d.r.score/8));}if(p.evidenceTier==='deep')score+=1.5;return {p,score};}).filter(x=>x.score>0).sort((a,b)=>b.score-a.score);
  if(category&&productResults.length===0){
    productResults=candidateProducts.map(p=>({p,score:tokenScore(q,p)+(p.evidenceTier==='deep'?1.5:0)})).sort((a,b)=>b.score-a.score);
  }
  productResults=productResults.slice(0,18).map(x=>x.p);
  ...
  const candidatePairs=category?pairPages.filter(x=>x.category===category.slug):pairPages;
  const comparisonResults=candidatePairs.map(x=>{const hay=norm(`${x.a.brand} ${x.a.name} ${x.b.brand} ${x.b.name} ${categories[x.category]?.label||''}`);let score=0;for(const t of words(q))if(hay.includes(t))score+=4;if(category&&x.category===category.slug)score+=8;if(n.includes(' vs ')||n.includes(' versus '))score+=2;return {x,score};}).filter(x=>x.score>=6).sort((a,b)=>b.score-a.score).slice(0,6).map(x=>x.x);
  let directCompare=null;const split=q.split(/\s+(?:vs\.?|versus)\s+/i);if(split.length===2){const a=matchProduct(split[0]),b=matchProduct(split[1]);if(a&&b&&a.slug!==b.slug&&a.category===b.category)directCompare={a,b,url:`/compare/custom/?products=${a.slug},${b.slug}`};}
...
function evidenceMeta(p){return p.evidenceLabel|| (p.evidenceTier==='deep'?'Deep evidence':'Starter evidence');}
function searchIndex(){return [
  ...
  ...products.map(p=>({type:'product',label:`${p.brand} ${p.name}`,meta:`${p.categoryLabel} · ${evidenceMeta(p)}`,url:`/products/${p.slug}/`,slug:p.slug})),
  ...
  {type:'search',label:'65-inch TV for a bright room and gaming',meta:'Popular search',url:'/search/?q=65-inch+TV+for+a+bright+room+and+gaming'},
  {type:'search',label:'light laptop for university and travel',meta:'Popular search',url:'/search/?q=light+laptop+for+university+and+travel'},
  {type:'search',label:'9kg washing machine with automatic dosing',meta:'Popular search',url:'/search/?q=9kg+washing+machine+with+automatic+dosing'},
