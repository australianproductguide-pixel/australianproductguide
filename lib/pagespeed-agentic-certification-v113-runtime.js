...
// APG PageSpeed + Agentic Delivery Certification v113.1
...
// - follows APG's internal canonical asset redirects while building the bundle;
...
function blockingStylesheetLinks(html){
  ...
  // Mask no-JS fallbacks without changing offsets. Lighthouse runs with JS enabled, so
  // noscript styles are not render-blocking and must not be pulled into the blocking bundle.
...
function capture(handler,url,depth=0){
  ...
  // Many APG handlers are declared async for transport compatibility but synchronously
  // complete generated asset responses. Accept that proven synchronous completion while
  // still failing closed if a capture genuinely needs a later async turn.
...
function consolidateHomepageCss(html,downstream){
  ...
  // Fail closed: only rewrite the document after every current blocking stylesheet has
  // been captured successfully. Preload, print and noscript delivery is preserved.
...
module.exports={
  ...
  requestUrl,stylesheetLinks,blockingStylesheetLinks,internalCssHref,absoluteCssUrls,capture,buildCombinedCss,
