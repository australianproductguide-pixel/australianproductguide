...
//
// P0 recovery, 1 Sep 2026: Production bisect subsequently proved the native Home renderer healthy
// through the authoritative runtime and outer assembly once the Home-only nested Scout Navigator /
// Header Marketplace / Smart Placement response chain was bypassed. The former internal Home ->
// Search availability fallback is therefore RETIRED. Real HTTP requests for `/` now proceed through
// the native Home renderer. The legacy fallback constants/functions remain exported only for
// forensic/source compatibility and are inert; they must never rewrite a public Home request.
//
// The explicit diagnostic path remains available for the post-recovery watch. Only a real HTTP
// GET/HEAD request can activate it. The request is rewritten to the native Home route with the
// diagnostic query before entering the application, marked no-store/noindex, and any synchronous/
// returned-Promise failure is logged and rethrown so diagnostics never mask real Production failure
// semantics.
...
// Legacy forensic compatibility only. Runtime application is retired below.
...
const HOME_ROUTING_STATE='NATIVE_HOME_RESTORED';
...
function applyHomeAvailabilityFallback(_req,_res,_u){
  // RETIRED after exact-Production native Home + outer-assembly certification. Keep the symbol
  // inert for compatibility with incident diagnostics; public `/` must never be rewritten here.
  return false;
}
...
function wrap(downstream){
  ...
  Object.assign(handler,downstream,{
    ...
    APG_P0_HOME_ROUTING_STATE:HOME_ROUTING_STATE,
...
module.exports={
  ...
  HOME_AVAILABILITY_STATE,HOME_ROUTING_STATE,HOME_DIAGNOSTIC_QUERY,HOME_DIAGNOSTIC_PATH,HOME_DIAGNOSTIC_STATE,HOME_FALLBACK_URL,
