...
//
// P0 visitor-availability containment, 1 Sep 2026: the native Home route is still the sole route
// crashing in Production after the unsafe recursive capture, Proof Rail, eBay image wrappers and
// Smart Placement were independently ruled out as the sole cause. While the inherited Home-only
// chain is bisected, real HTTP requests for `/` are internally sent through the already-certified
// Search renderer at this outer transport boundary. Source/build QA requests do not carry a real
// IncomingMessage socket/httpVersion and therefore continue to certify the intended native Home.
// `?__apg_home_diag=1` deliberately bypasses the fallback for controlled Production diagnosis.
//
// The explicit diagnostic path below is intentionally outside `/`, so Vercel's temporary edge
// redirect continues protecting visitors. Only a real HTTP GET/HEAD request can activate it. The
// request is rewritten to the native Home route with the diagnostic query before entering the
// application, marked no-store/noindex, and any synchronous/returned-Promise failure is logged and
// rethrown so diagnostics never mask the real Production failure semantics.
...
const HOME_AVAILABILITY_STATE='TEMPORARY_INTERNAL_SEARCH_FALLBACK';
const HOME_DIAGNOSTIC_QUERY='__apg_home_diag';
const HOME_DIAGNOSTIC_PATH='/__apg-p0-home-native-20260901';
const HOME_DIAGNOSTIC_STATE='NATIVE_HOME_DIAGNOSTIC';
const HOME_FALLBACK_URL='/search/?__apg_home_fallback=1';
...
function liveHttpRequest(req){return Boolean(req&&(req.socket||req.httpVersion))}
function applyNativeHomeDiagnostic(req,res,u){
  if(!liveHttpRequest(req)||!['GET','HEAD'].includes(String(req.method||'GET').toUpperCase())||u.pathname!==HOME_DIAGNOSTIC_PATH)return false;
  req.url=`/?${HOME_DIAGNOSTIC_QUERY}=1`;
  res.setHeader('Cache-Control','no-store, max-age=0');
  res.setHeader('X-Robots-Tag','noindex, nofollow, noarchive');
  res.setHeader('X-APG-P0-Home-Diagnostic',HOME_DIAGNOSTIC_STATE);
  return true;
}
function applyHomeAvailabilityFallback(req,res,u){
  if(!liveHttpRequest(req)||u.pathname!=='/'||u.searchParams.get(HOME_DIAGNOSTIC_QUERY)==='1')return false;
  req.url=HOME_FALLBACK_URL;
  res.setHeader('X-APG-P0-Home-Availability',HOME_AVAILABILITY_STATE);
  res.setHeader('Cache-Control','no-store, max-age=0');
  return true;
}
function logNativeHomeDiagnosticError(error){
  const detail=error&&error.stack?error.stack:String(error);
  console.error('[APG_P0_NATIVE_HOME_DIAGNOSTIC_FAILURE]',detail);
}
function wrap(downstream){
  if(typeof downstream!=='function')throw new TypeError('PageSpeed + agentic certification requires downstream handler');
  function handler(req,res){
    let u=requestUrl(req);
    const homeDiagnostic=applyNativeHomeDiagnostic(req,res,u);
    if(homeDiagnostic)u=requestUrl(req);
    const homeFallback=applyHomeAvailabilityFallback(req,res,u);
    if(homeFallback)u=requestUrl(req);
    ...
    res.end=(body,...args)=>{
      ...
      res.setHeader('X-APG-PageSpeed-Build',BUILD_ID);
      res.setHeader('X-APG-PageSpeed-Runtime-CSS',RUNTIME_CSS_CONSOLIDATION);
      if(homeFallback)res.setHeader('X-APG-P0-Home-Availability',HOME_AVAILABILITY_STATE);
      if(homeDiagnostic)res.setHeader('X-APG-P0-Home-Diagnostic',HOME_DIAGNOSTIC_STATE);
      return end(body,...args);
    };
    try{
      const result=downstream(req,res);
      if(homeDiagnostic&&result&&typeof result.then==='function'){
        return Promise.resolve(result).catch(error=>{logNativeHomeDiagnosticError(error);throw error});
      }
      return result;
    }catch(error){
      if(homeDiagnostic)logNativeHomeDiagnosticError(error);
      throw error;
    }
  }
  Object.assign(handler,downstream,{
    ...
    PAGESPEED_AGENTIC_RUNTIME_CSS:RUNTIME_CSS_CONSOLIDATION,
    APG_P0_HOME_AVAILABILITY_STATE:HOME_AVAILABILITY_STATE,
    APG_P0_HOME_DIAGNOSTIC_QUERY:HOME_DIAGNOSTIC_QUERY,
    APG_P0_HOME_DIAGNOSTIC_PATH:HOME_DIAGNOSTIC_PATH
  });
  return handler;
}
...
module.exports={
  ...
  HOME_AVAILABILITY_STATE,HOME_DIAGNOSTIC_QUERY,HOME_DIAGNOSTIC_PATH,HOME_DIAGNOSTIC_STATE,HOME_FALLBACK_URL,
  ...
  consolidateHomepageCss,repairScoutAriaJs,repairStaticScoutAria,transformHtml,sendCombinedCss,liveHttpRequest,
  applyNativeHomeDiagnostic,applyHomeAvailabilityFallback,logNativeHomeDiagnosticError,wrap,install,
