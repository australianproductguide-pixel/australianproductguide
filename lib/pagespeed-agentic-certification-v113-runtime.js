...
// APG PageSpeed + Agentic Delivery Certification v113.3
...
// - normalises Scout's aside semantics to an allowed labelled region so Lighthouse's
//   agent accessibility tree does not fail aria-allowed-role.
...
function repairStaticScoutAria(html){
  return String(html||'').replace(/<aside\b[^>]*id=["']apgAssistantPanel["'][^>]*>/i,tag=>tag
    ...
    .replace(/\srole=["']dialog["']/i,' role="region"')
