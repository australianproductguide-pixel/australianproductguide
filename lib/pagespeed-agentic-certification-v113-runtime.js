...
// APG PageSpeed + Agentic Delivery Certification v113.4
// Transport-only optimisation around the final public response. It does not score,
// rank, persist shopper state or alter recommendation/retailer logic. It:
...
// - reads source-controlled static CSS from the bundled Vercel function and follows
//   generated CSS through the established runtime handler;
// - supports cold-start generation of the certified CSS asset when downstream discovery
//   completes asynchronously;
// - gives versioned generated assets immutable cache lifetimes;
// - removes a redundant aria-hidden write from Scout's natively hidden surface;
// - removes Scout's invalid dialog role from its aside host so the labelled native aside
//   remains valid in Lighthouse's agent accessibility tree.
...
async function captureAsync(handler,url,depth=0){
  if(depth>5)throw new Error(`too many internal redirects while capturing ${url}`);
  const bundled=bundledStaticCss(url);
  if(bundled)return bundled;
  ...
  const res={
    ...
    removeHeader(name){headers.delete(String(name).toLowerCase())},
    write(chunk){if(chunk!==undefined&&chunk!==null)body+=Buffer.isBuffer(chunk)?chunk.toString('utf8'):String(chunk);return true},
    end(chunk){if(chunk!==undefined&&chunk!==null)body+=Buffer.isBuffer(chunk)?chunk.toString('utf8'):String(chunk);if(!settled){settled=true;resolveEnd()}return body}
  };
  try{
    const result=handler(fakeRequest(url),res);
    if(result&&typeof result.then==='function')await result;
    if(!settled)await ended;
  }catch(error){if(!settled){settled=true;rejectEnd(error)}throw error}
  if(res.statusCode>=300&&res.statusCode<400){
    const location=headers.get('location');
    if(!location)throw new Error(`redirect without location while capturing ${url}`);
    const next=new URL(String(location),new URL(url,ORIGIN));
    if(next.origin!==ORIGIN)throw new Error(`cross-origin redirect while capturing ${url}`);
    return captureAsync(handler,next.pathname+next.search,depth+1);
  }
  return {statusCode:res.statusCode,headers,body};
}
...
async function buildCombinedCssAsync(downstream,links){
  const unique=uniqueCssHrefs(links);
  if(!unique.length)throw new Error('homepage has no render-blocking internal stylesheets to consolidate');
  const signature=unique.join('|');
  if(cachedHomeCss&&cachedHomeCssSignature===signature)return cachedHomeCss;
  const chunks=[];
  for(const href of unique){
    const asset=await captureAsync(downstream,href);
    const type=String(asset.headers.get('content-type')||'').toLowerCase();
    if(asset.statusCode!==200||!type.startsWith('text/css')||!asset.body.trim()){
      throw new Error(`unable to consolidate stylesheet ${href}: status=${asset.statusCode} type=${type||'none'}`);
    }
...
async function discoverAndBuildCombinedCssAsync(downstream){
  const home=await captureAsync(downstream,'/');
  const type=String(home.headers.get('content-type')||'').toLowerCase();
  if(home.statusCode!==200||!type.startsWith('text/html'))throw new Error(`unable to capture homepage for CSS discovery: ${home.statusCode}`);
  return buildCombinedCssAsync(downstream,blockingStylesheetLinks(home.body));
}
...
function sendCombinedCssResponse(req,res,body){
  res.statusCode=200;
...
function sendCombinedCss(req,res,downstream){
  if(cachedHomeCss)return sendCombinedCssResponse(req,res,cachedHomeCss);
  return discoverAndBuildCombinedCssAsync(downstream)
    .then(body=>sendCombinedCssResponse(req,res,body))
    .catch(error=>sendCombinedCssError(req,res,error));
}
