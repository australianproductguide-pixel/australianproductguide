...
// APG Audit Search + Mobile Search v119.1.
// Search v52 / Search Ranking v4 remain authoritative. This narrow layer protects
// canonical autocomplete navigation, restores a persistent mobile Search row in the
// marketplace-style header hierarchy, and removes only the duplicate homepage hero
// Search on mobile once that persistent header Search is present.
const VERSION='119.1';
...
@media(max-width:920px){
  ...
  .site-header .apg-audit-mobile-search-v119>input[type="search"]{
    min-height:42px!important;min-width:0!important;width:100%!important;
  ...
  .site-header .apg-audit-mobile-search-v119>button[type="submit"]{
    min-width:72px!important;min-height:42px!important;margin:0!important;padding:0 12px!important;
  ...
  /* Homepage has a universal Search immediately above the decision-nav now. Keep the
     hero focused on the decision CTA rather than repeating the same Search control. */
  body[data-apg-home-mobile-search-single="true"] main#main .global-search{
...
function preserveNativeProductNavigation(event){
  ...
  // Preserve the canonical anchor default; only stop legacy autocomplete handlers from
  // swallowing/re-writing the product navigation.
...
function inject(html,path='/'){
  let out=String(html||'');
  if(path==='/')out=out.replace(/<body(\s|>)/i,'<body data-apg-home-mobile-search-single="true"$1');
  ...
  if(!out.includes('data-apg-mobile-search-v119')){
    // Desired mobile hierarchy: menu + APG identity + account, then Search, then the
    // horizontal decision/navigation row. Insert before primary-nav rather than after it.
    if(/<nav\s+class="primary-nav"/i.test(out))out=out.replace(/<nav\s+class="primary-nav"/i,`${MOBILE_FORM}<nav class="primary-nav"`);
    else if(out.includes('</header>'))out=out.replace('</header>',`${MOBILE_FORM}</header>`);
  ...
  function handler(req,res){
    ...
    res.end=(body,...args)=>{
      ...
      if(req.method!=='HEAD'&&textual&&res.statusCode>=200&&res.statusCode<500&&type.startsWith('text/html')){
        const wasBuffer=Buffer.isBuffer(body),source=wasBuffer?body.toString('utf8'):body,next=inject(source,path);
