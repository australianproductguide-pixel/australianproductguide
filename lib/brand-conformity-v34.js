// Australian Product Guide Brand Conformity v34.1.
// Whole-site palette normalisation, first-party Scout character system and
// bounded mobile/navigation reconciliation over the observer-loop-free v32.5.1 baseline.
...
function scoutCharacterMarkup(){
  ...
  return `<span class="apg-scout-character-v34" data-apg-v322-scout="true" data-apg-scout-character="v34.1" aria-hidden="true"><svg viewBox="0 0 64 64" role="img" aria-hidden="true"><circle cx="32" cy="32" r="30" fill="#EFF6FF"/><path d="M17 14 26 5h12l9 9-4 4H21l-4-4Z" fill="#2563EB"/><rect x="8.5" y="13" width="47" height="37" rx="18.5" fill="#0F172A"/><rect x="13" y="17" width="38" height="29" rx="14.5" fill="#FFFFFF"/><path d="M15.5 21c4-6 10-9 16.5-9s12.5 3 16.5 9c-5-2-10.5-3-16.5-3s-11.5 1-16.5 3Z" fill="#2563EB"/><circle cx="24" cy="29" r="3.2" fill="#2563EB"/><circle cx="40" cy="29" r="3.2" fill="#2563EB"/><circle cx="25" cy="28" r="1.15" fill="#06B6D4"/><circle cx="41" cy="28" r="1.15" fill="#06B6D4"/><path d="M25 37c2.1 2.2 4.4 3.2 7 3.2s4.9-1 7-3.2" fill="none" stroke="#0F172A" stroke-width="2.4" stroke-linecap="round"/><circle cx="8.5" cy="30" r="4" fill="#2563EB"/><circle cx="55.5" cy="30" r="4" fill="#2563EB"/><path d="M17 48c3.8-3.8 8.8-5.7 15-5.7S43.2 44.2 47 48v10H17V48Z" fill="#2563EB"/><g transform="translate(22.2 43.8) scale(.105)" fill="#FFFFFF"><path d="M54 81 86 83 105 51 123 83 154 83 125 32 83 33Z"/><path d="M81 96 48 96 26 135 59 137Z"/><path d="M128 97 151 137 182 136 159 95Z"/><path d="M104 87 76 136 106 126 132 137Z"/></g></svg></span>`;
}

const css=`
/* APG Brand Conformity v34.1 — approved whole-site palette + Scout character. */
:root{
...
/* Scout v34.1 — recognisable APG navigator, never a bare logo or the superseded mascot. */
...
body[data-brand-conformity-v34="true"] .apg-assistant-launcher{background:var(--apg-v34-navy)!important;border-color:#334155!important}
...
/* One semantic mobile Decision Lab destination only; older direct anchors cannot resurface. */
body[data-brand-conformity-v34="true"] #mobileNav a[href="/decision-lab/"]:not(.apg-v325-decision-mobile){display:none!important}
...
const clientJs=`(()=>{
  ...
  let applying=false;
  const setGraphic=el=>{if(el&&!el.querySelector('[data-apg-scout-character="v34.1"]'))el.innerHTML=character;};
  const setText=(el,html,key)=>{if(el&&el.dataset[key]!=='true'){el.innerHTML=html;el.dataset[key]='true';}};
  const removeDecisionDuplicates=()=>{
    const nav=document.getElementById('mobileNav');if(!nav)return;
    const links=[...nav.querySelectorAll('a[href="/decision-lab/"]')];
    let keep=links.find(x=>x.classList.contains('apg-v325-decision-mobile'))||links[0];
    if(keep){keep.classList.add('apg-v325-decision-mobile');keep.setAttribute('aria-label','Decision Lab');}
    links.forEach(x=>{if(x!==keep)x.remove();});
  };
  const apply=()=>{
    if(applying)return;applying=true;
    try{
      removeDecisionDuplicates();
      ...
      if(panel){
        panel.dataset.apgScoutCharacter='v34.1';
        ...
        setText(brand,'<strong>Scout</strong><small>Your APG decision guide</small>','apgV341Copy');
      }
      if(launcher){
        launcher.dataset.apgScoutCharacter='v34.1';
        ...
        setText(copy,'<strong>Ask Scout</strong><small>Your APG decision guide</small>','apgV341Copy');
      }
    }finally{applying=false;}
  };
  const schedule=()=>{apply();requestAnimationFrame(apply);[60,250,900].forEach(ms=>setTimeout(apply,ms));};
  const relevant=e=>e.target&&e.target.closest&&e.target.closest('#apgAssistantLauncher,[data-v26-scout-open],#apgAssistantPanel,[data-mobile-toggle],#mobileNav');
  const boot=()=>{
    schedule();
    document.addEventListener('click',e=>{if(relevant(e))schedule();},true);
    document.addEventListener('submit',e=>{if(e.target&&e.target.closest&&e.target.closest('#apgAssistantPanel'))schedule();},true);
  };
...
function normaliseMobileNav(html){
  return String(html||'').replace(/(<nav\b[^>]*id=["']mobileNav["'][^>]*>)([\s\S]*?)(<\/nav>)/i,(all,open,inner,close)=>{
    const anchors=inner.match(/<a\b[^>]*href=["']\/decision-lab\/["'][^>]*>[\s\S]*?<\/a>/gi)||[];
    if(anchors.length<=1)return all;
    const keep=anchors.find(x=>/apg-v325-decision-mobile/.test(x))||anchors[0];
    let kept=false;
    inner=inner.replace(/<a\b[^>]*href=["']\/decision-lab\/["'][^>]*>[\s\S]*?<\/a>/gi,m=>{
      if(!kept&&m===keep){kept=true;return m;}return '';
    });
    return open+inner+close;
  });
}
function inject(html){
  let out=normaliseMobileNav(normaliseText(String(html||'')));
  if(out.includes('data-brand-conformity-v341="true"'))return out;
  if(out.includes('data-brand-conformity-v34="true"'))out=out.replace('data-brand-conformity-v34="true"','data-brand-conformity-v341="true" data-brand-conformity-v34="true"');
  else out=out.replace(/<body\b([^>]*)>/i,'<body data-brand-conformity-v341="true" data-brand-conformity-v34="true"$1>');
  if(!out.includes(CSS_PATH+`?v=${VERSION}`))out=out.replace('</head>',`<link rel="stylesheet" href="${CSS_PATH}?v=${VERSION}"></head>`);
  if(!out.includes(JS_PATH+`?v=${VERSION}`))out=out.replace('</body>',`<script src="${JS_PATH}?v=${VERSION}" defer></script></body>`);
  return out;
}
...
Object.assign(handler,v29,v32,upstream,{VERSION,CSS_PATH,JS_PATH,css,clientJs,scoutCharacterMarkup,normaliseText,normaliseMobileNav,inject,transform});
