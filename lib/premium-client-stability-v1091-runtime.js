...
const VERSION='109.1';
const SCOUT_GLOBAL_SURFACE_VERSION='111.1';
...
// v111.x additionally ships a dedicated cache-distinct stylesheet on every document route.
// Strong ID-based rules do not depend on route/body feature attributes, so a valid Scout
// shell cannot be visually suppressed merely because a page family differs. v111.1 also
// explicitly yields to the established footer overlap guard so Scout never blocks footer links.
...
/* APG Scout Global Surface v111.1 — visible/clickable globally, footer-safe. */
...
/* The existing footer observer owns this class. Optional Scout must yield while the
   footer interaction zone is active, then automatically return when the guard clears. */
html body:not(.scout-v5-open) #apgAssistantLauncher.apg-assistant-launcher.apg-footer-overlap-guard{
