...
async function vercelSubjectToken(){
  ...
  const token=await getVercelOidcToken({project:'au-product-guide',team:'australian-product-guide'});
  if(!token)throw new Error('Vercel OIDC token is unavailable. Confirm Team issuer mode is enabled for this deployment.');
  return token;
}
...
async function exchangeFederatedToken(){
  const body=new URLSearchParams({
    ...
    subject_token:await vercelSubjectToken()
  });
...
function flattenGaProperties(accountSummaries){
  ...
  for(const account of accountSummaries||[]){
    for(const property of account.propertySummaries||[]){
      properties.push({account:account.account,accountDisplayName:account.displayName,property:property.property,propertyId:String(property.property||'').replace(/^properties\//,''),displayName:property.displayName,propertyType:property.propertyType,parent:property.parent});
    }
  }
  return properties;
}
...
function isoDate(daysAgo){return new Date(Date.now()-daysAgo*86400000).toISOString().slice(0,10);}

async function searchConsoleSnapshot(siteUrl=CONFIG.searchConsoleSite){
  const body={startDate:isoDate(28),endDate:isoDate(1),dimensions:['query'],rowLimit:10,dataState:'final'};
  const data=await googleFetch(`https://www.googleapis.com/webmasters/v3/sites/${encodeURIComponent(siteUrl)}/searchAnalytics/query`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});
  return {siteUrl,period:{startDate:body.startDate,endDate:body.endDate},topQueries:(data.rows||[]).map(row=>({query:row.keys&&row.keys[0],clicks:row.clicks,impressions:row.impressions,ctr:row.ctr,position:row.position}))};
}

async function analyticsSnapshot(propertyId){
  if(!propertyId)return null;
  const body={dateRanges:[{startDate:'28daysAgo',endDate:'yesterday'}],metrics:[{name:'activeUsers'},{name:'sessions'},{name:'engagedSessions'}]};
  const data=await googleFetch(`https://analyticsdata.googleapis.com/v1beta/properties/${encodeURIComponent(propertyId)}:runReport`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});
  const values=(data.rows&&data.rows[0]&&data.rows[0].metricValues)||[];
  return {propertyId:String(propertyId),period:'28daysAgo to yesterday',activeUsers:Number(values[0]&&values[0].value||0),sessions:Number(values[1]&&values[1].value||0),engagedSessions:Number(values[2]&&values[2].value||0)};
}

async function diagnose({includeSnapshots=true}={}){
  const [sites,accountSummaries]=await Promise.all([listSearchConsoleSites(),listAnalyticsAccountSummaries()]);
  const properties=flattenGaProperties(accountSummaries);
  const selectedProperty=selectGaProperty(properties);
  const result={ok:true,auth:'vercel-oidc-google-wif-service-account',projectId:CONFIG.projectId,searchConsole:{configuredSite:CONFIG.searchConsoleSite,accessibleSites:sites.map(item=>({siteUrl:item.siteUrl,permissionLevel:item.permissionLevel}))},analytics:{accessibleProperties:properties,selectedProperty}};
  if(includeSnapshots){
    const site=sites.find(item=>item.siteUrl===CONFIG.searchConsoleSite)||sites[0];
    const [gsc,ga4]=await Promise.all([site?searchConsoleSnapshot(site.siteUrl):Promise.resolve(null),selectedProperty?analyticsSnapshot(selectedProperty.propertyId):Promise.resolve(null)]);
...
module.exports={CONFIG,SERVICE_ACCOUNT_SCOPES,providerAudience,getGoogleAccessToken,googleFetch,listSearchConsoleSites,listAnalyticsAccountSummaries,flattenGaProperties,selectGaProperty,searchConsoleSnapshot,analyticsSnapshot,diagnose};
