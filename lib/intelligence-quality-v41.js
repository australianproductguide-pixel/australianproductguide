...
function exact75Gate(){
  ...
  const pass=!!top&&top.hardConstraintStatus==='eligible'&&Number(top.priceBasis)>0&&Number(top.priceBasis)<=2500&&/75/.test(String(top.name||''))&&exact.length>=4&&priced.length>=1&&brightRoomEvidenceAligned;
  return {pass,query:q,top:top?.slug||null,status:top?.hardConstraintStatus||'none',priceBasis:top?.priceBasis||null,exact75Maintained:exact.length,exact75WithMaintainedPriceAtOrBelow2500:priced.length,brightRoomEvidenceAligned,limitation:pass?null:'The exact 75-inch benchmark is missing a verified eligibility, price or explanation-evidence requirement.'};
}
function depthGate(){
