...
//
// Homepage CSS consolidation is build-time only. The public runtime will use the static bundle
// only when its embedded stylesheet-link signature exactly matches the final Home HTML. A missing
// or stale bundle fails closed to the established stylesheet cascade; no live request recursively
// invokes the application or assembles CSS.
...
function bundleInfo(){
  ...
  try{
    const css=fs.readFileSync(HOME_BUNDLE_FILENAME,'utf8');
    if(Buffer.byteLength(css)<250000)throw new Error('bundle too small');
    const signature=(css.match(/APG_HOME_CSS_LINK_SIGNATURE:([a-f0-9]{64})/i)||[])[1]||'';
    if(!signature)throw new Error('bundle signature missing');
    const hash=crypto.createHash('sha256').update(css).digest('hex').slice(0,20);
    cachedBundleInfo=Object.freeze({signature,hash,href:`${HOME_BUNDLE_PATH}?v=${hash}`});
  }catch{cachedBundleInfo=null;}
...
function wrap(downstream){
  ...
  function handler(req,res){
    ...
    res.end=(body,...args)=>{
      ...
      res.setHeader(HEADER_NAME,'v'+VERSION);
      if(url.pathname==='/'&&String(body||'').includes('data-apg-home-css-bundle='))res.setHeader('X-APG-Home-CSS-Bundle','v'+VERSION);
...
module.exports={
  ...
  injectMarker,bundleInfo,protectNoscript,removableCssTag,consolidateHomepageCss,transformHtml,
  isVersionedAsset,wrap
};
