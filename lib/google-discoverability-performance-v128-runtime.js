...
//
// Homepage CSS consolidation remains build-time only. Runtime delivery consumes a tiny,
// source-controlled manifest rather than reading, hashing or packaging the 500+ KiB generated CSS
// file inside the public function. The static bundle is used only when its certified stylesheet
// signature exactly matches the final Home HTML. Any stale manifest or presentation exception
// fails closed to the established stylesheet cascade and unchanged downstream response.
...
function validManifest(manifest=HOME_BUNDLE_MANIFEST){
  return Boolean(
    ...
    Number.isInteger(manifest.expandedBytes)&&manifest.expandedBytes>=250000&&
    Number.isInteger(manifest.bytes)&&manifest.bytes>=250000&&
    Number.isInteger(manifest.brotliBytes)&&manifest.brotliBytes>0&&manifest.brotliBytes<manifest.bytes&&
    Number.isInteger(manifest.gzipBytes)&&manifest.gzipBytes>0&&manifest.gzipBytes<manifest.bytes&&
...
function wrap(downstream){
  ...
  function handler(req,res){
    ...
    // Invariant observability headers are set before downstream rendering. The previous boundary
    // set headers from res.end, which is unsafe after a response has committed headers.
    safeSetHeader(res,HEADER_NAME,'v'+VERSION);
    safeSetHeader(res,DELIVERY_STABILITY_HEADER,'v'+DELIVERY_STABILITY_VERSION);
    ...
    res.end=(body,...args)=>{
      ...
      try{
        ...
        if(headersMutable&&url.pathname==='/'&&String(nextBody||'').includes('data-apg-home-css-bundle=')){
          ...
          safeSetHeader(res,HOME_BUNDLE_MANIFEST_HEADER,HOME_BUNDLE_MANIFEST.sha256.slice(0,20));
        }
      }catch(error){
