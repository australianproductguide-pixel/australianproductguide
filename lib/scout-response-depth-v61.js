...
const SITE_CONTEXT={
  '/methodology/':{label:'Methodology',purpose:'explains how APG turns maintained evidence, hard constraints and user priorities into recommendations without giving affiliate economics any suitability weight',actions:[['Read Sources','/sources/'],['See Affiliate Disclosure','/affiliate-disclosure/']]},
  '/sources/':{label:'Sources',purpose:'explains APG’s evidence hierarchy, provenance expectations and why manufacturer, manual/support, exact Australian retailer and credible independent sources are treated differently',actions:[['Open Methodology','/methodology/'],['See Corrections','/corrections-policy/']]},
  '/corrections-policy/':{label:'Corrections policy',purpose:'explains how APG handles reported errors, evidence changes and corrections so a product decision can be updated transparently',actions:[['Contact APG','/contact/'],['Review Sources','/sources/']]},
  '/affiliate-disclosure/':{label:'Affiliate disclosure',purpose:'explains APG’s commercial relationships and the rule that affiliate availability or commission contributes zero recommendation points',actions:[['Open Methodology','/methodology/'],['Retailer approach','/retailers/']]},
  '/privacy/':{label:'Privacy policy',purpose:'explains what APG collects, how consented analytics and account features are handled, and the privacy boundaries around decision and Scout interactions',actions:[['Open My APG','/my-apg/'],['Contact APG','/contact/']]},
  '/terms/':{label:'Terms of use',purpose:'sets the conditions for using APG, including the limits of product guidance, retailer information and site content',actions:[['Read Methodology','/methodology/'],['Contact APG','/contact/']]},
  '/coverage/':{label:'Coverage',purpose:'shows what APG currently maintains and makes catalogue/evidence limitations visible rather than implying universal market coverage',actions:[['Browse categories','/categories/'],['Recently updated','/updates/']]},
  '/updates/':{label:'Recently updated',purpose:'shows APG freshness and maintenance activity so shoppers can distinguish maintained research from generic evergreen copy',actions:[['Coverage','/coverage/'],['Sources','/sources/']]},
  '/about/':{label:'About APG',purpose:'explains what Australian Product Guide is trying to do for Australian shoppers and the decision-first principles behind the site',actions:[['How APG recommends','/methodology/'],['Browse categories','/categories/']]},
  '/contact/':{label:'Contact',purpose:'provides the route for questions, feedback and correction reports that need human follow-up',actions:[['Corrections policy','/corrections-policy/'],['About APG','/about/']]},
  '/retailers/':{label:'Retailer approach',purpose:'explains exact-model, verified-variant and fallback retailer pathways and why retailer participation never changes recommendation rank',actions:[['Affiliate disclosure','/affiliate-disclosure/'],['Sources','/sources/']]},
  '/brands/':{label:'Brands',purpose:'is APG’s maintained brand directory for moving from a known manufacturer into relevant products and category decisions',actions:[['Browse categories','/categories/'],['Search APG','/search/']]},
  '/categories/':{label:'Categories',purpose:'is the main maintained product-category directory and a starting point for category research, Help Me Choose, buying guides and comparisons',actions:[['Search APG','/search/'],['Open Decision Lab','/decision-lab/']]},
  '/guides/':{label:'Buying guides',purpose:'collects APG’s decision-first buying guidance so customers can understand category trade-offs before narrowing to a model',actions:[['Browse categories','/categories/'],['Open Decision Lab','/decision-lab/']]}
};
...
function institutionalContext(pageContext){return SITE_CONTEXT[pageContext.path]||null;}
function institutionalResponse(ctx,state){return {version:core.VERSION,intent:'apg_information',message:`You’re on APG’s ${ctx.label} page. It ${ctx.purpose}.`,bullets:['I can explain this page in plain English, point you to the related APG control or take you back into a product decision.','This information is part of APG’s trust and operating framework rather than a product-ranking signal.'],actions:[...(ctx.actions||[]).map(([label,url],index)=>link(label,url,index===0)),prompt('What should I do next?')],decisionState:state,meta:{scoutResponseDepthVersion:VERSION,siteKnowledge:'maintained-route-context',commercialRecommendationWeight:0}};}

function response(input,pageContext){
  const text=norm(input.text),state=safeState(input),trace=lastTrace(state),product=productFor(pageContext),category=categoryFor(pageContext,state),label=categoryLabel(category),siteContext=institutionalContext(pageContext);
  ...
  const asksPageGuide=/help me (?:with|on) this page|what is this page|explain this page|what is this page for|guide me here/.test(text);
  if(siteContext&&asksPageGuide)return institutionalResponse(siteContext,state);
  ...
  if(asksNext){
    ...
    if(!bullets.length&&siteContext)bullets.push(`Use ${siteContext.label} to understand this part of APG, then return to Search, Decision Lab or Compare when you are ready to continue the product decision.`);
  ...
  if(asksSummary){
    ...
    if(siteContext)return institutionalResponse(siteContext,state);
...
function classifyIntent(text,pageContext={}){
  ...
  if(SITE_CONTEXT[String(pageContext&&pageContext.path||'')]&&/help me (?:with|on) this page|what is this page|explain this page|guide me here/.test(q))return 'customer_decision_support';
...
module.exports={VERSION,SITE_CONTEXT,install,response,institutionalContext,institutionalResponse,get installed(){return installed;}};
