...
const CSS=`
[data-consumer-intelligence-v47=true] .ci47-panel{margin:28px 0;border:1px solid #cedfdd;border-radius:20px;background:linear-gradient(145deg,#f7fbfa,#fff);padding:22px;box-shadow:0 10px 30px rgba(16,48,59,.05)}
[data-consumer-intelligence-v47=true] .ci47-panel h2{margin:.2rem 0 .55rem;font-size:clamp(1.35rem,2vw,1.75rem)}
[data-consumer-intelligence-v47=true] .ci47-panel p{max-width:76ch}
[data-consumer-intelligence-v47=true] .ci47-proof{display:flex;gap:8px;flex-wrap:wrap;margin:14px 0}
[data-consumer-intelligence-v47=true] .ci47-proof span{display:inline-flex;align-items:center;min-height:32px;padding:6px 10px;border:1px solid #d9e6e4;border-radius:999px;background:#fff;font-size:.84rem;line-height:1.2;color:#23414a}
[data-consumer-intelligence-v47=true] .ci47-actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:15px}
[data-consumer-intelligence-v47=true] .ci47-actions a{display:inline-flex;align-items:center;justify-content:center;min-height:42px;padding:9px 14px;border-radius:12px;border:1px solid #1d655e;text-decoration:none;font-weight:700}
[data-consumer-intelligence-v47=true] .ci47-actions a:first-child{background:#164f4a;color:#fff}
[data-consumer-intelligence-v47=true] .ci47-actions a:not(:first-child){background:#fff;color:#164f4a}
[data-consumer-intelligence-v47=true] .ci47-note{display:block;margin-top:12px;color:#5a6f75;font-size:.82rem;line-height:1.45}
[data-consumer-intelligence-v47=true] .ci47-handoff{margin:20px 0;padding:14px 16px;border:1px solid #d6e4e2;border-radius:15px;background:#f8fbfb;display:flex;align-items:center;justify-content:space-between;gap:14px;flex-wrap:wrap}
[data-consumer-intelligence-v47=true] .ci47-handoff strong{display:block;margin-bottom:3px}
[data-consumer-intelligence-v47=true] .ci47-handoff a{font-weight:800;color:#164f4a}
@media(max-width:700px){[data-consumer-intelligence-v47=true] .ci47-panel{padding:17px;border-radius:16px}[data-consumer-intelligence-v47=true] .ci47-actions{display:grid;grid-template-columns:1fr}[data-consumer-intelligence-v47=true] .ci47-actions a{width:100%}}
`;
...
function consumerSnapshot(){
  ...
  const releaseGate={
    pass:maintainedCategories===mappedCategoryImages&&categoryImagery.finalReviewRequired===0&&imagery.invalid===0&&retailers.exactOfferCount>=52&&retailers.productsWithExactOffers>=46&&retailers.verifiedRetailers>=20&&obs.recommendation?.affiliateRecommendationWeight===0,
    requirements:['Every maintained category has reviewed editorial imagery','No invalid verified product-image record','At least 52 exact Australian retailer/manufacturer destinations across at least 46 products','At least 20 verified retailer/manufacturer sources','Retailer and affiliate participation remain zero recommendation weight']
  };
  return {
...
function productPanel(url){
  ...
  const offers=(node.commerce?.offers||[]),currentOffers=offers.filter(x=>x.freshness==='current-check'&&x.exactModel&&x.url).length;
  ...
  const decisionUrl=`/decision-lab/?q=${encodeURIComponent(brief)}`;
  ...
  return `<section class="section ci47-section" aria-label="Continue this product decision"><div class="wrap"><div class="ci47-panel"><p class="kicker">Connected decision intelligence</p><h2>Keep this decision moving</h2><p>Carry this maintained product context into APG’s other decision tools instead of starting again. Decision Lab can apply your budget, must-haves and priorities; Compare can expose the trade-offs against a close maintained alternative.</p><div class="ci47-proof"><span>${facts} maintained fact-level evidence point${facts===1?'':'s'}</span><span>${currentOffers} current exact-model retailer check${currentOffers===1?'':'s'}</span><span>Product photography: ${imageReady?'verified authorised image':'awaiting an authorised exact-product source'}</span></div><div class="ci47-actions"><a href="${esc(decisionUrl)}">Refine in Decision Lab</a>${compare}<a href="${esc(searchUrl)}">Search ${esc(category?.label||product.categoryLabel||'this category')}</a></div><small class="ci47-note">Retailer coverage, affiliate status and image availability contribute zero recommendation points. Missing evidence stays visible rather than being guessed.</small></div></div></section>`;
}
...
function transform(html,url){
  ...
  const product=productPanel(url),handoff=handoffPanel(url);
  const addition=[product,handoff].filter(Boolean).join('');
  if(addition&&!out.includes('Connected decision intelligence')&&!out.includes('Want a more explicit decision?')&&!out.includes('Want to widen the discovery set?'))out=out.replace('</main>',`${addition}</main>`);
  return out;
}
...
Object.assign(handler,{VERSION,CSS_PATH,API_PATH,MARKER,CSS,consumerSnapshot,productPanel,handoffPanel,transform,downstream});
