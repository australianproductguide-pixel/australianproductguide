...
// APG eBay image search plan v1.1
...
// v1.1 adds maintained model/product-code evidence from structured specifications and official
// source URLs, and removes feature-count descriptors such as 3-in-1 from model search plans.
...
function sourceModelHints(product){
  ...
  for(const source of sourceUrls(product)){
    try{
      ...
      out.push(...modelTermsFromText(decoded));
    }catch{}
  }
...
function specificationModelHints(product){
  ...
  for(const value of matcher.specModelValues(product)){
    ...
    if(terms.length)out.push(...terms);else if(/[a-z]/i.test(value)&&/\d/.test(value))out.push(value);
  }
...
function productModels(product){
  return uniq([
    ...matcher.modelTokens(product).filter(token=>!matcher.isFeatureDescriptorModel(token)),
    ...specificationModelHints(product),
    ...sourceModelHints(product),
    ...explicitAliases(product).flatMap(modelTermsFromText)
  ]).slice(0,4);
}
...
function plansFor(product,{maxQueries=MAX_QUERIES}={}){
  ...
  if(models.length)pushPlan(plans,seen,'brand-model',{q:clean([brand,models.join(' ')].filter(Boolean).join(' ')),categoryIds:ebayCategoryId});
...
module.exports={VERSION,MAX_QUERIES,SEARCH_LIMIT,NEW_CONDITION_FILTER,CATEGORY_HINTS,clean,norm,compact,uniq,brandlessName,explicitAliases,sourceUrls,modelTermsFromText,sourceModelHints,specificationModelHints,categoryHints,productModels,plansFor,publicPlans};
