...
// APG eBay image search plan v1.3
...
// v1.3 preserves precise source/spec model evidence while adding bounded support for legitimate
// manufacturer model slashes (for example BT5515/15), a safe base-model alias, and removal of
// duplicated brand prefixes from model hints before eBay queries are assembled.
...
function plansFor(product,{maxQueries=MAX_QUERIES}={}){
  ...
  const models=productModels(product),hints=categoryHints(product),identity=matcher.identityTokens(product).slice(0,6).join(' ');
  const gtin=clean(product&&product.gtin),epid=clean(product&&product.epid),ebayCategoryId=clean(product&&product.ebayCategoryId);
...
module.exports={VERSION,MAX_QUERIES,SEARCH_LIMIT,NEW_CONDITION_FILTER,CATEGORY_HINTS,clean,norm,compact,uniq,brandlessName,stripBrandPrefix,explicitAliases,sourceUrls,plausibleModelHint,modelTermsFromText,expandModelHint,sourcePieces,sourceModelHints,specificationModelHints,categoryHints,productModels,plansFor,publicPlans};
