...
function productDisplayName(p){const brand=String(p.brand||'').trim(),name=String(p.name||'').trim();if(!brand)return name;if(name.toLocaleLowerCase('en-AU').startsWith(brand.toLocaleLowerCase('en-AU')))return name;return `${brand} ${name}`.trim();}
