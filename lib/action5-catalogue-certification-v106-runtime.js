...
const catalogueCoverage=require('./catalogue-coverage-v117');
...
function handler(req,res){
  ...
  res.setHeader=function(name,value){
    ...
    if(key==='x-apg-amazon-catalogue-certification')return originalSetHeader(name,'v'+VERSION);
    if(key==='x-apg-catalogue-coverage')return originalSetHeader(name,'v'+catalogueCoverage.VERSION);
    return originalSetHeader(name,value);
  };
  originalSetHeader('X-APG-Amazon-Catalogue-Certification','v'+VERSION);
  originalSetHeader('X-APG-Catalogue-Coverage','v'+catalogueCoverage.VERSION);
  ...
  if(path===catalogueCoverage.ENDPOINT){res.statusCode=200;originalSetHeader('Content-Type','application/json; charset=utf-8');originalSetHeader('Cache-Control','no-store');return res.end(JSON.stringify(catalogueCoverage.snapshot()));}
  return downstream(req,res);
}

Object.assign(handler,downstream,{
  ACTION5_CATALOGUE_CERTIFICATION_VERSION:VERSION,
  amazonCatalogueCertificationSnapshot:snapshot,
  AMAZON_CATALOGUE_CERTIFICATION_ENDPOINT:ENDPOINT,
  CATALOGUE_COVERAGE_VERSION:catalogueCoverage.VERSION,
  CATALOGUE_COVERAGE_ENDPOINT:catalogueCoverage.ENDPOINT,
  catalogueCoverageSnapshot:catalogueCoverage.snapshot,
  PLATFORM_STATE_VERSION:platformState.VERSION,
  PLATFORM_STATE_ENDPOINT:platformState.ENDPOINT,
  platformStateSnapshot:(env=process.env)=>platformState.snapshot({downstream,retailerSnapshot:snapshot(),env})
});
