...
// APG eBay product-image exact identity guard v2.4.
// Extends the conservative v2.3 named-product fallback for legitimate branded retail identities
// that do not expose a manufacturer-SKU-shaped model token (for example Tile Mate or Tile Pro).
// The fallback requires token-bounded brand evidence and either the exact branded maintained name
...
function namedIdentityCheck(product,accepted,allProducts=[]){
  ...
  const multiWordComplete=coreWords.length>=2&&coverage===1&&(
    exactBrandedPhrase||coreWords.some(word=>/\d/.test(word))||coreWords.filter(word=>word.length>=4).length>=2
  );
  if(!oneWordExact&&!multiWordComplete){
    return {ok:false,reason:'named-product-full-name-missing',core,matched,coverage,exactBrandedPhrase};
  }
  ...
  return {ok:true,reason:'exact-named-product-title-evidence',core,matched,coverage,exactBrandedPhrase};
}
...
module.exports={VERSION,MODEL_GATE_FAILURES,phraseInTitle,brandlessName,meaningfulModelTokens,nameCoreWords,nameCore,namedIdentityCheck,evaluate};
