...
// APG eBay product-image exact identity guard v3.24.
// Extends v3.23 with one persisted, item-bound Dyson Purifier Cool PC1 identity recovery. The exact
// current eBay AU whole-product item is bound to Dyson model 544929-01, which is independently the PC1
// model identity, so runtime rendering can reproduce the same strict evidence decision made at discovery.
// Miofive S1, Apple AirTag 4 Pack, PETLIBRO, Revodok Pro 106, ZeroWater, Revodok 1071, Insta360 X4/X5,
// Paperwhite and prior rules are unchanged. Generic accessory, sibling, bundle, category, condition,
// voltage, AUD-price, eBay-image-host, eBay AU URL and active-listing controls remain fail-closed,
// including the v3.13 mount protection.
...
function verifiedDirectNamedIdentity(product,accepted,hostCompatibility){
  ...
  return Boolean(expected&&accepted&&accepted.searchKind==='verified-direct-item'&&clean(accepted.itemId)===expected&&((hostCompatibility&&hostCompatibility.ok)||evidenceBound));
}
