...
// APG eBay product-image exact identity guard v3.4.
// Extends v3.3 with a tightly bounded set of category-path exceptions for current eBay AU
// candidates that already passed APG's detailed brand/model/condition checks and were rejected only
// because eBay placed the whole product under an accessory-like parent category. Every exception is
// restricted to the maintained APG slug, exact current Browse item IDs and exact title identity
// phrases; parts/accessories/replacement leaves remain blocked and all ordinary model, sibling,
// bundle, voltage, AUD-price, URL, active-listing and second-pass controls still run.
...
function evidenceBoundWholeProductLeaf(product,accepted){
  ...
  if(!rule||clean(product&&product.category)!==rule.category||!itemId||!Array.isArray(rule.itemIds)||!rule.itemIds.includes(itemId)||!leaf||UNSAFE_ACCESSORY_LEAF.test(leaf))return false;
...
function safeWholeProductLeaf(product,accepted){
  const leaf=categoryLeaf(accepted);if(!leaf||UNSAFE_ACCESSORY_LEAF.test(leaf))return false;
  const rules=ACCESSORY_PARENT_WHOLE_PRODUCT_LEAVES[clean(product&&product.category)]||[];
  return rules.some(pattern=>pattern.test(leaf))||evidenceBoundWholeProductLeaf(product,accepted);
}
function accessoryParentWholeProductOverride(product,accepted){
  ...
  const rules=ACCESSORY_PARENT_WHOLE_PRODUCT_LEAVES[clean(product&&product.category)]||[];
  if(!rules.some(pattern=>pattern.test(leaf))&&!evidenceBoundWholeProductLeaf(product,accepted))return {ok:false};
...
function evaluate(product,row,allProducts=[],options={}){
  ...
  if(ordinary.reason==='accessory-category'&&accepted){
    ...
    if(override.ok){accepted=override.accepted;const adjusted={...workingRow,accepted};ordinary=base.evaluate(product,adjusted,allProducts,options);if(ordinary.eligible)return {...ordinary,reason:'exact-current-ebay-au-product-accessory-parent-category',categoryOverride:{path:override.path,leaf:override.leaf},hostCompatibility:hostCompatibility.ok?{leaf:hostCompatibility.leaf}:undefined};workingRow=adjusted;}
  }
