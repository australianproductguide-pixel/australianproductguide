...
// APG eBay product-image exact identity guard v2.7.
// Extends the conservative v2.6 guard without weakening title/model/variant safety.
// Some legitimate whole products are classified by eBay beneath broad parent categories whose
// names contain "Accessories" (for example Dash Cams beneath Vehicle Parts & Accessories and
// Webcams beneath Laptop & Desktop Accessories). v2.7 permits a narrow APG-category/leaf-category
// override only for known whole-product leaves, then re-runs the complete base exact-product guard.
// A leaf that is itself Parts, Accessories, Components or Replacement remains rejected.
...
const ACCESSORY_PARENT_WHOLE_PRODUCT_LEAVES=Object.freeze({
  ...
  'power-banks':[/\bpower\s*banks?\b/i,/\bportable\s+power\s+banks?\b/i],
