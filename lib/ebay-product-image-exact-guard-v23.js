...
// APG eBay product-image exact identity guard v3.12.
// Extends v3.11 by preventing a false sibling collision when an exact target product name naturally
// contains the complete name of a shorter sibling (for example Fire TV Stick 4K Max contains
// Fire TV Stick 4K). This exception applies only after the target's own complete branded/core phrase
// has already matched. True sibling-only listings still fail closed. All item, model, accessory,
// bundle, category, condition, voltage, AUD-price, image-host, URL and active-listing controls remain.
...
function namedIdentityCheck(product,accepted,allProducts=[],options={}){
  ...
  for(const sibling of Array.isArray(allProducts)?allProducts:[]){
    ...
    // If the fully matched target itself contains the shorter sibling phrase, that phrase is not
    // independent evidence for the sibling. Example: Fire TV Stick 4K Max contains Fire TV Stick 4K.
...
function evaluate(product,row,allProducts=[],options={}){
  ...
  let named=namedIdentityCheck(product,accepted,allProducts,{allowMeaningfulModel:directNamed});
  ...
  return {eligible:true,reason:directSiblingSafe?'exact-current-ebay-au-verified-direct-sibling-safe-product':(directNamed?'exact-current-ebay-au-verified-direct-named-product':(hostCompatibility.ok?'exact-current-ebay-au-named-product-host-compatibility-title':'exact-current-ebay-au-named-product')),model:named,hostCompatibility:hostCompatibility.ok?{leaf:hostCompatibility.leaf}:undefined,directNamedIdentity:directNamed,directSiblingSafeIdentity:directSiblingSafe};
}
module.exports={VERSION,MODEL_GATE_FAILURES,GENERIC_NAME_WORDS,EXTRA_ACCESSORY_TITLE_PATTERNS,BUNDLE_COMPONENT_RULES,ACCESSORY_PARENT_WHOLE_PRODUCT_LEAVES,EVIDENCE_BOUND_WHOLE_PRODUCT_LEAVES,EVIDENCE_BOUND_CANONICAL_LEAVES,UNSAFE_ACCESSORY_LEAF,HOST_COMPATIBILITY_WHOLE_PRODUCT_RULES,VERIFIED_DIRECT_NAMED_IDENTITY_ITEMS,VERIFIED_DIRECT_SIBLING_SAFE_ITEMS,VERIFIED_INCLUDED_RETAIL_BUNDLES,phraseInTitle,extraAccessoryTitle,unexpectedBundleComponent,verifiedIncludedRetailBundle,categoryPath,categoryLeaf,evidenceBoundWholeProductLeaf,safeWholeProductLeaf,accessoryParentWholeProductOverride,hostCompatibilityWholeProductOverride,verifiedDirectNamedIdentity,verifiedDirectSiblingSafeIdentity,brandlessName,meaningfulModelTokens,nameCoreWords,nameCore,namedIdentityCheck,evaluate};