...
// APG eBay product-image exact identity guard v3.0.
// Extends the conservative v2.9 guard for one evidence-backed whole-product listing whose title
// contains host compatibility wording. The exception is product-scoped, requires the exact APG
// whole-product identity plus a safe USB-hub marketplace leaf, and removes only the trailing
// "compatible with ..." clause for internal guard evaluation. All category/model/sibling/bundle/
// pack/voltage/condition/URL/AUD-price/state checks are still run. Recommendation weight = zero.
...
function evaluate(product,row,allProducts=[],options={}){
  ...
  return {eligible:true,reason:hostCompatibility.ok?'exact-current-ebay-au-named-product-host-compatibility-title':'exact-current-ebay-au-named-product',model:named,hostCompatibility:hostCompatibility.ok?{leaf:hostCompatibility.leaf}:undefined};
}
module.exports={VERSION,MODEL_GATE_FAILURES,GENERIC_NAME_WORDS,EXTRA_ACCESSORY_TITLE_PATTERNS,BUNDLE_COMPONENT_RULES,ACCESSORY_PARENT_WHOLE_PRODUCT_LEAVES,UNSAFE_ACCESSORY_LEAF,HOST_COMPATIBILITY_WHOLE_PRODUCT_RULES,phraseInTitle,extraAccessoryTitle,unexpectedBundleComponent,categoryPath,categoryLeaf,safeWholeProductLeaf,accessoryParentWholeProductOverride,hostCompatibilityWholeProductOverride,brandlessName,meaningfulModelTokens,nameCoreWords,nameCore,namedIdentityCheck,evaluate};
