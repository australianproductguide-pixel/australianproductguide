...
// APG eBay product-image exact identity guard v3.3.
// Extends v3.2 with one evidence-bound category exception for the exact Anker 737 Power Bank
// official eBay AU listing. eBay places that whole product in the generic leaf "Batteries"; the
// exception is therefore restricted to the maintained APG slug, exact current item ID, expected
// power-bank category and exact identity phrases. All ordinary title/model/brand/condition/AUD,
// sibling, bundle, URL and active-listing controls still run. Recommendation weight remains zero.
...
const EVIDENCE_BOUND_WHOLE_PRODUCT_LEAVES=Object.freeze({
  'anker-737-power-bank-24000mah-140w':Object.freeze({
    category:'power-banks',
    itemId:'v1|404283093095|0',
    leaves:[/^batteries$/i],
    identityPhrases:['anker 737 power bank','24,000mah','140w']
  })
});
...
function evidenceBoundWholeProductLeaf(product,accepted){
  const rule=EVIDENCE_BOUND_WHOLE_PRODUCT_LEAVES[clean(product&&product.slug)],leaf=categoryLeaf(accepted),title=clean(accepted&&accepted.title);
  if(!rule||clean(product&&product.category)!==rule.category||clean(accepted&&accepted.itemId)!==rule.itemId||!leaf||UNSAFE_ACCESSORY_LEAF.test(leaf))return false;
