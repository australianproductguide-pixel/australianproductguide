...
// APG eBay product-image exact identity guard v3.14.
// Extends v3.13 with a narrowly item-bound structured-evidence identity path for listings whose
// maintained APG product name is more specific than the seller title. The first rule covers UGREEN
// Revodok 107 only when the exact eBay AU item, UGREEN category, structured SKU 60515 and the title's
// UGREEN + Revodok + 7-in-1 USB-C hub identity all agree. Generic accessory, sibling, bundle, category,
// condition, voltage, AUD-price, eBay-image-host, eBay AU URL and active-listing controls remain
// fail-closed. v3.13's wall/shelf/desk/ceiling-mount protection is unchanged.
...
function verifiedEvidenceIdentity(product,accepted){
  ...
  const evidence=base.evidenceModels(accepted),matched=evidence.find(value=>rule.evidence.some(expected=>compact(value)===compact(expected)));
  if(!matched)return {ok:false};
  return {ok:true,itemId,evidence:matched,proof:'item-bound-structured-model-evidence'};
}
...
function evaluate(product,row,allProducts=[],options={}){
  ...
  let named;
  if(evidenceIdentity.ok)named={ok:true,reason:'verified-item-bound-evidence-identity',evidence:evidenceIdentity.evidence};
  else{
