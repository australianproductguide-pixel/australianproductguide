...
// APG eBay product-image exact identity guard v3.25.
// Extends v3.24 by making the deliberately item-bound direct identity decision reproducible after
// persistence. The discovery-only searchKind marker is not stored in the governed image registry,
// so a persisted row may use the direct identity exception only when the exact allowlisted eBay item
// still independently passes the existing whole-product category/host-compatibility or evidence-bound
// checks. No arbitrary direct item, model, accessory, sibling, bundle or compatibility relaxation is
// introduced. Dyson PC1 and all prior evidence-bound rules remain unchanged.
