...
function inject(html,url){
  ...
  // Product pages already receive the richer v47 evidence/retailer/imagery continuity panel.
  // Do not stack a second generic decision rail underneath it: one clear next-step surface
  // is a deliberate premium UX requirement. v108 remains the owner on Search, Compare,
  // Decision Lab and category/guide journeys.
  if(/^\/products\/[a-z0-9-]+\/$/.test(url.pathname)&&out.includes('Connected decision intelligence'))return out;
