...
// APG Homepage Situation Images v71.
...
// suitable governed product image has not yet been selected.
...
const HOMEPAGE_SITUATION_IMAGES_VERSION='71.0';
...
function enhanceSituationCards(html){
  ...
  return text.replace(CARD_RE,card=>{
    ...
    const img=`<img class="apg-v70-situation-image" data-apg-situation-image="${escAttr(slug)}" data-apg-image-kind="${escAttr(image.kind)}"${productSlug} src="${escAttr(image.src)}" alt="" loading="lazy" decoding="async">`;
...
Object.assign(handler,downstream,{HOMEPAGE_SITUATION_IMAGES_VERSION,CSS_PATH,CSS,CARD_RE,CATEGORY_HREF_RE,imageForCategory,enhanceSituationCards,injectHomepageSituationAssets,transformHomepage});
