...
function exactOffers(product){
  const rows=[...(Array.isArray(product?.retailers)?product.retailers:[]),...(Array.isArray(product?.offers)?product.offers:[])],seen=new Set();
  return rows.filter(x=>{
    if(!x||!x.retailer)return false;
    const href=x.affiliateUrl||x.url;if(!href)return false;
    const key=`${x.retailer}|${href}`;if(seen.has(key))return false;seen.add(key);
    const status=String(x.amazonMatchStatus||'').toUpperCase(),model=String(x.amazonModelMatch||'').toLowerCase();
    return x.exactModel===true||status==='EXACT_VERIFIED'||model==='exact';
  });
}
...
module.exports={VERSION,RELEASE,CHECKED,snapshot,imagerySnapshot,retailerSnapshot,scoutEvaluation,exactOffers};
