...
function imagerySnapshot(){
  ...
  return {verified,pending,needsReview,unavailable,invalid,total:products.length,coveragePct:products.length?Math.round(verified/products.length*1000)/10:0,priority};
}
function retailerSnapshot(){
  ...
  return {productsWithExactOffers:withOffers.length,exactOfferCount:offers.length,verifiedRetailers:retailers.length,retailers,independentRetailerOfferCount:independent.length,byCategory};
}
...
function snapshot(){
  ...
  return {
    ...
    releaseGate:{pass:!!q.releaseGate?.pass&&scout.pass&&imagery.invalid===0&&retailers.exactOfferCount>=15,requirements:['Decision Intelligence quality gate passes','Scout benchmark scenarios pass','No invalid verified image records','At least 15 verified exact Australian retailer/manufacturer offer destinations','Affiliate recommendation weight remains zero']}
  };
}
