...
// APG eBay product-image continuity v3.8.
...
// v3.8 removes a fragile embedded regular expression that serialised into invalid browser
// JavaScript and replaces it with an exact trusted-origin prefix check.
  ...
  function handler(req,res){
    ...
    res.end=function(chunk,encoding,cb){
      ...
      inject(body,path).then(result=>{
        if(productRoute){patchResponseCsp(res);if(typeof res.setHeader==='function'){res.setHeader('X-APG-eBay-Product-Image-Result',safeResultHeader(result&&result.reason));res.setHeader('X-APG-eBay-Product-Image-Progressive','v1.1');}}
