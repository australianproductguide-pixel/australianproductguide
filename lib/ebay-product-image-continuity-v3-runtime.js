...
// APG eBay product-image continuity v3.6.
...
// v3.6 restores the product-page registry read to APG's standard 1.2-second bounded Supabase
// budget after Production diagnostics proved the prior 850 ms override could falsely miss a valid
// governed state during the fully assembled page workload. Failure still serves the normal APG
// fallback and never triggers a live eBay request.
  ...
  function handler(req,res){
    ...
    res.end=function(chunk,encoding,cb){
      ...
      inject(body,path).then(result=>{
        if(productRoute&&typeof res.setHeader==='function')res.setHeader('X-APG-eBay-Product-Image-Result',safeResultHeader(result&&result.reason));
...
module.exports={VERSION,STYLE_HREF,EBAY_IMAGE_ORIGIN,REFRESH_TARGET_MS,MAX_DISPLAY_AGE_MS,STATE_CACHE_TTL_MS,PRODUCT_LOOKUP_TIMEOUT_MS,PRODUCT_MAP,stateCache,slugForPath,productForSlug,ageMs,displayFresh,stateToMapping,pilotFallbackMapping,completeMapping,toGuardRow,guardEligible,currentMapping,ensureStyle,heroMarkup,replaceHeroPlaceholder,reconcileImageCopy,withEbayImageCsp,safeResultHeader,inject,wrap,install};
