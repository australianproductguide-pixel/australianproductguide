...
// APG eBay product-image continuity v3.3.
// Public rendering remains zero-eBay-network. Exact retailer imagery is read only from APG's
// governed Supabase image-state registry. Owner policy from 1 Sep 2026: a verified exact-model
// image remains displayable until it is explicitly replaced, revoked or fails an identity/safety
// guard. Age alone does not hide an already verified image. Refresh/discovery remains a separate
// bounded worker concern and never influences recommendation order.
...
function reconcileImageCopy(html){return String(html||'').replace('Brand identity placeholder via APG governed brand resolver','Current eBay Australia listing image · exact model verified by APG').replace('Product photography: awaiting an authorised exact-product source','Product photography: verified exact-model retailer image').replace('Approved exact-product image not yet available','Verified exact-model retailer image via eBay Australia').replace('Genuine product photography awaiting an authorised exact-product source','Retailer-supplied image remains until APG explicitly refreshes, replaces or revokes it');}
