...
const {enhance:enhanceFooterV8,footerV8Css}=require('./footer-v8');
...
function renderV7(html,path,u){
  const platform=enhancePlatformV7(enhancePlatformV5(enhancePlatformV3(enhanceDecisionShell(html),path,u),path,u),path,u);
  const discovered=shopping.enhance(platform,path,u);
  return shoppingShell.enhance(enhanceFooterV8(discovered));
}
module.exports=(req,res)=>{
  ...
  if(path==='/api/decision'||path==='/api/decision/'){
    ...
    const result=decision.publicDecision(u.searchParams.get('q')||'',{category:u.searchParams.get('category')||'',budget:u.searchParams.get('budget')||'',brand:u.searchParams.get('brand')||''});
  ...
  if(path==='/api/catalogue.csv'){
    if(readOnly(req,res))return;
    return send(req,res,200,'text/csv; charset=utf-8',catalogueCsv(),{'Cache-Control':'no-store','X-Robots-Tag':'noindex','Content-Disposition':'inline; filename="apg-platform-v7-catalogue.csv"'});
  }
  if(path==='/api/catalogue.json'){
    if(readOnly(req,res))return;
    return send(req,res,200,'application/json; charset=utf-8',JSON.stringify(catalogueJson()),{'Cache-Control':'no-store','X-Robots-Tag':'noindex'});
  }
  if(path==='/assets/site.css')return send(req,res,200,'text/css; charset=utf-8',css+decisionCss+platformV3Css+platformV5Css+platformV6Css+platformV7Css+platformV7RichCss+footerV8Css+shopping.css+shoppingShell.css,{'Cache-Control':'public, max-age=3600'});
  ...
  if(path==='/sitemap.xml'){
    const o=origin(req),body='<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">'+indexableRoutes.map(x=>`<url><loc>${xmlEscape(o+x)}</loc></url>`).join('')+'</urlset>';
    return send(req,res,200,'application/xml; charset=utf-8',body,{'Cache-Control':'public, max-age=900'});
  }
  if(path!=='/'&&!path.endsWith('/')){res.statusCode=308;res.setHeader('Location',path+'/'+u.search);return res.end();}
  let html=null;
  if(path==='/')html=pages.home(req);
  else if(path==='/deals/')html=shopping.dealsPage(req);
