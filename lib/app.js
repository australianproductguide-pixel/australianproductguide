...
function renderV5(html,path,u){return enhancePlatformV5(enhancePlatformV3(enhanceDecisionShell(html),path,u),path,u);}
module.exports=(req,res)=>{
  ...
  if(path==='/api/catalogue.csv'){
    if(readOnly(req,res))return;
    return send(req,res,200,'text/csv; charset=utf-8',catalogueCsv(),{'Cache-Control':'public, max-age=3600','X-Robots-Tag':'noindex','Content-Disposition':'inline; filename="apg-platform-v6-catalogue.csv"'});
  }
  ...
  if(path==='/assets/site.css')return send(req,res,200,'text/css; charset=utf-8',css+decisionCss+platformV3Css+platformV5Css+platformV6Css,{'Cache-Control':'public, max-age=3600'});
  ...
  if(html){html=enhanceAccountPolicy(html,path);return send(req,res,200,'text/html; charset=utf-8',renderV5(html,path,u));}
  return send(req,res,404,'text/html; charset=utf-8',renderV5(pages.notFound(req),path,u),{'X-Robots-Tag':'noindex,follow'});
};
