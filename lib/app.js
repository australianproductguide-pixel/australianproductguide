...
function headers(res){
  ...
  res.setHeader('Content-Security-Policy',"default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self'; img-src 'self' data:; connect-src 'self'; font-src 'self'; object-src 'none'; base-uri 'self'; form-action 'self'; frame-ancestors 'self'; upgrade-insecure-requests");
}
...
module.exports=(req,res)=>{
  ...
  if(path==='/api/decision'||path==='/api/decision/'){
    if(!['GET','HEAD'].includes(req.method)){res.setHeader('Allow','GET, HEAD');return send(req,res,405,'application/json; charset=utf-8',JSON.stringify({error:'Method not allowed'}),{'Cache-Control':'private, no-store','X-Robots-Tag':'noindex'});}
  ...
  if(path==='/assets/site.css')return send(req,res,200,'text/css; charset=utf-8',css+decisionCss+platformV3Css,{'Cache-Control':'public, max-age=3600'});
  if(path==='/assets/app.js')return send(req,res,200,'application/javascript; charset=utf-8',clientJs+decisionClientJs+platformV3ClientJs,{'Cache-Control':'public, max-age=3600'});
  ...
  if(path==='/sitemap.xml'){
    const o=origin(req),body='<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">'+indexableRoutes.map(x=>`<url><loc>${xmlEscape(o+x)}</loc><lastmod>2026-08-16</lastmod></url>`).join('')+'</urlset>';
  ...
  if(html)return send(req,res,200,'text/html; charset=utf-8',enhancePlatformV3(enhanceDecisionShell(html),path,u));
  return send(req,res,404,'text/html; charset=utf-8',enhancePlatformV3(enhanceDecisionShell(pages.notFound(req)),path,u),{'X-Robots-Tag':'noindex,follow'});
};
