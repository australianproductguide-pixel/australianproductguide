...
function resultCard(r,i,cleanAvailable){
  ...
  return `<article class="decision-result ${i===0&&cleanAvailable?'decision-result-top':''}">
    ...
    <div class="decision-result-grid">${productVisual(p,i===0)}<div class="decision-result-copy">
      ...
      ${verify.length?`<div class="decision-verification"><strong>${r.eligibility==='ineligible'?'Must-have requirement conflict':'Verify before relying on this fit'}</strong><ul>${verify.slice(0,4).map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div>`:''}
...
function decisionLab(req,u){
  ...
  const body=`<section class="decision-hero"><div class="wrap decision-hero-grid"><div><p class="kicker">APG Decision Lab</p><h1>Describe the purchase. Get the shortlist and the reasoning.</h1><p class="lede">Decision Lab turns your needs, maximum budget, priorities and deal-breakers into an explainable shortlist drawn only from products APG currently maintains. Must-have requirements are respected; missing proof stays visible rather than being guessed.</p><div class="decision-principles"><span>Current APG product data</span><span>Must-haves checked first</span><span>Affiliate commission never affects ranking</span><span>No invented performance scores</span></div></div><aside class="decision-engine-card"><span class="engine-status">Explainable matching</span><strong>Reasoning you can inspect</strong><p>Match labels describe suitability to your stated needs. They are not laboratory scores or claims of hands-on testing.</p><a href="/methodology/">How recommendations work →</a></aside></div></section>
