...
const {rankDecision,describeIntent,human}=require('./decision-engine');
const {esc,layout,productVisual,pill,priceContext}=require('./ui');

const homeCrumb={name:'Home',path:'/'};
const selected=(a,b)=>a===b?' selected':'';
const tone=label=>label==='Strong match'?'strong':label==='Good match'?'good':label==='Compromise match'?'compromise':'explore';

function resultCard(r,i){
  const p=r.p;
  return `<article class="decision-result ${i===0?'decision-result-top':''}">
    <div class="decision-rank"><span>${i===0?'Best fit':'Alternative '+(i+1)}</span><strong class="decision-match ${tone(r.matchLabel)}">${esc(r.matchLabel)}</strong></div>
    <div class="decision-result-grid">${productVisual(p,i===0)}<div class="decision-result-copy">
      ...
      <p>${esc(p.summary)}</p>
      <div class="decision-coverage">${esc(r.coverage)}</div>
...
function decisionLab(req,u){
  ...
  ${hasInput?`<section class="section soft-section full-bleed"><div class="wrap"><div class="decision-summary"><div><p class="kicker">What APG understood</p><h2>Your decision profile</h2><p>The engine only rewards evidence signals that exist in the maintained dataset. Missing or conflicting evidence is shown rather than guessed.</p></div><div class="decision-signals">${signals.length?signals.map(x=>pill(x,'good')).join(''):'<span class="pill">No specific priority signals detected</span>'}</div><div class="decision-share"><button type="button" class="button secondary compact" data-copy-decision>Copy this decision link</button><span data-copy-status aria-live="polite"></span></div></div></div></section>
  <section class="section"><div class="wrap"><div class="section-head"><div><p class="kicker">Explainable shortlist</p><h2>${intent.category?`Best fits in ${esc(intent.category.label)}`:'Best fits across the maintained catalogue'}</h2><p>Ordered by fit to the inputs above, not by affiliate availability or commission.</p></div><a class="text-link" href="/my-apg/">Open My APG workspace →</a></div><div class="decision-results">${top.map(resultCard).join('')}</div><div class="decision-disclaimer"><strong>Interpretation boundary</strong><p>A strong match means the product aligns well with the explicit needs APG can substantiate. It does not mean APG has hands-on tested the product or that it will be best for every buyer.</p></div></div></section>`:
  `<section class="section soft-section full-bleed"><div class="wrap decision-onboarding"><div><p class="kicker">More powerful than a generic “best” list</p><h2>Decision Lab works from your constraints backwards.</h2></div><div class="grid four"><article class="feature-card"><span class="step-number">01</span><h3>Describe the situation</h3><p>Use natural language instead of translating your life into specification filters.</p></article><article class="feature-card"><span class="step-number">02</span><h3>Expose deal-breakers</h3><p>“Avoid premium”, budget ceilings and other explicit negatives can reduce fit instead of being ignored.</p></article><article class="feature-card"><span class="step-number">03</span><h3>Inspect the reasoning</h3><p>See why a product matches, what remains uncertain and the trade-off you are accepting.</p></article><article class="feature-card"><span class="step-number">04</span><h3>Compare and verify</h3><p>Move shortlisted products into comparison, product evidence and verified retailer pathways.</p></article></div></div></section>`}
  <section class="section"><div class="wrap"><div id="decisionHistory" hidden></div></div></section>`;
  return layout(req,{title:'APG Decision Lab | Explainable Product Matching',description:'Describe what you need and get an explainable shortlist from Australian Product Guide’s maintained product data.',path,body,noindex:true,crumbs:[homeCrumb,{name:'Decision Lab',path}],bodyData:`data-decision-query="${esc(q)}" data-decision-url="${esc(u.pathname+u.search)}"`});
}
...
module.exports={decisionLab,workspace};
