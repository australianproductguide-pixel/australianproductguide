...
const mobile=q('[data-mobile-toggle]');function setMobile(open){if(!mobile)return;const nav=q('#mobileNav');mobile.setAttribute('aria-expanded',String(open));if(nav)nav.hidden=!open;document.documentElement.classList.toggle('mobile-nav-open',open);if(open)setTimeout(()=>q('[data-site-search]',nav)?.focus(),0)}
...
function renderSuggestions(input,box,items){const value=input.value.trim(),recent=read(searchKey).slice(0,4);let list=value?matches(items,value):[];if(!value&&recent.length)list=recent.map(v=>({type:'recent',label:v,meta:'Recent search on this device',url:'/search/?q='+encodeURIComponent(v)}));if(!list.length&&value)list=[{type:'guide',label:'Search all maintained products',meta:'See full results and recovery suggestions',url:'/search/?q='+encodeURIComponent(value)}];if(!list.length){box.hidden=true;box.innerHTML='';input.setAttribute('aria-expanded','false');return}const grouped={};for(const item of list)(grouped[item.type]||(grouped[item.type]=[])).push(item);let i=0;box.innerHTML=Object.entries(grouped).map(([type,group])=>'<div class="suggest-group"><span class="suggest-label">'+safe(typeLabel(type))+'</span>'+group.map(item=>{const id=box.id+'Item'+i++;return '<a id="'+id+'" role="option" class="suggest-item" href="'+item.url+'"><span class="suggest-thumb type-'+safe(item.type)+'">'+thumb(item)+'</span><span><strong>'+safe(item.label)+'</strong><small>'+safe(item.meta||'')+'</small></span><span aria-hidden="true">→</span></a>'}).join('')+'</div>').join('');box.hidden=false;input.setAttribute('aria-expanded','true');input.removeAttribute('aria-activedescendant');}
qa('[data-search-shell]').forEach(shell=>{const input=q('[data-site-search]',shell),box=q('[data-search-suggestions]',shell);if(!input||!box)return;let active=-1;const links=()=>qa('.suggest-item',box);const setActive=n=>{const all=links();if(!all.length)return;active=(n+all.length)%all.length;all.forEach((a,i)=>a.classList.toggle('active',i===active));all[active].scrollIntoView({block:'nearest'});input.setAttribute('aria-activedescendant',all[active].id)};getIndex().then(items=>{
 ...
 shell.addEventListener('submit',()=>{const v=input.value.trim();if(v)write(searchKey,[v,...read(searchKey).filter(x=>x!==v)].slice(0,6))});
});});
