...
qa('[data-search-shell]').forEach((shell,idx)=>{const input=q('[data-site-search]',shell),box=q('[data-search-suggestions]',shell);if(!input||!box)return;const inputId='apgSearch'+idx,boxId='apgSearchSuggestions'+idx;input.id=inputId;box.id=boxId;input.setAttribute('aria-controls',boxId);const label=q('label',shell);if(label)label.setAttribute('for',inputId);getIndex().then(items=>{input.addEventListener('input',()=>renderSuggestions(shell,input,box,items));input.addEventListener('focus',()=>renderSuggestions(shell,input,box,items));shell.addEventListener('submit',()=>{const v=input.value.trim();if(v){const recent=[v,...read(searchKey).filter(x=>x!==v)].slice(0,6);localStorage.setItem(searchKey,JSON.stringify(recent));}});});});
