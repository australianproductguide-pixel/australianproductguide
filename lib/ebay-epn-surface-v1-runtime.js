...
function enhanceEbayAnchor(html,row){
  ...
  return String(html).replace(pattern,(full,attrs,inner)=>{
    ...
    let body=inner;
    body=body.replace(/<span class="apg112-retailer-status">[\s\S]*?<\/span>/i,`<span class="apg112-retailer-status">${status}</span>`);
    body=body.replace(/<small>[\s\S]*?<\/small>/i,`<small>${sub}</small>`);
