...
function scheduleOutcomeFocus(main){
 ...
 // Do not synchronously smooth-scroll a freshly imported result tree. On some
 // Chromium result layouts that can block the renderer after the result has
 // already committed. Accessibility focus is deferred and does not move scroll.
 setTimeout(()=>{
...
function commitOutcome(outcome,finalUrl,push){
 ...
 const state=outcome.result?'success':outcome.zero?'no-results':'server-recovery';
