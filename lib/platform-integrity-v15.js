...
function replaceGenericProductArt(card,slug){if(!slug||slug==='wireless-headphones')return card;return card.replace(/<div class="product-art art-headphones"><span class="art-halo"><\/span><span class="category-icon (normal|large)" aria-hidden="true"><svg[\s\S]*?<\/svg><\/span><span class="art-model">/g,(_,size)=>fallbackVisual(slug,size));}
function fixProductVisuals(out,path){
  ...
  // Custom SSR surfaces such as Decision Intelligence v4 can render the shared
  // product card before legacy decorators add data-product-category. Recover the
  // maintained category from the canonical product link instead of allowing the
  // historical headphones fallback artwork to leak across categories.
  out=out.replace(/<article class="product-card[^"]*"[\s\S]*?<\/article>/g,card=>{const m=card.match(/href="\/products\/([^/]+)\//);const p=m?PRODUCT_BY_SLUG.get(m[1]):null;return p?replaceGenericProductArt(card,p.category):card;});
