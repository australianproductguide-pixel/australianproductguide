...
function fixProductVisuals(out,path){
  ...
  return out.replace(/<div class="product-art art-headphones"><span class="art-halo"><\/span><span class="category-icon (normal|large)" aria-hidden="true"><svg[\s\S]*?<\/svg><\/span><span class="art-model">/g,(_,size)=>`<div class="product-art art-v15-category" data-v15-category="${slug}"><span class="art-halo"></span>${semanticIcon(slug,size)}<span class="art-model">`);
}
...
function decisionIndexing(out,path,search){if(path==='/decision-lab/'&&!search)out=out.replace('<meta name="robots" content="noindex,follow">','');return out;}
...
function transform(html,url){const path=url.pathname,search=url.search;let out=String(html||'');out=fixHeaderCompare(out);out=fixProductVisuals(out,path);out=improveDirectoryIcons(out,path);out=compareDirectoryTools(out,path);out=decisionIndexing(out,path,search);out=assets(out);return out;}
