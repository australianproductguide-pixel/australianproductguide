...
const BRAND_INDEX_LOGOS_VERSION='62.0';
...
function enrichBrandCard(card){
  ...
  return card.replace(
    /<span\b([^>]*class="[^"]*\bbrand-card-mark\b[^"]*"[^>]*)>([\s\S]*?)<\/span>/i,
    (whole,attrs,fallback)=>`<span${attrs}><span class="brand-card-fallback">${fallback}</span><img class="brand-card-logo" src="${esc(brandMarkUrl(slug))}" alt="" width="40" height="40" loading="lazy" decoding="async" onerror="this.hidden=true"></span>`
  );
}

const CSS=`<style id="apg-brand-index-logos-v62">
body[data-apg-brand-index-logos="v62"] .brand-card-mark{position:relative!important;display:grid!important;place-items:center!important;overflow:hidden!important;width:54px!important;height:54px!important;min-width:54px!important;border:1px solid #e2e8f0!important;border-radius:15px!important;background:#fff!important;color:#2563eb!important;box-shadow:0 3px 12px rgba(15,23,42,.05)!important}
...
body[data-apg-brand-index-logos="v62"] .brand-card-logo{position:absolute;inset:7px;display:block;width:40px;height:40px;object-fit:contain;object-position:center;background:#fff}
body[data-apg-brand-index-logos="v62"] .brand-card:hover .brand-card-mark{border-color:#bfdbfe!important;box-shadow:0 5px 16px rgba(37,99,235,.09)!important}
@media (max-width:640px){body[data-apg-brand-index-logos="v62"] .brand-card-mark{width:50px!important;height:50px!important;min-width:50px!important;border-radius:14px!important}body[data-apg-brand-index-logos="v62"] .brand-card-logo{inset:6px;width:38px;height:38px}}
@media (prefers-reduced-motion:reduce){body[data-apg-brand-index-logos="v62"] .brand-card-mark{transition:none!important}}
</style>`;

function inject(html,path){
  let out=String(html||'');
  if(path!=='/brands/'||out.includes('apg-brand-index-logos-v62'))return out;
  out=out.replace(/<a\b[^>]*class="[^"]*\bbrand-card\b[^"]*"[^>]*>[\s\S]*?<\/a>/gi,enrichBrandCard);
