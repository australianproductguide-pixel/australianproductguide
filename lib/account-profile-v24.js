...
const JOURNEY_JS='/assets/account-journey-v241.js?v=24.1';
...
async function handleProfileApi(req,res,path){
  ...
  try{
    ...
    if(path==='/api/account/resend-confirmation'&&req.method==='POST'){
      ...
      if(email){
        const resend=await supabase(`/auth/v1/resend?redirect_to=${encodeURIComponent(PRIMARY_ORIGIN+'/my-apg/')}`,{method:'POST',body:{type:'signup',email}});
        if(resend.status===429)return sendJson(res,429,{error:'Please wait a moment before requesting another confirmation email.'});
      }
...
function injectProfileAssets(html){
  ...
  if(!out.includes(JOURNEY_JS))out=out.replace('</body>',`<script src="${JOURNEY_JS}" defer></script></body>`);
  return out;
}
