...
function counter(value){
  ...
  const digits=String(value).split('').map(d=>`<span aria-hidden="true">${esc(d)}</span>`).join('');
  return `<span class="apg-counter-v20" aria-hidden="true">${digits}</span><span class="sr-only">${text}</span>`;
}
...
function reframeHomepage(out,path){
  ...
  out=out.replace('Affiliate availability, retailer participation and commission contribute zero suitability points.','Affiliate availability, retailer participation and commission never improve a product\'s recommendation or ranking.');
  return out;
}
