...
// APG eBay Australia category shopping presentation v134.0.
...
// asset, while the six owner-supplied refurbished/promotion tiles now use the exact creatives
// supplied for those destinations. Retailer promotional assets are not APG product evidence or
// recommendation inputs and do not affect product rankings.
const categoryRegistry=require('../data/ebay-official-creatives-v121');

const VERSION='134.0';
const STYLE_HREF='/assets/ebay-category-shopping-v133.css';
const REFURBISHED_CREATIVE_VERSION='134.0';
...
function ensureRefurbishedStyle(html){
  const out=String(html||'');
  if(!out.includes('apg-ebay-refurb-v134-art')||out.includes('id="apgEbayRefurbV134Style"'))return out;
  const css=`<style id="apgEbayRefurbV134Style" data-ebay-refurbished-creatives="v${REFURBISHED_CREATIVE_VERSION}">
.apg-ebay-v11 .apg-ebay-refurb-v134-art{position:relative!important;inset:auto!important;display:block!important;width:100%!important;max-width:none!important;height:auto!important;min-height:0!important;padding:0!important;border:0!important;border-radius:16px!important;overflow:hidden!important;background:transparent!important;box-shadow:none!important;line-height:0!important}
.apg-ebay-v11 .apg-ebay-refurb-v134-image{display:block!important;width:100%!important;height:auto!important;aspect-ratio:900/212!important;object-fit:contain!important;border-radius:16px!important}
.apg-ebay-v11 .apg-amz-v41-stack .apg-amz-v41-card:has(.apg-ebay-refurb-v134-art){grid-template-columns:minmax(160px,180px) minmax(0,1fr)!important;column-gap:16px!important;align-items:center!important}
@media(max-width:620px){.apg-ebay-v11 .apg-amz-v41-stack .apg-amz-v41-card:has(.apg-ebay-refurb-v134-art){grid-template-columns:1fr!important}.apg-ebay-v11 .apg-ebay-refurb-v134-art{width:100%!important}}
</style>`;
  return out.replace(/<\/head>/i,`${css}</head>`);
}
...
function inject(html,path){
  ...
  // Replace only the imagery inside the existing six refurbished/promotion cards. Titles,
  // destinations, affiliate disclosures, recommendation weighting and decision logic remain
  // untouched.
...
module.exports={VERSION,STYLE_HREF,REFURBISHED_CREATIVE_VERSION,REFURBISHED_CREATIVES,card,section,ensureStyle,ensureRefurbishedStyle,applyRefurbishedCreatives,inject,wrap};
