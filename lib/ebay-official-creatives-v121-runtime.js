...
function inject(html,path){
  ...
  const block=section(rows,{primary});
...
function wrap(downstream){
  ...
  function handler(req,res){
    ...
    res.end=function(chunk,encoding,cb){
      ...
      const body=Buffer.concat(chunks).toString('utf8');const isHtml=/text\/html/i.test(contentType())||/<html|<!doctype/i.test(body);const next=isHtml?inject(body,path):body;
...
module.exports={VERSION,STYLE_HREF,card,promotionCard,section,ensureStyle,replaceLegacyDiscovery,inject,wrap};
