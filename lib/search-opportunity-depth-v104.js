...
function patchAbout(html){
  let out=html.replace('Reviewed 23 August 2026','Reviewed 25 August 2026');
  if(out.includes('id="search-feedback-loop"'))return out;
  const section=`<h2 id="search-feedback-loop">Decision depth and search feedback</h2><p>APG is deliberately prioritising depth over mass page creation. Six high-intent decision areas — televisions, laptops, washing machines, coffee machines, robot vacuums and smartphones — now carry an additional decision-first layer across category, buying-guide and comparison journeys. The next category expansions should follow demonstrated Australian search demand and evidence readiness rather than catalogue volume targets.</p><p>Google Search Console is connected as APG's first-party source for query and page performance. Search-demand signals inform prioritisation, but they do not change product suitability scoring or give affiliate-linked products any recommendation advantage.</p>`;
  return insertBefore(out,'<h2 id="research">How the research is produced</h2>',section);
}
function patchUpdates(html){
  let out=html.replace('Reviewed 23 August 2026','Reviewed 25 August 2026');
  if(out.includes('id="search-depth-25-aug"'))return out;
  const section=`<h2 id="search-depth-25-aug">25 August 2026 — High-intent decision depth and search feedback loop</h2><p>APG deepened six existing high-intent decision areas — televisions, laptops, washing machines, coffee machines, robot vacuums and smartphones — without creating new catalogue pages simply to increase URL count. Category hubs, buying guides and curated head-to-heads now surface stronger decision gates, comparison questions, purchase-verification checks and category-specific shortcuts to avoid.</p><p>The release also formalises a Search Console feedback loop: use query and page impressions to identify where Google is already testing APG, then deepen evidence and decision usefulness in those demonstrated opportunities before expanding further. Search demand remains an editorial prioritisation input only; it does not alter suitability scoring or retailer ordering.</p>`;
  return insertBefore(out,'<h2 id="trust-centre">23 August 2026 — Trust Centre and company-information reconciliation</h2>',section);
}
function transformHtml(html,path){
  let out=String(html||'');
  if(!out||!out.includes('<html'))return out;
  if(path==='/about/')out=patchAbout(out);
  else if(path==='/updates/')out=patchUpdates(out);
  ...
  if((path==='/about/'||path==='/updates/'||(info&&info.depth))&&!out.includes('name="apg-search-opportunity-depth"'))out=addMarker(out);
  return out;
}

module.exports={VERSION,REVIEWED,REVIEW_LABEL,categoryDepth,depthForPath,categorySection,guideSection,compareIndexSection,pairSection,patchAbout,patchUpdates,transformHtml};
