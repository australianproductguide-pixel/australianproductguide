...
const trust=['about','contact','methodology','editorial-standards','sources','corrections-policy','affiliate-disclosure','privacy','terms'];
...
const noindexRoutes=['/search/','/sitemap/',...pathways.filter(p=>!p.maintained).map(p=>`/categories/${p.slug}/`)];
...
if(indexableRoutes.length!==137)throw new Error(`Expected 137 indexable routes, found ${indexableRoutes.length}`);
