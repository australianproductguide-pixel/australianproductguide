...
const productIds=products.map(p=>p.id).filter(Boolean),duplicateIds=productIds.filter((s,i)=>productIds.indexOf(s)!==i);
if(duplicateIds.length)throw new Error(`Duplicate product IDs detected: ${[...new Set(duplicateIds)].join(', ')}`);
