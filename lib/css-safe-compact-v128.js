...
function compactCss(input){
  ...
  for(let index=0;index<source.length;index+=1){
    ...
    if(ch==='/'&&next==='*'){
      ...
      // CSS comments can separate otherwise adjacent tokens. Preserve that boundary while still
      // allowing punctuation-aware whitespace removal on the next loop iteration.
      pendingSpace=true;
      index=end+1;
      continue;
    }
