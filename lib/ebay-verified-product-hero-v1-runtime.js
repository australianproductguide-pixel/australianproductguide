...
// APG verified eBay product hero v1.1.
...
async function currentDetail(slug,row,{getItem=ebay.getItem,now=Date.now}={}){
  ...
  try{
    ...
    if(prior&&detailAge(prior.detail,t)<MAX_CACHE_AGE_MS)return prior.detail;
    const fallback=registryFallbackDetail(slug,row,{now:t});
    if(fallback){cache.set(slug,{at:t,detail:fallback});return fallback;}
    throw error;
  }
}
...
function reconcileImageCopy(html,{freshRegistryFallback=false}={}){
  const provenance=freshRegistryFallback?'Recent eBay Australia listing image · exact model verified within freshness window':'Current eBay Australia listing image · exact model verified at render time';
  const photography=freshRegistryFallback?'Product photography: recent exact-model image supplied by eBay Australia':'Product photography: current exact-model image supplied by eBay Australia';
  return String(html||'')
    .replace('Brand identity placeholder via APG governed brand resolver',provenance)
    .replace('Product photography: awaiting an authorised exact-product source',photography)
...
async function inject(html,pathname,options={}){
  ...
  try{
    ...
    const next=ensureStyle(reconcileImageCopy(replaced,{freshRegistryFallback:detail.freshRegistryFallback===true}));
    return {html:next,usedEbayImage:true,slug,itemId:detail.legacyItemId,imageUrl:detail.imageUrl,resolvedAt:detail.resolvedAt,freshRegistryFallback:detail.freshRegistryFallback===true};
