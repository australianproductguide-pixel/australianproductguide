...
// never spend eBay Browse API quota. Retailer participation and affiliate status contribute zero
...
function detailAge(detail,now=Date.now()){
  ...
  const t=Number(now);
  if(!Number.isFinite(resolved)||!Number.isFinite(t)||t<resolved)return Infinity;
  return t-resolved;
}
