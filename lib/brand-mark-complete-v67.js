...
// APG Brand Mark Complete v67.2
//
// Purpose: provide one governed, device-consistent identity result for every canonical
// APG brand without reopening the low-quality or misleading-image failure modes closed
// by v65/v66. A rejected asset must never leak back in from an older resolver layer.
//
...
//      - white-only official SVGs are preserved on a contrast-safe APG Navy backing
//   3. high-resolution identity explicitly declared by the brand's official domain
//   4. high-resolution cached favicon for that same governed official domain
//   5. canonical brand-name SVG as a terminal fail-closed state
//
...
const BRAND_MARK_COMPLETE_VERSION='67.2';
const BRAND_MARK_ASSET_VERSION='67.2';
...
function contrastAdaptWhiteSvg(buffer){
  if(!buffer||!svgAppearsWhiteOnly(buffer))return null;
  const original=Buffer.isBuffer(buffer)?buffer.toString('utf8'):String(buffer||'');
  if(!/<svg\b[^>]*>/i.test(original))return null;
  // Preserve the official white artwork exactly; only add a neutral APG Navy canvas
  // beneath it so the mark remains visible in APG's white logo tiles.
  const adapted=original.replace(/(<svg\b[^>]*>)/i,'$1<rect x="0" y="0" width="100%" height="100%" fill="#0F172A"/>');
  return Buffer.from(adapted,'utf8');
}
function canonicalFallbackImage(slug){
  const name=brandBySlug.get(slug)||slug;
  const svg=typeof downstream.fallbackBrandSvg==='function'
    ? downstream.fallbackBrandSvg(slug)
    : `<?xml version="1.0" encoding="UTF-8"?><svg xmlns="http://www.w3.org/2000/svg" width="320" height="96" viewBox="0 0 320 96"><rect width="320" height="96" rx="12" fill="#FFFFFF"/><text x="160" y="50" text-anchor="middle" dominant-baseline="middle" fill="#0F172A" font-family="Arial,Helvetica,sans-serif" font-size="30" font-weight="700">${String(name||'Brand').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}</text></svg>`;
  return {
    buffer:Buffer.from(svg,'utf8'),
    type:'image/svg+xml',
    quality:'text-fallback-svg',
    assetKind:'canonical-brand-name',
    resolverSource:'canonical-brand-name-fallback',
    officialReference:officialDomains[slug]?`https://${officialDomains[slug]}/`:null,
...
function assessOfficialIcon(raw,candidate){
  ...
  if(isSvg){
    ...
    if(svgAppearsWhiteOnly(raw.buffer)){
      const adapted=contrastAdaptWhiteSvg(raw.buffer);
      if(!adapted)return null;
      return {buffer:adapted,type:'image/svg+xml',source:raw.url,width:meta.width||null,height:meta.height||null,quality:'premium-vector',assetKind:'official-brand-mark-contrast-adapted',presentation:'contrast-safe-dark-backing'};
    }
...
async function resolveCompleteBrandMark(slug){
  ...
  // If v65 found the correct official SVG but its artwork is white-only, keep the
  // trademark artwork untouched and add contrast behind it instead of serving it invisibly.
  if(qualityResult&&String(qualityResult.type||'').toLowerCase().includes('svg')&&svgAppearsWhiteOnly(qualityResult.buffer)){
    const adapted=contrastAdaptWhiteSvg(qualityResult.buffer);
    if(adapted)return {...qualityResult,buffer:adapted,type:'image/svg+xml',quality:'premium-vector',assetKind:'official-brand-mark-contrast-adapted',presentation:'contrast-safe-dark-backing',officialReference:`https://${officialDomains[slug]}/`,resolverSource:'official-domain-quality-resolver-contrast-safe'};
  }
  ...
  if(declaredResult)return {...declaredResult,resolverSource:declaredResult.presentation?'official-domain-declared-identity-contrast-safe':'official-domain-declared-identity'};
  if(faviconResult)return {...faviconResult,resolverSource:'official-domain-favicon-cache'};
  // Terminal fallback: never delegate a known unresolved brand back to v65/v66.2,
  // because that could reintroduce the exact asset v67.2 rejected above.
  return canonicalFallbackImage(slug);
}
function serveImage(req,res,slug,image){
  ...
  if(image.terminalFallback)res.setHeader('X-APG-Brand-Mark-Fallback','canonical-name');
...
Object.assign(handler,downstream,{
  ...
  declaredIconCandidates,manifestCandidates,svgAppearsWhiteOnly,contrastAdaptWhiteSvg,canonicalFallbackImage,
  assessOfficialIcon,resolveDeclaredOfficialIcon,resolveOfficialDomainFavicon,resolveCompleteBrandMark,
  versionBrandMarkUrls,injectCompleteMeta
});
