...
const EARBUDS_CATEGORY_IMAGE_VERSION='72.1';
...
function refreshEarbudsImage(html){
  ...
  // Existing SEO code assumes category imagery is same-origin and prefixes APG's origin.
  // Correct that derived URL when this governed hotfix uses the verified Commons asset.
  out=out.split('https://australianproductguide.au'+IMAGE.src).join(IMAGE.src);
  out=out.replace(
...
function allowEarbudsImageSource(res){
  const current=String(res.getHeader('Content-Security-Policy')||'');
  if(!current||current.includes('https://upload.wikimedia.org'))return;
  const next=current.replace("img-src 'self'", "img-src 'self' https://upload.wikimedia.org");
  if(next!==current)res.setHeader('Content-Security-Policy',next);
}
...
Object.assign(handler,downstream,{EARBUDS_CATEGORY_IMAGE_VERSION,refreshEarbudsImage,allowEarbudsImageSource,IMAGE});
