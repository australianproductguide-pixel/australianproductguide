...
function workspaceCount(){return new Set([...read(keys.compare),...read(keys.saved)]).size;}
...
async function renderWorkspace(){
  ...
  const compare=read(keys.compare).slice(0,4),saved=read(keys.saved).slice(0,50),recent=read(keys.recent).slice(0,6),decisions=read(keys.decisions).slice(0,8);
  ...
  if(compare.length>=2)q('[data-workspace-compare] [data-workspace-content]',root).insertAdjacentHTML('beforeend','<a class="button compact" href="/compare/custom/?products='+compare.join(',')+'">Compare '+compare.length+' products</a>');
