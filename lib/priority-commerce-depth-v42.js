...
const pathnameOf=value=>{
  if(value&&typeof value==='object'&&typeof value.pathname==='string')return value.pathname;
  const raw=String(value||'/');
  try{return /^https?:\/\//i.test(raw)?new URL(raw).pathname:raw.split('?')[0]||'/';}catch{return '/';}
};
