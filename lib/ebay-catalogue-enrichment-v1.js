...
// APG eBay catalogue enrichment v1.12
...
// v1.12 recognises eBay's structured Item Model Number and Model Number aspect labels as model
// evidence. These fields are exact identity metadata, not free-text title relaxation. All existing
// brand, category, accessory, variant, condition, voltage, pricing and media gates remain unchanged.
...
function detailedModelEvidence(detail){
  return uniq([
    ...
    ...aspectValues(detail,/^(model|item\s+model\s+number|model\s+number|mpn|manufacturer\s+part\s+number)$/i)
