...
// APG eBay catalogue enrichment v1.5
...
// v1.5 improves exact-identity recall by exposing maintained specification model hints to the
// search planner, ignoring non-evidence placeholders in eBay item specifics, and distinguishing
// feature-count descriptors from genuine model identifiers. Acceptance safety remains unchanged.
...
const LOW_VOLTAGE_TOKEN_PATTERN=/\b(?:100|110|115|120|125)\s*v\b/i;
const LOW_TO_AU_WIDE_RANGE_PATTERN=/\b(?:100|110|115|120|125)\s*(?:-|–|—|\/|to)\s*(?:220|230|240)\s*v\b/i;
const AU_TO_LOW_WIDE_RANGE_PATTERN=/\b(?:220|230|240)\s*(?:-|–|—|\/|to)\s*(?:100|110|115|120|125)\s*v\b/i;
...
function materialIdentityConflict(product,title){
  if(listingLooksAccessory(title,product))return {conflict:true,reason:'accessory-or-part-language',detail:{title:clean(title)}};
