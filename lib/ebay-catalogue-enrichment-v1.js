...
// APG eBay catalogue enrichment v1.11
...
// v1.11 keeps the existing exact-product controls but fixes media selection: when eBay detail exposes
// multiple image sources, APG selects the first HTTPS i.ebayimg.com image from the exact item instead
// of allowing an unsupported catalogue host to mask a valid eBay listing/summary image. The allowed
// host is not broadened and product identity, variant, category, condition and voltage gates are unchanged.
...
const MATERIAL_MODEL_SUFFIXES=['dsl','pro','plus','max','ultra','mini','lite','se','xl','x'];
const UNINFORMATIVE_IDENTITY_VALUES=new Set([
  '','n a','na','none','unknown','unspecified','not specified','not supplied','not applicable','does not apply',
  'no brand','brandless','unbranded','generic','universal','no model','see description','as pictured'
]);
// Evidence-bound equivalences only. These do not weaken product/model matching: they only allow
// eBay's structured Brand field to use the parent brand when the APG identity uses a sub-brand.
const BRAND_EQUIVALENCE_GROUPS=Object.freeze([
  Object.freeze(['bosch','bosch professional']),
  Object.freeze(['fujifilm','fuji']),
  Object.freeze(['philips','philips hue','philips sonicare'])
]);

const UNIVERSAL_ACCESSORY_PATTERNS=[
  /\breplacement\b/i,/\bspares?\b/i,/\b(?:replacement|spare)\s+parts?\b/i,/\bparts?\s+(?:for|only)\b/i,
  /\bcompatible\s+with\b/i,/\bfits?\s+(?:for|with|only)\b/i,
...
const LOW_VOLTAGE_TOKEN_PATTERN=/\b(?:100|110|115|120|125)\s*v(?:ac)?\b/i;
const LOW_TO_AU_WIDE_RANGE_PATTERN=/\b(?:100|110|115|120|125)\s*v?(?:ac)?\s*(?:-|–|—|\/|~|to)\s*(?:220|230|240)\s*v(?:ac)?\b/i;
const AU_TO_LOW_WIDE_RANGE_PATTERN=/\b(?:220|230|240)\s*v?(?:ac)?\s*(?:-|–|—|\/|~|to)\s*(?:100|110|115|120|125)\s*v(?:ac)?\b/i;

function clean(value){return String(value==null?'':value).trim();}
function norm(value){
  return clean(value).toLowerCase().normalize('NFKD').replace(/[\u0300-\u036f]/g,'').replace(/[’']/g,'').replace(/[^a-z0-9]+/g,' ').replace(/\s+/g,' ').trim();
}
...
function exactEbayImageUrl(value){
  const raw=clean(value);
  if(!raw)return false;
  try{const u=new URL(raw);return u.protocol==='https:'&&u.hostname==='i.ebayimg.com';}catch{return false;}
}
function preferredEbayImage(...values){
  for(const value of values){const candidate=clean(value);if(candidate&&exactEbayImageUrl(candidate))return candidate;}
  return null;
}
function informativeIdentityEvidence(value){
  const key=norm(value);
  return Boolean(key)&&!UNINFORMATIVE_IDENTITY_VALUES.has(key);
}
function specModelValues(product){
  const rows=Array.isArray(product&&product.specs)?product.specs:[];
  const out=[];
  for(const row of rows){
    let label='',value='';
    if(Array.isArray(row)){label=row[0];value=row[1];}
    else if(row&&typeof row==='object'){
      label=row.label||row.name||row.key||'';
      value=row.value||row.text||row.content||'';
    }
    if(!/^(?:model(?:\s*(?:code|number|family))?|product\s*code|mpn|manufacturer\s*(?:part|product)\s*number|part\s*number|sku)$/i.test(clean(label)))continue;
    if(Array.isArray(value))out.push(...value);else out.push(value);
  }
  return uniq(out.map(clean).filter(informativeIdentityEvidence));
}
...
function isFeatureDescriptorModel(value){
  const token=compact(value).toUpperCase();
  if(!token)return false;
  return /^(?:[248]K|720P|1080P|1440P|\d+(?:ST|ND|RD|TH)(?:GEN)?|\d+IN1|\d+(?:PORTS?|PACK|PK|CAMERAS?|CAM|BASKETS?|ZONES?|PIECES?|PCS?|FUNCTIONS?|MODES?|SLICES?|CUPS?|HEADS?|SPEEDS?|STAGES?))$/.test(token);
}
...
function regionalVoltageConflict(text){
  const value=clean(text);
  if(!value)return {conflict:false};
  if(LOW_TO_AU_WIDE_RANGE_PATTERN.test(value)||AU_TO_LOW_WIDE_RANGE_PATTERN.test(value))return {conflict:false};
  const observed=value.match(LOW_VOLTAGE_TOKEN_PATTERN);
  return observed?{conflict:true,observed:clean(observed[0]),reason:'low-voltage-regional-variant'}:{conflict:false};
}
...
function brandMatch(title,brand){
  ...
  const equivalence=BRAND_EQUIVALENCE_GROUPS.find(group=>group.includes(b));
  if(equivalence&&equivalence.some(alias=>` ${t} `.includes(` ${alias} `)))return true;
  return b.includes(' ')&&compact(t).includes(compact(b));
}
...
function variantFacts(text){
  return {
    screenInches:collectNumbers(text,/(\d{2,3}(?:\.\d+)?)[\s-]*(?:["”]|inch(?:es)?\b)/ig),
...
function materialIdentityConflict(product,title,{includeAccessory=false}={}){
  if(includeAccessory&&listingLooksAccessory(title,product))return {conflict:true,reason:'accessory-or-part-language',detail:{title:clean(title)}};
  const voltage=regionalVoltageConflict(title);
  if(voltage.conflict)return {conflict:true,reason:'regional-voltage-mismatch',detail:voltage};
...
function detailedBrandEvidence(detail){
  return uniq([clean(detail&&detail.brand),...aspectValues(detail,/^brand$/i)]).filter(informativeIdentityEvidence);
}
function detailedModelEvidence(detail){
  ...
  ]).filter(informativeIdentityEvidence);
}
...
async function verifyDetailedCandidate(product,candidate){
  ...
  const detailedImage=preferredEbayImage(catalogImage,listingImage,candidate.imageUrl);
  ...
  const imageSource=detailedImage===catalogImage?'ebay-product-catalog':'ebay-listing';
...
module.exports={
  VERSION,AUTO_ACCEPT_SCORE,REVIEW_SCORE,MAX_CANDIDATES,MAX_DETAIL_CHECKS,UNINFORMATIVE_IDENTITY_VALUES,BRAND_EQUIVALENCE_GROUPS,
  norm,compact,tokens,exactEbayImageUrl,preferredEbayImage,informativeIdentityEvidence,specModelValues,productModelSource,productIdentitySource,isUnitLikeModel,isFeatureDescriptorModel,
  compoundModelTokens,modelTokens,identityTokens,brandMatch,variantFacts,materialVariantConflict,materialSuffixConflict,materialIdentityConflict,
  listingLooksAccessory,listingLooksUsed,regionalVoltageConflict,pricePlausibility,detailedBrandEvidence,detailedModelEvidence,detailedCategoryRisk,
  scoreCandidate,queryFor,verifyDetailedCandidate,enrichProduct
};