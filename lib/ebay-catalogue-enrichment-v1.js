...
// APG eBay catalogue enrichment v1.2
...
async function verifyDetailedCandidate(product,candidate){
  ...
  try{detail=await ebay.getItem(candidate.itemId,{referenceId:`apg:${product.slug}:verify`});}
    ...
    return {ok:false,review:true,reason:'detail-verification-error',code:error&&error.code?String(error.code):'EBAY_DETAIL_ERROR'};
  }
...
module.exports={
  VERSION,AUTO_ACCEPT_SCORE,REVIEW_SCORE,MAX_CANDIDATES,MAX_DETAIL_CHECKS,norm,compact,tokens,modelTokens,identityTokens,
  listingLooksAccessory,listingLooksUsed,pricePlausibility,detailedBrandEvidence,detailedModelEvidence,detailedCategoryRisk,
