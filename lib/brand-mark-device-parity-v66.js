...
function removeObsoleteBrandImageErrorHandlers(html){
  return String(html||'').replace(/\s+onerror=(["'])this\.hidden=true\1/gi,'');
}
...
function handler(req,res){
  ...
  res.end=(body,...args)=>{
    ...
    if(req.method!=='HEAD'&&res.statusCode===200&&(typeof body==='string'||Buffer.isBuffer(body))&&type.startsWith('text/html')){
      ...
      next=removeObsoleteBrandImageErrorHandlers(next);
...
Object.assign(handler,downstream,{
  ...
  removeObsoleteBrandImageErrorHandlers,
