...
function enrichSemanticSurfaces(html){
  // Comparison/product visual modules pre-date v7 and can exist outside .product-card.
  // Upgrade any module that already declares its product category, preserving copy and links.
  html=html.replace(/<div class="product-visual([^"]*)"([^>]*)data-product-category="([^"]+)"([^>]*)>([\s\S]*?)(?=<div class="visual-copy">)/g,(block,classes,before,category,after,inside)=>{
    if(!categoryMeta[category])return block;
    let upgraded=inside;
    upgraded=upgraded.replace(/<span class="category-icon[^>]*>[\s\S]*?<\/span>/,categoryScene(category,{compact:true}));
    upgraded=upgraded.replace(/class="product-art art-[^"]+"(?: data-v7-category="[^"]+")?/,`class="product-art art-${category} v7-product-art" data-v7-category="${category}"`);
    return `<div class="product-visual${classes} v7-semantic-product-visual"${before}data-product-category="${category}" data-v7-category="${category}"${after}>${upgraded}`;
  });
  // Finder launch tiles already have a category URL, so use that as the deterministic semantic key.
  html=html.replace(/<a class="finder-launch([^"]*)"([^>]*)href="\/categories\/([^/]+)\/"([^>]*)>([\s\S]*?)<\/a>/g,(block,classes,before,category,after,inside)=>{
    if(!categoryMeta[category])return block;
    const upgraded=inside.replace(/<span class="category-icon[^>]*>[\s\S]*?<\/span>/,categoryScene(category,{compact:true}));
    return `<a class="finder-launch${classes} v7-semantic-launch"${before}href="/categories/${category}/"${after} data-v7-category="${category}">${upgraded}</a>`;
  });
  // Buying-guide cards map directly to the same 48 category slugs.
  html=html.replace(/<a class="([^"]*(?:editorial-card|guide-card)[^"]*)"([^>]*)href="\/guides\/([^/]+)-buying-guide\/"([^>]*)>([\s\S]*?)<\/a>/g,(block,classes,before,category,after,inside)=>{
    if(!categoryMeta[category])return block;
    const upgraded=inside.replace(/<span class="category-icon[^>]*>[\s\S]*?<\/span>/,categoryScene(category,{compact:true}));
    return `<a class="${classes} v7-semantic-guide"${before}href="/guides/${category}-buying-guide/"${after} data-v7-category="${category}">${upgraded}</a>`;
  });
  return html;
}
...
function enhance(html,path='/',u){
  ...
  html=enrichSemanticSurfaces(html);
