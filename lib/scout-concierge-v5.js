...
async function accountContext(auth){const saved=await savedProducts(auth);return {authenticated:!!auth,displayName:auth?core.displayName(auth.user):null,savedCount:saved.length,savedProducts:saved};}
...
async function handleScoutApi(req,res){
  ...
  try{
    const auth=await authContext(req,res),account=await accountContext(auth);
    if(req.method==='GET')return sendJson(res,200,{version:core.VERSION,account:publicAccount(account),privacy:{chatTranscriptPersisted:false,structuredDecisionStateSessionOnly:true},capabilities:['product-search','recommendation','comparison','product-facts','retailers','site-navigation','apg-knowledge','authenticated-saved-products']});
    ...
    let result=core.buildResponse({text,pageContext:body.pageContext,decisionState:body.decisionState,references:body.references,account});
    if(result.accountAction){
      if(!auth){result={...result,message:'You’ll need to sign in before I can change saved products in My APG. You can keep comparing products here without an account.',accountAction:null,actions:[{label:'Log in',url:'/my-apg/?account=login',kind:'link',primary:true},{label:'Keep shopping',url:'/categories/',kind:'link',primary:false}]};}
        ...
        const refreshed=await accountContext(auth),verb=result.accountAction.action==='save'?'added to':'removed from';
    ...
    if(!result.account)result.account=publicAccount(account);
...
Object.assign(handler,upstream,{VERSION,API_PATH,CSS_PATH,inject,authContext,savedProducts,accountContext,publicAccount,mutateSaved,handleScoutApi,core,client,brand});
