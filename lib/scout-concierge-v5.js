...
async function handleScoutApi(req,res){
  ...
  try{
    if(req.method==='GET'){
      ...
      return sendJson(res,200,{version:core.VERSION,account:publicAccount(account),privacy:{chatTranscriptPersisted:false,structuredDecisionStateSessionOnly:true},capabilities:['product-search','recommendation','comparison','product-facts','retailers','amazon-au','site-navigation','apg-knowledge','authenticated-saved-products']});
    }
    ...
    const pageContext=core.validatePageContext(body.pageContext||{}),intent=core.classifyIntent(text,pageContext),needsAccount=ACCOUNT_SENSITIVE_INTENTS.has(intent);
...
Object.assign(handler,upstream,{VERSION,API_PATH,CSS_PATH,ACCOUNT_SENSITIVE_INTENTS,inject,authContext,savedProducts,accountContext,publicAccount,mutateSaved,handleScoutApi,core,client,brand,amazon});
