...
// APG eBay family-variant guard v1.3.3
...
// explicit per APG product. Recommendation/commercial weighting = 0.
...
const FAMILY_VARIANTS={
  ...
  'anker-nebula-capsule-3':[
    {suffix:'laser',tokens:['capsule3laser','d2426']}
  ]
};
...
function applyToEnrichment(product,row){
  ...
  const evidenceText=acceptedEvidenceText(row.accepted);
  const variant=familyVariantConflict(product,evidenceText);
  const required=variant.conflict?{conflict:false}:requiredFamilyMarkerConflict(product,evidenceText);
