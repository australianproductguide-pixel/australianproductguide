...
function setAria(el,name,value){const next=String(value);if(el.getAttribute(name)!==next)el.setAttribute(name,next)}
function syncScoutAria(){const launcher=qs('#apgAssistantLauncher'),panel=qs('#apgAssistantPanel');if(!launcher||!panel)return;setAria(panel,'aria-hidden',panel.hidden);setAria(launcher,'aria-expanded',!panel.hidden)}
labelComparison();syncCompareTray();syncScoutAria();contextualPrompts();
const observer=new MutationObserver(()=>{labelComparison();syncCompareTray();syncScoutAria();contextualPrompts()});observer.observe(document.body,{subtree:true,childList:true,attributes:true,attributeFilter:['hidden']});
window.addEventListener('pageshow',()=>{labelComparison();syncCompareTray();syncScoutAria();contextualPrompts()});
})();`;
