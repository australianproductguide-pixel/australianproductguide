...
document.addEventListener('click',event=>{
  ...
  const placement=link.dataset.affiliatePlacement||'retailer_panel';
  const destinationKey=link.dataset.affiliateDestination||'';
  const payload={
    ...
    category:link.dataset.affiliateCategory||document.body?.dataset?.productCategory||'',
    placement,
    destination_key:destinationKey,
    referral_context:link.dataset.affiliateContext||placement,
  ...
  try{
    if(typeof window.gtag==='function'){
      ...
      if(destinationKey)window.gtag('event','amazon_shopping_click',payload);
    }
  }catch{}
},{capture:true});
})();
...
function reconcileImagePolicyCopy(html){
  ...
  body=body.replace(
    /Amazon imagery may be displayed only through a supported authorised mechanism such as Amazon Creators API and only for the matching product identifier\. Until that integration is authorised, (?:APG|Australian Product Guide)-owned visuals remain the fallback\./g,
    'Amazon imagery may be displayed only through a current Amazon Associates-approved mechanism and only for the matching product identifier. In the current pre-API phase, Australian Product Guide may use an approved manual Associates image/linking mechanism where available and documented. Australian Product Guide-owned visuals remain the fallback wherever compliant product photography has not been verified.'
  );
  body=body.replace(
    /Amazon's supported Associates product-data route is now Creators API\. (?:APG|Australian Product Guide) has prepared its retailer\/media data model for exact product identifiers and authorised image URLs, but no Amazon product photograph is displayed until the account has eligible access, credentials are configured securely and the returned product identifier matches the verified (?:APG|Australian Product Guide) product\./g,
    'Amazon Program Content may be displayed only when obtained through a current Amazon Associates-approved mechanism and matched to the verified Australian Product Guide product. In the current pre-API phase, Australian Product Guide can record approved manual Associates imagery, such as Basic Display imagery where that mechanism is offered, with its ASIN, rights basis, verification date and Amazon destination. Future authorised API access can automate the same controlled provider path.'
  );
  body=body.replace(
    /Amazon Product Advertising Content can be made available through supported Associates tools subject to programme terms\. PA-API has been deprecated; (?:APG|Australian Product Guide)'s planned supported product-data route is Amazon Creators API\. No Amazon product photograph is displayed until eligible API access and secure credentials are available and the image is returned for the exact matching product identifier\./g,
    'Amazon Program Content may be displayed only through a current Amazon Associates-approved mechanism and for the correctly matched product. During the current pre-API phase, Australian Product Guide may use approved manual Associates image/linking tools, such as Basic Display imagery where available, provided the image source, ASIN, rights basis, exact product match and Amazon destination are recorded. Future authorised API access can automate this controlled process; Australian Product Guide does not scrape Amazon product pages or fabricate API access.'
  );
  return body;
}
