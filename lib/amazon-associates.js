...
function decorateAmazonLinks(html){
  return String(html||'').replace(/<a\b([^>]*?)href="(https:\/\/www\.amazon\.com\.au\/[^\"]+)"([^>]*)>/gi,(full,before,href,after)=>{
    ...
    const placement=/\bdata-affiliate-placement="([^"]+)"/i.exec(full)?.[1]||'retailer_panel';
    return `<a${before}href="${href}"${after} data-affiliate-link data-affiliate-retailer="Amazon Australia" data-affiliate-kind="${kind}" data-affiliate-placement="${placement}">`;
  });
}
