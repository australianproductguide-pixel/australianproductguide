...
function queryCoverage(q,categorySlug){
  const tokens=[...new Set(words(q).filter(t=>t.length>=3&&!stop.has(t)&&!/^\d+$/.test(t)))];
  if(tokens.length<4||!categorySlug)return {tokens,matched:tokens.length,ratio:1};
  const category=categories[categorySlug]||{};
  const categoryProducts=catalogue.filter(p=>p.category===categorySlug).slice(0,40);
  const evidence=words([
    categorySlug,category.label,category.name,category.title,category.description,
    ...categoryProducts.flatMap(p=>[p.brand,p.name,p.model,...(p.tags||[]),...(p.highlights||[]),p.bestFor,p.watch])
  ].filter(Boolean).join(' '));
  const hay=new Set(evidence);
  const matched=tokens.filter(token=>hay.has(token)||evidence.some(x=>x.includes(token)||token.includes(x))).length;
  return {tokens,matched,ratio:tokens.length?matched/tokens.length:1};
}
function searchSite(q=''){
  ...
  const coverage=queryCoverage(q,category);
  const categoryOnlyLowCoverage=!!category&&coverage.tokens.length>=4&&coverage.ratio<0.5&&!modelProducts.length&&!brandExact&&!directCompare;
  const recognised=!String(q||'').trim()||modelProducts.length>0||(!categoryOnlyLowCoverage&&structuredIntent(state))||(old.categories||[]).length>0&&!categoryOnlyLowCoverage||lexicalIdentity(q,old.products)&&!categoryOnlyLowCoverage||!!directCompare;
  ...
  if(categoryOnlyLowCoverage)interpretation.push(`Low-confidence query coverage: ${coverage.matched}/${coverage.tokens.length} meaningful terms matched maintained ${categories[category]?.label||category} evidence.`);
  if(zeroResult)interpretation.push(zeroResult.message);
  return {...old,version:VERSION,decisionState:state,queryUnderstanding:{category,brand:model.brand||brandExact,modelTokens:model.tokens,modelMatchCount:modelProducts.length,modelAmbiguous,hardConstraints:isStrict,required:state?.hardConstraints?.requiredTags||[],excludedBrands:state?.hardConstraints?.excludedBrands||[],budget:state?.budget||null,numeric:state?.numericConstraints||[],coverage},interpretation,products:products.slice(0,18),categories:cats,comparisons,directCompare:recognised?directCompare:null,zeroResult,closestProducts};
}
module.exports={VERSION,searchSite,searchIndex:base.searchIndex,matchProduct:base.matchProduct,norm:base.norm,extractTags:base.extractTags,extractBudget:base.extractBudget,categoryFromQuery:base.categoryFromQuery,unsupportedCategoryFromQuery:base.unsupportedCategoryFromQuery,exactBrand,brandInQuery,modelTokens,modelCandidates,queryCoverage};