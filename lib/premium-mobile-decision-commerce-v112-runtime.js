...
// APG Premium Mobile Decision-Commerce v112
// Presentation and evidence-surfacing layer only. It does not change recommendation
// scoring, retailer weighting, product identity, privacy, authentication or decision state.
...
const dateLabel=value=>{if(!value)return 'Not separately recorded';const d=new Date(`${value}T00:00:00+10:00`);return Number.isNaN(d.getTime())?String(value):d.toLocaleDateString('en-AU',{day:'numeric',month:'short',year:'numeric',timeZone:'Australia/Sydney'});};
...
function retailerState(row){
  ...
  if(row.amazonMatchStatus==='EXACT_VERIFIED'||row.amazonModelMatch==='exact')return {rank:5,key:'exact',label:'Exact verified destination',detail:'Exact Amazon Australia product identity verified'};
  if(row.amazonMatchStatus==='VARIANT_VERIFIED'||row.amazonModelMatch==='verified-variant')return {rank:4,key:'variant',label:'Verified variant',detail:'Product family verified; the linked variant is explicitly identified'};
  if(row.kind==='affiliate-search'||row.amazonMatchStatus==='SEARCH_FALLBACK')return {rank:1,key:'fallback',label:'Model-search fallback',detail:'No exact Amazon Australia detail-page identity is being claimed'};
  if(row.exactModel===true)return {rank:5,key:'exact',label:'Exact AU retailer listing',detail:'Exact Australian retailer model/configuration verified'};
  ...
  if(row.url)return {rank:2,key:'listing',label:'Retailer pathway',detail:'Retailer destination recorded; recheck product identity and availability'};
...
function retailerRows(product){
  ...
  const seen=new Set();return rows.filter(row=>{const key=`${row.retailer||''}|${row.url||row.affiliateUrl||''}`;if(!row||!key||seen.has(key))return false;seen.add(key);return true;});
}
...
function priceState(product){
  const value=money(product.price);
  if(value&&product.lastPriceCheck)return {value,label:`${value} maintained price basis`,detail:`Price checked ${dateLabel(product.lastPriceCheck)} · recheck before purchase`,live:false};
  if(value)return {value,label:`${value} reference price`,detail:'No separate live-price check is recorded · recheck before purchase',live:false};
...
function productDepth(product){const c=categories[product.category];return c&&c.evidenceTier||product.evidenceTier||'maintained';}
function matchSignals(product,query){
  const tokens=String(query||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]+/g,' ').split(/\s+/).filter(x=>x.length>=3);
  if(!tokens.length)return [];
  const fields=[{label:'model',value:`${product.brand} ${product.name}`},...arr(product.tags).map(value=>({label:'need',value})),{label:'category',value:product.categoryLabel||product.category}];
  const out=[];
  for(const field of fields){const hay=String(field.value||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]+/g,' ');if(tokens.some(token=>hay.includes(token)))out.push(field.value);if(out.length===2)break;}
  return [...new Set(out)];
}

function productCardV2(product,{query=''}={}){
  const best=arr(product.tags)[0]||'maintained fit';
  const reasons=arr(product.highlights).slice(0,3);
  const tradeoff=String(product.watch||'').trim();
  ...
  const reviewed=product.lastSubstantiveReview||product.lastReviewed||product.lastSourceVerification||null;
  ...
  return `<article class="product-card apg112-product-card" data-apg112-product-card="${esc(product.slug)}">
    ...
    <div class="apg112-card-body">
      <div class="apg112-card-heading"><div><p class="eyebrow">${esc(product.brand)} · ${esc(human(productDepth(product)))} evidence</p><h3><a href="/products/${esc(product.slug)}/">${esc(product.name)}</a></h3></div><span class="apg112-freshness">Reviewed ${esc(dateLabel(reviewed))}</span></div>
      ...
      <div class="apg112-commerce-line">
        <span class="apg112-offer-state is-${esc(retailer.state.key)}"><strong>${esc(retailer.state.label)}</strong><small>${esc(retailerChecked?`Retailer identity checked ${dateLabel(retailerChecked)}`:'Retailer check date unavailable')}</small></span>
...
function renderOfferRow(product,row){
  ...
  const checked=row.checkedAt||row.amazonVerifiedAt||row.verified||product.lastRetailerCheck||null;
  const rowPrice=money(row.price);
  const variant=row.variant||row.amazonVariantMatch||null;
  const stock=row.availability==='listing-verified'?'Listing verified · live stock not maintained':'Current stock not maintained by APG';
...
function retailerPanelV2(product){
  ...
  return `<section class="retailer-panel apg112-offer-panel" id="where-to-buy" data-apg112-offer-layer="true">
    ...
    ${rows.map(row=>renderOfferRow(product,row)).join('')}
...
function categoryDepthBanner(slug){
  const category=categories[slug];if(!category)return '';
  const tier=category.evidenceTier||'maintained',deep=tier==='deep';
  return `<aside class="apg112-depth-banner ${deep?'is-deep':''}" data-apg112-depth="${esc(tier)}"><div><span>${deep?'Deep maintained category':'Maintained evidence tier'}</span><strong>${esc(deep?'Decision-critical evidence prioritised':human(tier))}</strong></div><p>${deep?'This category is one of APG’s deepest maintained decision sets. Evidence depth can improve confidence; it does not create a paid ranking advantage.':'This category remains useful, but APG does not present it as equivalent in evidence depth to a deep maintained category.'}</p></aside>`;
}
...
function homeEnhance(html){
  ...
  out=out.replace(/(<div class="category-grid premium-category-grid" data-apg112-home-categories="true">[\s\S]*?<\/div>)(\s*<\/section>)/,`$1<button class="button secondary apg112-show-categories" type="button" data-apg112-show-categories aria-expanded="false">Show all categories</button>$2`);
  return out;
}
...
function compareEnhance(html,compared){
  ...
  let out=html;
  if(out.includes('class="comparison-summary'))out=out.replace(/(<section class="comparison-summary[\s\S]*?<\/section>)/,`<div class="apg112-decision-first"><span>Decision first</span><strong>Start with who should choose each product, then inspect only the differences that can change the decision.</strong></div>$1${toolbar}`);
...
function transform(html,path,u){
  ...
  if(context.product)out=replaceRetailerPanel(out,context.product);
  ...
  return function premiumMobileCommerceV112(req,res){
    ...
    res.end=(body,...args)=>{
      ...
      if(req.method!=='HEAD'&&res.statusCode>=200&&res.statusCode<500&&type.startsWith('text/html')&&(typeof body==='string'||Buffer.isBuffer(body))){
        const original=Buffer.isBuffer(body)?body.toString('utf8'):body;let u;try{u=new URL(req.url,'https://australianproductguide.au')}catch{u=new URL('https://australianproductguide.au/');}
...
module.exports={VERSION,CSS_PATH,JS_PATH,PRODUCT_BY_SLUG,pageContext,retailerState,retailerRows,strongestRetailer,priceState,matchSignals,productCardV2,retailerPanelV2,comparisonToolbar,categoryDepthBanner,transform,wrap};
