...
function homeEnhance(html){
  ...
  if(rail&&!out.includes('data-apg112-depth-rail')){
    const mainClose=out.lastIndexOf('</main>');
    if(mainClose>=0){
      const marker='<section class="trust-strip',markerPos=out.indexOf(marker);
      const insertAt=markerPos>=0&&markerPos<mainClose?markerPos:mainClose;
      out=out.slice(0,insertAt)+rail+out.slice(insertAt);
    }
  }
  return out;
}
...
function wrap(downstream){
  if(typeof downstream!=='function')throw new TypeError('premium mobile decision-commerce requires downstream handler');
  ...
  Object.assign(handler,downstream,{
    PREMIUM_MOBILE_DECISION_COMMERCE_VERSION:VERSION,
    PREMIUM_MOBILE_DECISION_COMMERCE_CSS_PATH:CSS_PATH,
    PREMIUM_MOBILE_DECISION_COMMERCE_JS_PATH:JS_PATH,
    transformPremiumMobileDecisionCommerce:transform
  });
  return handler;
}
