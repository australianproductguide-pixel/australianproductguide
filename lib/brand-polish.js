...
function polishText(value){
  return String(value||'').replace(/—/g,'-').replace(/\bAPG\b/g,BRAND);
}
...
function polishHtml(html){
  ...
  const tokenised=String(html).replace(PROTECTED_BLOCK,block=>`\u0000APGPROTECTED${protectedBlocks.push(block)-1}\u0000`);
...
const replaceText=value=>String(value||'').replace(/—/g,'-').replace(/\\bAPG\\b/g,BRAND);
