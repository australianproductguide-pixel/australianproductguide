...
// APG eBay Buy Browse API client v1.3
...
function origins(cfg){
  ...
  return {
    root,
    oauth:`${root}/identity/v1/oauth2/token`,
    browse:`${root}/buy/browse/v1`,
    developerAnalytics:`${root}/developer/analytics/v1_beta`
  };
}
function diagnostics(env=process.env){
  ...
  return Object.freeze({
    ...
    developerAnalyticsOrigin:new URL(base.developerAnalytics).origin,
...
// eBay's Developer Analytics API is the authoritative source for the application's
// current Browse quota, usage and reset time. It uses the same client-credentials token
// but a separate developer endpoint, so checking quota does not spend Browse calls.
async function getRateLimits({apiName='browse',apiContext='buy',env=process.env,timeoutMs}={}){
  const cfg=requireConfiguration(env);
  const {developerAnalytics}=origins(cfg);
  const token=await getApplicationToken({env,timeoutMs});
  const url=addQuery(new URL(`${developerAnalytics}/rate_limit/`),{
    api_name:clean(apiName)||'browse',
    api_context:clean(apiContext)||'buy'
  });
  ...
  if(response.status===204)return {rateLimits:[]};
  const text=await response.text();
  let payload=null;
  if(text){try{payload=JSON.parse(text);}catch{payload=null;}}
  if(!response.ok||!payload||typeof payload!=='object'){
    throw new EbayBrowseApiError(`eBay Developer Analytics request failed with HTTP ${response.status}`,{status:response.status,code:'EBAY_DEVELOPER_ANALYTICS_HTTP_ERROR'});
  }
  return payload;
}
function rateLimitRows(payload,{apiName='browse',apiContext='buy'}={}){
  const wantedName=clean(apiName).toLowerCase();
  const wantedContext=clean(apiContext).toLowerCase();
  const limits=Array.isArray(payload&&payload.rateLimits)?payload.rateLimits:[];
  const rows=[];
  for(const block of limits){
    const name=clean(block&&block.apiName).toLowerCase();
    const context=clean(block&&block.apiContext).toLowerCase();
    if(wantedName&&name&&name!==wantedName)continue;
    if(wantedContext&&context&&context!==wantedContext)continue;
    const resources=Array.isArray(block&&block.resources)?block.resources:[];
    for(const resource of resources){
      const rates=Array.isArray(resource&&resource.rates)?resource.rates:[];
      for(const rate of rates){
        const limit=Number(rate&&rate.limit);
        const remaining=Number(rate&&rate.remaining);
        const count=Number(rate&&rate.count);
        const timeWindow=Number(rate&&rate.timeWindow);
        rows.push(Object.freeze({
          apiName:clean(block&&block.apiName)||wantedName||null,
          apiContext:clean(block&&block.apiContext)||wantedContext||null,
          apiVersion:clean(block&&block.apiVersion)||null,
          resource:clean(resource&&resource.name)||null,
          limit:Number.isFinite(limit)?limit:null,
          remaining:Number.isFinite(remaining)?remaining:null,
          count:Number.isFinite(count)?count:null,
          reset:clean(rate&&rate.reset)||null,
          timeWindow:Number.isFinite(timeWindow)?timeWindow:null
        }));
      }
    }
  }
...
function summariseRateLimits(payload,options={}){
  const apiName=clean(options.apiName||'browse')||'browse';
  const apiContext=clean(options.apiContext||'buy')||'buy';
  const resources=rateLimitRows(payload,{apiName,apiContext});
  const remaining=resources.map(row=>row.remaining).filter(Number.isFinite);
  const lowestRemaining=remaining.length?Math.min(...remaining):null;
  const candidates=resources
    .filter(row=>row.reset&&Number.isFinite(Date.parse(row.reset)))
    .sort((a,b)=>Date.parse(a.reset)-Date.parse(b.reset));
  return Object.freeze({
    found:resources.length>0,
    apiName,
    apiContext,
    exhausted:lowestRemaining==null?null:lowestRemaining<=0,
    lowestRemaining,
    resetAt:candidates.length?candidates[0].reset:null,
    resources
  });
}
...
module.exports={
  ...
  request,getRateLimits,rateLimitRows,summariseRateLimits,searchItems,getItem,safeItemProjection
};