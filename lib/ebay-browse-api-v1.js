...
// APG eBay Buy Browse API client v1.0
...
class EbayBrowseApiError extends Error{
  constructor(message,{status=null,code='EBAY_BROWSE_API_ERROR'}={}){
    super(message);
...
function environment(value){
  ...
  if(!['sandbox','production'].includes(out))throw new TypeError('EBAY_BROWSE_ENVIRONMENT must be sandbox or production');
  return out;
}
function configuration(env=process.env){
  return {
    ...
    environment:environment(env[ENV_KEYS.environment]||'sandbox'),
...
async function getApplicationToken({env=process.env,scope=DEFAULT_SCOPE,timeoutMs,forceRefresh=false}={}){
  ...
  const response=await fetchWithTimeout(oauth,{
    ...
    headers:{
      ...
      Authorization:basicAuthorization(cfg)
    },
    body:body.toString()
  },timeoutMs);
...
function safeItemProjection(item){
  ...
  return Object.freeze({
    ...
    seller:item.seller&&typeof item.seller==='object'?{username:clean(item.seller.username)||null,feedbackPercentage:clean(item.seller.feedbackPercentage)||null}:null,
...
module.exports={
  VERSION,ENV_KEYS,DEFAULT_SCOPE,EbayBrowseApiError,configuration,diagnostics,requireConfiguration,
  getApplicationToken,request,searchItems,getItem,safeItemProjection
};
