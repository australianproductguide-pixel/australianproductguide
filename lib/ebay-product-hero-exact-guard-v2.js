...
// APG eBay product hero exact-identity guard v2.0.
// Presentation-only safety layer. It deliberately accepts fewer listings than the wider
// eBay enrichment programme: a product hero is only eligible when current eBay detail has
// structured model evidence and no accessory, bundle, pack, sibling-model or lifecycle risk.
...
const ACCESSORY_CATEGORY_PATTERNS=[
  ...
  /\bremote\s+controls?\b/i,/\bpower\s+tool\s+batter(?:y|ies)\b/i,/\bsmall\s+kitchen\s+appliance\s+accessories\b/i
];
...
function modelEvidenceCheck(product,accepted){
  ...
  if(!evidence.length)return {ok:false,reason:'structured-model-evidence-missing',expected,evidence};
  ...
  const hit=ids.filter(token=>new RegExp(`(?:^|\\s)${norm(token)}(?:$|\\s)`,'i').test(norm(title)));
  const coverage=ids.length?hit.length/ids.length:0;
...
function evaluate(product,row,allProducts=[]){
  ...
  if(accepted.verificationLevel!=='detail-model-evidence')return {eligible:false,reason:'structured-model-evidence-required'};
...
module.exports={
  ...
  modelEvidenceCompatible,modelEvidenceCheck,siblingModelConflict,exactEbayImage,exactEbayItemUrl,evaluate
};
