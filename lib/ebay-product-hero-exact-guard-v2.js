...
// APG eBay product hero exact-identity guard v2.8.
...
// the maintained product identity and no accessory, bundle, pack, regional-voltage or sibling-model/variant risk remains.
// v2.8 adds an item-bound manufacturer-SKU alias for the Australian Dyson V15s Detect Submarine
// Complete (448842-01), while retaining the independently verified Echo Show 5 manufacturer alias.
// It also treats bread pans, remote controls and ink cartridges as explicit accessory-only titles.
// Aliases remain title-bound (and item-bound where seller-specific), while all accessory, sibling,
// variant, AUD-price, image-host, item-URL and active-listing controls remain unchanged.
...
const ACCESSORY_TITLE_PATTERNS=[
  ...
  /\bconversion\s+(?:chart\s+)?plate\b/i,/\bbread\s+pan\b/i,/\bremote\s+controls?\b/i,/\bink\s+cartridges?\b/i,
  /\bsilicone\b[^,;]{0,60}\bliner\b/i,/\bliner\s+for\b/i,/\boutlet\s+cover\b/i,
  /\bpalm[\s-]*rests?\b/i,/\bwrist[\s-]*rests?\b/i,
...
// Known same-family variants that share a base retail model token but are materially different
// products. Keep these product-scoped rather than adding ambiguous terms globally. A strong exact
// identifier can override an exclusion only when it proves the maintained variant.
const PRODUCT_VARIANT_EXCLUSIONS={
  'anker-solix-c300':[
    {
      code:'C300_DC_SIBLING',
      pattern:/\b(?:anker\s+)?(?:solix\s+)?c300\b[^,;]{0,80}\bdc\b/i,
      allowPattern:/\ba1722[a-z0-9-]*\b/i
    }
  ],
  'karcher-k5-premium-smart-control':[
    {
      code:'K5_1324_681_0_SIBLING',
      pattern:/\b1[.\s-]?324[.\s-]?681[.\s-]?0\b/i,
      allowPattern:/\b1[.\s-]?324[.\s-]?673[.\s-]?0\b/i
    }
  ]
};

// Evidence-bound aliases for products where eBay's structured Model/MPN field uses a manufacturer SKU
// (or, for one current Samsung listing, a documented I/l transcription error) rather than the retail model
// token in APG. Manufacturer-SKU rules remain title-bound; seller-specific/typo rules are also item-bound.
const VERIFIED_MODEL_ALIASES=Object.freeze({
  'amazon-echo-show-5-3rd-gen':Object.freeze([
    Object.freeze({evidence:Object.freeze(['B09B2RL8CG']),itemIds:Object.freeze([]),titlePhrases:Object.freeze(['amazon','echo show 5','3rd gen']),proof:'manufacturer-model'})
  ]),
  'apple-ipad-a16-128gb':Object.freeze([
    Object.freeze({evidence:Object.freeze(['KHMD4A4XA']),itemIds:Object.freeze(['v1|198078800527|0']),titlePhrases:Object.freeze(['apple','ipad','a16','128gb','wi fi']),proof:'evidence-bound-current-item'})
  ]),
  'asus-tuf-gaming-vg27aql3a':Object.freeze([
    Object.freeze({evidence:Object.freeze(['90LM09A0-B01370']),itemIds:Object.freeze([]),titlePhrases:Object.freeze(['asus','vg27aql3a']),proof:'manufacturer-sku'})
  ]),
  'corsair-k70-core-tkl':Object.freeze([
    Object.freeze({evidence:Object.freeze(['67738']),itemIds:Object.freeze(['v1|237000834483|0']),titlePhrases:Object.freeze(['corsair','k70 core tkl','mlx red']),proof:'evidence-bound-current-item-with-verified-vendor-sku'})
  ]),
  'dyson-v15s-detect-submarine-complete':Object.freeze([
    Object.freeze({evidence:Object.freeze(['448842-01']),itemIds:Object.freeze(['v1|375684597812|0']),titlePhrases:Object.freeze(['dyson','v15s detect submarine complete','448842 01']),proof:'item-bound-manufacturer-sku'})
  ]),
  'elgato-facecam-mk-2':Object.freeze([
    Object.freeze({evidence:Object.freeze(['10WAC9901']),itemIds:Object.freeze([]),titlePhrases:Object.freeze(['elgato','facecam','mk 2']),proof:'manufacturer-sku'})
  ]),
  'samsung-galaxy-smarttag2':Object.freeze([
    Object.freeze({evidence:Object.freeze(['EI-T5600']),itemIds:Object.freeze([]),titlePhrases:Object.freeze(['samsung','smarttag2']),proof:'manufacturer-model'}),
    Object.freeze({evidence:Object.freeze(['El-T5600']),itemIds:Object.freeze(['v1|296082302198|594211313158']),titlePhrases:Object.freeze(['samsung','smarttag2','1 pack','ei t5600']),proof:'evidence-bound-current-item-structured-mpn-transcription'})
  ])
});

function clean(value){return String(value==null?'':value).trim();}
function norm(value){return clean(value).toLowerCase().normalize('NFKD').replace(/[\u0300-\u036f]/g,'').replace(/[’']/g,'').replace(/[^a-z0-9]+/g,' ').replace(/\s+/g,' ').trim();}
...
function verifiedModelAlias(product,accepted,evidence=evidenceModels(accepted)){
  const rules=VERIFIED_MODEL_ALIASES[clean(product&&product.slug)]||[];
  if(!rules.length||!evidence.length)return {ok:false};
  const itemId=clean(accepted&&accepted.itemId);
  const titleNorm=norm(accepted&&accepted.title);
  const titleCompact=compact(accepted&&accepted.title);
  for(const rule of rules){
    const itemIds=Array.isArray(rule&&rule.itemIds)?rule.itemIds:[];
    if(itemIds.length&&!itemIds.includes(itemId))continue;
    const aliases=Array.isArray(rule&&rule.evidence)?rule.evidence:[];
    const matchedEvidence=evidence.find(value=>aliases.some(alias=>compact(value)===compact(alias)));
    if(!matchedEvidence)continue;
    const phrases=Array.isArray(rule&&rule.titlePhrases)?rule.titlePhrases:[];
    const titleOk=phrases.every(phrase=>{
      const wanted=norm(phrase);const wantedCompact=compact(phrase);
      return Boolean(wanted)&&((` ${titleNorm} `).includes(` ${wanted} `)||(wantedCompact&&titleCompact.includes(wantedCompact)));
    });
    if(!titleOk)continue;
    return {ok:true,evidence:matchedEvidence,proof:clean(rule.proof)||'verified-model-alias'};
  }
  return {ok:false};
}
...
function modelEvidenceCheck(product,accepted){
  ...
  const alias=verifiedModelAlias(product,accepted,evidence);
  if(alias.ok)return {ok:true,reason:'verified-structured-model-alias',expected,evidence,matched:[],alias};
  if(specific.length){
    const matched=specific.filter(model=>evidence.some(value=>modelEvidenceCompatible(model,value)));
    if(matched.length)return {ok:true,reason:'specific-model-evidence',expected,evidence,matched};
    return {ok:false,reason:'structured-model-evidence-mismatch',expected,evidence};
  }
...
function productVariantConflict(product,accepted){
  const rules=PRODUCT_VARIANT_EXCLUSIONS[clean(product&&product.slug)]||[];
  if(!rules.length)return {conflict:false};
  const text=evidenceText(accepted);
  for(const rule of rules){
    if(!rule||!(rule.pattern instanceof RegExp)||!rule.pattern.test(text))continue;
    if(rule.allowPattern instanceof RegExp&&rule.allowPattern.test(text))continue;
    return {conflict:true,reason:'product-variant-exclusion',code:clean(rule.code)||'KNOWN_SIBLING_VARIANT'};
  }
  return {conflict:false};
}
...
function evaluate(product,row,allProducts=[],{now=Date.now()}={}){
  ...
  const voltage=matcher.regionalVoltageConflict(evidenceText(accepted));
  if(voltage.conflict)return {eligible:false,reason:'regional-voltage-mismatch',detail:voltage};
  ...
  const knownVariant=productVariantConflict(product,accepted);
  if(knownVariant.conflict)return {eligible:false,reason:knownVariant.reason,detail:knownVariant};
...
module.exports={
  VERSION,ACCESSORY_TITLE_PATTERNS,ACCESSORY_CATEGORY_PATTERNS,PRODUCT_VARIANT_EXCLUSIONS,VERIFIED_MODEL_ALIASES,clean,norm,compact,productIdentity,evidenceModels,evidenceText,
  titleLooksAccessory,categoryLooksAccessory,compatibilityTargetConflict,packCounts,packOrBundleConflict,isSpecificModel,
  modelEvidenceCompatible,verifiedModelAlias,titleModelCandidates,titleIdentityCoverage,titleSpecificModelCheck,modelEvidenceCheck,siblingModelConflict,productVariantConflict,
