...
// APG eBay product hero exact-identity guard v2.6.
...
// v2.6 adds tightly bounded manufacturer-SKU/current-item aliases for five exact products whose eBay
// structured Model/MPN value does not repeat APG's retail model token. The aliases require matching
// title identity and, where evidence is seller-specific or typoed, the exact current eBay item ID.
// It also blocks conversion-chart plates as accessories. Existing sibling and variant controls remain unchanged.
...
function modelEvidenceCheck(product,accepted){
  ...
  if(specific.length){
    ...
    return alias.ok
      ?{ok:true,reason:'verified-structured-model-alias',expected,evidence,matched:[],alias}
      :{ok:false,reason:'structured-model-evidence-mismatch',expected,evidence};
  }
