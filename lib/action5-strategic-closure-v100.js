...
const P1_RESOLUTIONS=Object.freeze({
  ...
  'philips-3000-series-dual-basket-na35310':{
    status:'CLOSED_VERIFIED_DIRECT',
    canonicalModel:'NA353/10',
    resolution:'Australian canonical product identity is confirmed. ASIN B0DK74HSQC is now promoted as an exact verified destination after model NA353/10, 9L capacity and White Body / Champagne Handle configuration were independently corroborated at HIGH confidence.',
    amazonState:'EXACT_VERIFIED',
    evidence:[
      'https://www.philips.com.au/c-p/NA353_10/3000-series-dual-basket-airfryer',
      'https://device.report/philips/NA353-10'
    ]
  },
...
function structuralSnapshot(){
  ...
  const gateChecks={
    ...
    noGuessedP1Asins:Object.entries(P1_RESOLUTIONS).every(([slug,row])=>{
      const hit=amazon.VERIFIED[slug];
      if(slug===RECALL_SLUG)return !hit;
      if(row.amazonState==='EXACT_VERIFIED')return hit?.matchStatus==='EXACT_VERIFIED'&&hit?.confidence==='HIGH';
      if(row.amazonState==='VARIANT_VERIFIED')return hit?.matchStatus==='VARIANT_VERIFIED'&&hit?.confidence==='HIGH';
      return !hit;
    }),
