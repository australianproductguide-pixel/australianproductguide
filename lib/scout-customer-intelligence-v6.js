...
// URL/query text here is a backwards-compatible navigation handoff only. Internal Scout
// recommendation evaluation remains canonical structured Decision State v2 and never
// reparses this generated prose to score candidates.
function decisionLabHandoff(state,pageContext={}){
  const u=new URL('/decision-lab/','https://australianproductguide.au');
  const safe=state&&typeof state==='object'?state:{};
  const category=contextCategory(pageContext,safe);
  const generated=core.stateToQuery(safe);
  const requiredBrands=uniq(safe.hardConstraints&&safe.hardConstraints.requiredBrands||[]).slice(0,4);
  const requiredBrandText=requiredBrands.map(brand=>String(brand).trim()).filter(Boolean).map(brand=>brand+' only').join(' ');
  const pageQuery=String(pageContext.currentSearchQuery||'').trim().slice(0,300);
  const q=[generated,requiredBrandText].filter(Boolean).join(' ').trim()||pageQuery;
  if(q)u.searchParams.set('q',q.slice(0,600));
  if(category)u.searchParams.set('category',category);
  const budget=Number(safe.budget&&safe.budget.amount);if(Number.isFinite(budget)&&budget>0)u.searchParams.set('budget',String(budget));
  if(safe.brandPreference)u.searchParams.set('brand',String(safe.brandPreference).slice(0,80));
...
function structuredRecommendationParity(out,input,pageContext){
  ...
  actions.push(link('Refine in Decision Lab',decisionLabHandoff(preserved,pageContext),cards.length===1));
  actions.push(prompt('Why did this one win?'));
  actions.push(prompt('What would change this recommendation?'));
  return {...out,message,products:cards,references:cards.map(card=>card.slug),decisionState:preserved,actions,meta:{...(out.meta||{}),scoutCustomerIntelligenceVersion:VERSION,decisionStateSchema:decisionState.VERSION,decisionInputMode:'structured-state',decisionLabHandoff:'context-preserving-navigation-only',hardConstraintFallback:!!(result.audit&&result.audit.hardConstraintFallback),commercialRecommendationWeight:0}};
}
...
module.exports={VERSION,install,pageHelp,decisionLabHandoff,structuredRecommendationParity,get installed(){return installed;}};
