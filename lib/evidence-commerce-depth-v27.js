...
function transform(html,pathOrUrl){
  ...
  let out=app.transform?app.transform(String(html||''),pathOrUrl):String(html||'');
  ...
  out=out.replace(/<body([^>]*)>/i,(m,a)=>`<body${a} data-evidence-commerce-v27="true">`);
...
module.exports=(req,res)=>{
  ...
  res.end=(body,...args)=>{
    ...
    if(req.method!=='HEAD'&&res.statusCode===200&&typeof body==='string'&&type.startsWith('text/html'))body=transform(body,url.href);
    res.setHeader('X-APG-Experience','evidence-commerce-depth-v27');
