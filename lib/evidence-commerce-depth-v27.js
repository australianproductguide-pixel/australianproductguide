...
function injectCoverageNote(html,path){
  ...
  // Category templates do not all share the same national-shortcuts boundary. Fall
  // back to the semantic main boundary so the evidence note is not silently lost
  // when a category uses a richer or older layout variant.
  if(html.includes('</main>'))return html.replace('</main>',note+'</main>');
  return html;
}
