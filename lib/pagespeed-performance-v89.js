...
// APG PageSpeed Performance v89.1.
// Evidence-led mobile remediation against the 23 Aug 2026 PSI run (81 mobile / 97 desktop).
// v89.1 hardens the v89 critical CSS bundle after Production verification found that
// membership-proof-v19.css is a virtual/redirected asset in the internal runtime chain.
// That stylesheet therefore remains an ordinary direct request; only assets proven safe
// for internal capture are consolidated. No consumer content, recommendation, analytics,
// affiliate, privacy, SEO or agentic-discovery behaviour is changed.
...
const PAGESPEED_PERFORMANCE_VERSION='89.1';
...
const HOME_CRITICAL_CSS=[
  ...
  '/assets/mobile-account-proof-v20.css','/assets/mobile-menu-polish-v21.css',
...
function captureAsset(path){
  ...
  if(type&&!type.startsWith('text/css'))throw new Error(`Unable to capture ${path}: unexpected content type ${type}`);
  if(!body.trim())throw new Error(`Unable to capture ${path}: empty stylesheet`);
  return body;
}
...
function sendCriticalCss(req,res){const body=homeCriticalCss();res.statusCode=200;res.setHeader('Content-Type','text/css; charset=utf-8');res.setHeader('Cache-Control','public, max-age=31536000, immutable');res.setHeader('X-Content-Type-Options','nosniff');res.setHeader('X-APG-PageSpeed-Performance','v'+PAGESPEED_PERFORMANCE_VERSION);return res.end(req.method==='HEAD'?'':body)}
