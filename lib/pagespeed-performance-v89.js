...
// Preserve exact source order. These styles are still required for the rendered homepage,
// but paying a separate blocking network round-trip for each file is unnecessary.
// site-optimised.css stays separate because it is the established core bundle and is
// already long-cacheable. v88 non-critical styles remain asynchronous and are not bundled.
...
function consolidateHomepageCss(html){
  let out=String(html||'');
  let firstIndex=-1;
  const pattern=/<link rel="stylesheet" href="([^"]+)">/g;
  out=out.replace(pattern,(full,href,offset)=>{
    if(!HOME_CRITICAL_SET.has(pathOf(href)))return full;
    if(firstIndex<0)firstIndex=offset;
    return '';
  });
  if(firstIndex<0||out.includes(HOME_CRITICAL_CSS_PATH))return out;
  const href=`${HOME_CRITICAL_CSS_PATH}?v=${encodeURIComponent(BUILD_ID)}`;
  const tag=`<link rel="stylesheet" href="${href}" data-apg-critical-css="v${PAGESPEED_PERFORMANCE_VERSION}">`;
  const core=/<link rel="stylesheet" href="\/assets\/site-optimised\.css[^"]*">/;
  if(core.test(out))return out.replace(core,m=>m+tag);
  return out.replace('</head>',tag+'</head>');
}
function versionMutableAssets(html){
  // The PSI run also reported a small cache-lifetime opportunity. Only rewrite APG-owned
  // assets whose responses are deterministic for the deployment, then mark matching
  // versioned requests immutable below. This avoids stale privacy or illustration assets.
  return String(html||'').replace(/\b(src|href)="(\/assets\/(?:privacy-experience|illustrative-experience)\.(?:js|css))"/g,
    (m,attr,url)=>`${attr}="${url}?v=${encodeURIComponent(BUILD_ID)}"`);
}
function transformHtml(html,path){
  let out=String(html||'');
  if(path==='/')out=consolidateHomepageCss(out);
  out=versionMutableAssets(out);
  if(!out.includes('name="apg-pagespeed-performance-v89"'))out=out.replace('</head>',`<meta name="apg-pagespeed-performance-v89" content="v${PAGESPEED_PERFORMANCE_VERSION}"></head>`);
  return out;
}
function handler(req,res){
  const url=requestUrl(req);
  if(url.pathname===HOME_CRITICAL_CSS_PATH){
    res.setHeader('X-APG-PageSpeed-Performance','v'+PAGESPEED_PERFORMANCE_VERSION);
    return sendCriticalCss(req,res);
  }
  ...
  res.end=(body,...args)=>{
    ...
    if(url.pathname.startsWith('/assets/')&&url.searchParams.get('v')===BUILD_ID){
      res.setHeader('Cache-Control','public, max-age=31536000, immutable');
    }
...
Object.assign(handler,downstream,{
  PAGESPEED_PERFORMANCE_VERSION,HOME_CRITICAL_CSS_PATH,HOME_CRITICAL_CSS,
  pathOf,captureAsset,homeCriticalCss,consolidateHomepageCss,versionMutableAssets,transformHtml
});
